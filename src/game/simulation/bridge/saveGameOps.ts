  ...
  function saveGame(): SaveBlob {
    ...
    return {
      ...
      sideMaps: {
        ...
        monkCarriedRelic: [...accessor.get(monkCarriedRelicCodec).entries()],
        monkHealCounters: [...accessor.get(monkHealCountersCodec).entries()],
        relicsInMonastery: [...accessor.get(relicsInMonasteryCodec).entries()],
        ...
        relicCountdowns: [...accessor.get(relicCountdownsCodec).entries()].map(([id, entry]) => [
          id,
          {
            remainingTicks: entry.remainingTicks,
            totalTicks: entry.totalTicks,
        ...
        lastSeenStatic: [...accessor.get(lastSeenStaticCodec).entries()].map(([playerId, innerMap]) => [
          playerId,
        ...
        productionQueues: [...accessor.get(productionQueuesCodec).entries()].map(([id, queue]) => [
          id,
          queue.map((entry) => ({
            kind: entry.kind,
            label: entry.label,
        ...
        constructionStates: [...accessor.get(constructionStatesCodec).entries()].map(([id, state]) => [
          id,
          { ...state },
        ]),
        combatStates: [...accessor.get(combatStatesCodec).entries()].map(([id, state]) => [id, { ...state }]),
        buildingHealthStates: [...accessor.get(buildingHealthStatesCodec).entries()].map(([id, state]) => [
          id,
          { ...state },
        ]),
        ...
        aiStates: [...accessor.get(aiStatesCodec).entries()].map(([owner, state]) => [
          owner,
          {
