...
// Monk conversion range (AoE2-faithful since v0.3.91): base 9, Block Printing
// +3 → 12. Discriminator fixtures — a player-1 Monk BOXED by trees at (10,10)
// so it cannot walk closer, with an enemy villager at Manhattan `distance`.
// At 9 the bare monk converts (positive base-range proof); at 10 it never can
// (10 > 9) until Block Printing stretches the reach to 12. The test seeds the
// convert task directly (initialMonkTasks) so no auto-search is involved.
function createBlockPrintingScenario(
  seed: string,
  researched: boolean,
  distance: number,
): PrototypeScenario {
  ...
  return {
    ...
    spawns: [
      ...
      // Enemy TRADE CART at the measured distance: it never auto-gathers
      // (a villager would walk to the box trees and wander), never fights
      // (no-attack stance), so the distance under test actually holds.
      ownedSpawn('trade-cart', 2, monk.x + distance, 10, { vision: 3 }),
...
export function createMonkBlockPrintingBaselineFixture(seed: string): PrototypeScenario {
  return createBlockPrintingScenario(seed, false, 10);
}

export function createMonkBlockPrintingFixture(seed: string): PrototypeScenario {
  return createBlockPrintingScenario(seed, true, 10);
}

export function createMonkBaseConvertRangeFixture(seed: string): PrototypeScenario {
  return createBlockPrintingScenario(seed, false, 9);
}
