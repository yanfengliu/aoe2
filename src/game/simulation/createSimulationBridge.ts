...
export function createSimulationBridge(
  ...
  const issueContextCommandAtEntity = (entityId: number, options?: { garrison?: boolean; forceAttack?: boolean; queue?: boolean }): boolean => {
    const didIssue = ack(issueContextCommandAtEntityInternal(
      entityId, options?.garrison ?? false, options?.forceAttack ?? false, options?.queue ?? false,
    ));
    if (!didIssue) {
      return false;
    }
