...
// pointer→cell hit-test runs through `isoToWorld`. No Phaser dependency, no
// state — a rotation+scale of the square grid into a diamond layout.
//
// The camera (a normal Phaser 2D camera) still pans/zooms over the resulting
// layout; only the per-cell placement changes from `cell * cellSize` (top-down)
// to `worldToIso(cell)` (isometric).
