...
// kinds is the tile edge, and sand and dirt have hard edges; the fog is the blend's. An unexplored cell is black,
// known ground fades to black over the half tile beside one (the known field runs from 1 at a known cell's centre
// to a half at its edge with an unexplored cell), and the explored level is the same filtered byte.
//
...
vec4 deGround( vec2 p, vec2 gx, vec2 gy ) {
	...
	float fade = kind == 0 ? 0.0 : smoothstep( 0.5, 1.0, field.g );
