...
function saddleAndRider(context: UnitRecipeContext, unitType: UnitType, profile: UnitVisualProfile): void {
  ...
  add(context, 'cavalry-rider-tunic', profile.armor === 'plate' ? 'metal' : 'matte', profile.armor === 'plate' ? VOXEL_COLORS.steel : context.team, 0, 0.91, 0, 0.38, 0.52, 0.3);
  add(context, 'cavalry-rider-arm-left', profile.armor === 'plate' ? 'metal' : 'matte', profile.armor === 'plate' ? VOXEL_COLORS.steelDark : context.team, -0.29, 1.01, 0.04, 0.13, 0.46, 0.14, { roll: -0.16 });
  add(context, 'cavalry-rider-arm-right', profile.armor === 'plate' ? 'metal' : 'matte', profile.armor === 'plate' ? VOXEL_COLORS.steelDark : context.team, 0.29, 1.01, -0.03, 0.13, 0.46, 0.14, { roll: 0.16 });
  ...
  add(context, 'cavalry-rider-helmet', helmetMetal ? 'metal' : 'matte', helmetMetal ? VOXEL_COLORS.steel : VOXEL_COLORS.leather, 0, 1.67, 0, profile.headgear === 'crested-helmet' ? 0.36 : 0.32, 0.16, 0.3);
  if (profile.headgear === 'crested-helmet') {
    add(context, 'cavalry-rider-crest', 'matte', context.team, 0, 1.83, 0, 0.12, 0.26, 0.28);
  }
  if (profile.armor === 'mail') {
    add(context, 'cavalry-rider-mail', 'metal', VOXEL_COLORS.steelDark, 0, 0.92, 0.01, 0.4, 0.42, 0.32);
  }
  const signatureTint = profile.tier === 3 ? VOXEL_COLORS.gold : shade(context.team, 0.72);
  add(context, `detail-${unitType}-${profile.signature}`, 'matte', signatureTint, -0.18, 0.72, 0.27, 0.18 + profile.tier * 0.04, 0.32, 0.08, { yaw: -0.45 });
}
...
function mountedShield(context: UnitRecipeContext, profile: UnitVisualProfile): void {
  ...
  add(context, 'cavalry-shield', 'matte', shade(context.team, 0.76), -0.34, 0.98, 0.28, width, height, 0.1);
  add(context, 'cavalry-shield-boss', 'metal', VOXEL_COLORS.steel, -0.34, 1.12, 0.34, 0.1, 0.1, 0.07);
}

function mountedWeapon(context: UnitRecipeContext, profile: UnitVisualProfile): void {
  if (profile.weapon === 'mounted-bow') {
    add(context, 'cavalry-archer-bow-upper', 'matte', VOXEL_COLORS.timber, 0.38, 1, 0.2, 0.06, 0.65, 0.06, { roll: -0.5 });
    add(context, 'cavalry-archer-bow-lower', 'matte', VOXEL_COLORS.timber, 0.38, 0.48, 0.2, 0.06, 0.65, 0.06, { roll: 0.5 });
    add(context, 'cavalry-archer-bow-grip', 'matte', VOXEL_COLORS.leather, 0.38, 0.75, 0.2, 0.1, 0.24, 0.09);
    add(context, 'cavalry-archer-quiver', 'matte', VOXEL_COLORS.leather, -0.27, 0.91, -0.24, 0.16, 0.62, 0.15, { roll: -0.22 });
    return;
  }
  if (profile.weapon === 'mounted-sword') {
    add(context, 'cavalry-sword', 'metal', VOXEL_COLORS.steel, 0.43, 0.86, -0.15, 0.09, 0.94, 0.08, { roll: -0.43 });
    add(context, 'cavalry-sword-hilt', 'metal', VOXEL_COLORS.gold, 0.27, 0.78, -0.12, 0.3, 0.09, 0.11, { roll: -0.43 });
    return;
  }
  if (profile.weapon === 'mounted-polearm') {
    add(context, 'cavalry-polearm-shaft', 'matte', VOXEL_COLORS.timber, 0.43, 0.6, -0.3, 0.08, 1.55, 0.08, { roll: -0.64 });
    add(context, 'cavalry-polearm-head', 'metal', VOXEL_COLORS.steel, 0.86, 1.78, -0.56, 0.17, 0.38, 0.1, { roll: -0.64 });
    return;
  }
  add(context, 'cavalry-lance', 'matte', VOXEL_COLORS.timber, 0.48, 0.74, -0.35, 0.07, 1.45, 0.07, { roll: -0.72 });
  add(context, 'cavalry-lance-tip', 'metal', VOXEL_COLORS.steel, 0.96, 1.58, -0.72, 0.14, 0.36, 0.09, { roll: -0.72 });
}
