...
// routine for every texture read, and on the default map most known ground genuinely blends two surfaces
// (docs/debugging/2026-09-24-de-ground-swiftshader-cost.md). With Natural the default in v0.3.233, CI's live frame
// doubled (233.3 ms against Moebius's 116.6) and two frame-timed specs failed. On CI's four vCPUs the opening view's
// frame costs 2.12 of Moebius's with the blend and 0.85-0.87 with one sample per pixel
// (tests/browser/de-ground-frame-cost.spec.ts, run 36100866192). On an RTX 4090 every arm took 2.2 to 2.7 ms, so a
// graphics card keeps the blend.
//
