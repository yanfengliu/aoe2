...
The original procedural art uses role-specific building masses with concrete-type facade and equipment details, animated rigid-part units, varied resources, contact shadows, material palettes, and terrain-specific surface grammar: grass flecks and tufts, forest litter and logs, hill strata and rocks, plus water with moving wave crests, shoreline foam, and low-roughness sky-and-sun reflection cues. Unit and moving-wildlife roots interpolate from the exact preceding simulation tick, including when one browser frame spans multiple ticks. Grounded feet, paired limbs, cavalry legs, and siege wheels advance by displayed travel distance so cadence follows visible speed. Travel direction eases through path corners, the complete actor body turns through the shortest arc to face that direction, feet flex in the travel plane, and simulation pause freezes gait, water, identity-phased breathing, secondary motion, and attack geometry on one monotonic display-progress clock. A perspective-eligible successful unit hit produces an immediate target-facing rigid-part strike and smooth recovery; simultaneous attackers animate independently, while units still approaching retain locomotion. Simulation state, saves, replays, commands, damage, reloads, roots, terrain occupancy, and hit rules remain authoritative outside the renderer.
