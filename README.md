...
The original procedural art uses role-specific building masses with concrete-type facade and equipment details, varied resources, sun-cast ground shadows, material palettes, and terrain-specific surface grammar: grass flecks and tufts, forest litter and logs, hill strata and rocks, plus water with moving wave crests, shoreline foam, and low-roughness sky-and-sun reflection cues. All 34 unit types select an exhaustive armor, headgear, shield, mount, weapon, and signature-detail profile through seven shared body families; matching attack rigs chop, slash, cleave, thrust, draw, cast, recoil, or release the concrete equipment that is shown. Unit and moving-wildlife roots interpolate from the exact preceding simulation tick, including when one browser frame spans multiple ticks. Grounded feet, paired limbs, cavalry legs, and siege wheels advance by displayed travel distance so cadence follows visible speed. Travel direction eases through path corners, the complete actor body turns through the shortest arc to face that direction, feet flex in the travel plane, and simulation pause freezes gait, water, identity-phased breathing, secondary motion, and attack geometry on one monotonic display-progress clock. A perspective-eligible successful unit hit produces an immediate target-facing rigid-part strike and smooth recovery; simultaneous attackers animate independently, while units still approaching retain locomotion. Simulation state, saves, replays, commands, damage, reloads, roots, terrain occupancy, and hit rules remain authoritative outside the renderer.
...
2. Start the app (`npm run dev`), open the **☰ game menu**, and choose the replay-film icon whose accessible name and tooltip read **Watch a replay…**.
