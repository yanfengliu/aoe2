...
// gives the three attack orders a player can give, at a unit, at a building
// and at the ground. On the tick the attack lands it takes the landing point
// from the shot itself: a shot at a unit or at the ground must land on the
// very cell it was aimed at, and one at a building on the building. Every unit
// within the table's radius of the landing point must be hurt and every other
// one spared, and the witness the fixture stands on the farthest cell inside
// the radius must be among the hurt, so a blast that reached nobody cannot
// pass. The ground order is judged against DE's own
// roster below, not against the validator: a blast unit DE lets attack the
// ground must blast there, and every other unit, blast or not, is refused.
//
...
//  - The three orders a player gives. Auto-attack, attack-move and patrol
//    deliver through the same two functions as an ordered attack
//    (`deliverUnitAttackOnUnit`, `deliverUnitAttackOnBuilding`), but no case
//    drives them. An attack on WILDLIFE takes another path, which lands its
//    damage at once with no shot and no blast; no case here reaches it.
//  - A detonation fires no shot to read, so its landing point is taken as its
//    target's cell, where the code centres it. A shot at a building lands on
//    its anchor cell here; the census asks only that it land on the footprint,
//    which DE's centre would too. Spec §10.7 records how DE differs on both.
...
//  - Blast on units only. The blast does not damage buildings here at all,
//    and no case asks it to.
...
function hitPoints(bridge: Bridge, id: number): number | undefined {
  for (const slot of ['aoe2.combatStates', 'aoe2.buildingHealthStates']) {
...
function track(bridge: Bridge, what: string, entity: { id: number } | undefined, cell: Position): Tracked {
  ...
  return { what, id: entity.id, ref: bridge.world.getEntityRef(entity.id), hp, cell: { ...cell } };
}
...
function census(unitType: UnitType, impact: BlastCensusImpact): void {
  ...
  // Every unit the blast could reach: all of them but the attacker.
  const units = bridge.getEconomyState().units
    .filter((u) => u.id !== attacker!.id)
    .map((u) => track(bridge, `${u.unitType} #${u.id} of owner ${u.owner}`, u, { x: u.x, y: u.y }));
  const insideCell = blastCensusWitnessCell(unitType, impact, true);
  const inside = units.find((t) => t.cell.x === insideCell.x && t.cell.y === insideCell.y);
  if (!inside) throw new Error(`the census fixture has no witness inside the radius at (${insideCell.x}, ${insideCell.y})`);
  const watched = [...units];
  ...
  if (impact === 'ground') {
    ...
    const target = impact === 'unit'
      ? unitAt(bridge, 2, at)
      : bridge.getEconomyState().buildings.find((b) => b.owner === 2 && b.x === at.x && b.y === at.y);
  ...
  // has in the air and where it will land, and any unit that leaves its cell.
  ...
  for (let step = 0; step < MAX_TICKS && !landed; step += 1) {
    ...
    const cells = new Map(bridge.getEconomyState().units.map((u) => [u.id, u]));
  ...
  // The census judges every unit at the cell it began on, which holds only if
  ...
  // Where it landed: the shot's own aim point. A detonation fires no shot,
  // and is centred on its target's cell (see BOUNDS).
  ...
  const landing: Position = shot ? { x: shot.x, y: shot.y } : at;
  if (shot && impact === 'building') {
    // DE lands a shot at a building on the building itself, and this game on
    // its anchor cell (spec §10.7): the census asks only that it land on the
    // footprint, so either rule passes and a shot that drifts off does not.
    ...
    // A shot at a unit that stood still, or at the ground, lands on the very
    // cell it was aimed at, in this game and in DE.
    expect(landing, `the ${unitType}'s shot ${order} landed somewhere other than the cell it was aimed at`).toEqual(at);
  }
  const reached = (tracked: Tracked) => Math.hypot(tracked.cell.x - landing.x, tracked.cell.y - landing.y) <= radius;
  ...
  expect(reached(inside) && hurt(bridge, inside), `the ${unitType}'s attack ${order} ${where} and did not reach `
    + `the witness at (${insideCell.x}, ${insideCell.y}), which stands inside its ${radius} radius around `
    + `(${at.x}, ${at.y}); if the landing rule changed on purpose, move the witnesses (fixtures/blastCensusLayout.ts)`).toBe(true);
...
describe('every unit with a blast radius blasts wherever its attack lands', () => {
  ...
  for (const unitType of unitTypesWithBlast()) {
    for (const impact of ['unit', 'building', 'ground'] as const) {
      it(`a ${unitType}'s attack at ${impact === 'ground' ? 'the ground' : `a ${impact}`}`, () => {
