...
describe('queueResearchValidator', () => {
  it('rejects non-integer buildingId', () => {
    ...
    const result = validator(
      { buildingId: 1.5, technologyType: 'feudal-age' },
      freshWorld(),
    );
  ...
  it('rejects when building is dead', () => {
    const validator = makeValidator();
    const result = validator(
      { buildingId: 9999, technologyType: 'feudal-age' },
      freshWorld(),
    );
  ...
  it('rejects when alive entity is not a building', () => {
    ...
    const result = validator(
      { buildingId: tileId, technologyType: 'feudal-age' },
      world,
    );
  ...
  it('rejects when building is under construction', () => {
    ...
    const result = validator(
      { buildingId: tcId, technologyType: 'feudal-age' },
      world,
    );
