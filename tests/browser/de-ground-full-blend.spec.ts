...
import { median, sampleRim } from './helpers/deGroundRim';
...
//     cell's own surface (and anything standing there is drawn alike), the frames agree within a few levels; elsewhere they differ over far more pixels (the blend's soft,
//     wandering lines against tile edges), so the blend is what drew.
// Nothing here is timed. What each tier costs is tests/browser/de-ground-frame-cost.spec.ts's.
//
...
// GROUND_TIER=blend RASTERISER=swiftshader).
...
test('draws the full blend on SwiftShader when asked, keeping the fog rule, where the single-sample tier draws its own', async ({ page, browser, baseURL }) => {
  ...
  try {
    const single = await groundFrame(await (async () => {
    ...
    // Measured 2026-09-25 on SwiftShader: 69 centres, 68 identical and one a level apart; 19,908 pixels (2.30%)
