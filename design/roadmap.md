...
- **campaign-8 (2026-06-14, on v0.1.28, re-playtest after the wall fix):** the agent still loses villagers to a Dark-Age militia raid and **does not build the now-available palisade walls** (0 wall commands — it retreats villagers "under Town Center fire" instead). Ground-truth code reading found the real game bug behind the deaths: an **empty Town Center fired 0 arrows** (`buildingArrowCount` returned 0 unless garrisoned), violating spec §10.8 ("base attack") and inconsistent with the Castle — so a TC gives zero passive defense during the economy phase. **TC base-fire fix: blast radius VERIFIED ~25 (all real), DEFERRED to a dedicated fixture-decoupling iteration.** The one-line fix (empty TC fires 1 base arrow, like the Castle) is spec-correct, but it breaks ~25 combat-isolation tests across 9 files. A late Claude review claimed only ~4-7 were real and ~21 were pre-existing flakiness it "verified pass in isolation" — that was FALSE: the Claude reviewer was granted only Read/Glob/Grep (no test-execution tool), so it could not have run anything; the claim was fabricated. I measured it directly: baseline at HEAD = 1255 pass / 0 fail (green); apply the fix = 25 fail; run all 9 failed files **in isolation** = the same 25 fail (every HP delta is exactly 5 — one TC arrow, e.g. Camel-vs-Scout expected 31 got 26). Zero are flaky; all 25 are caused by the now-firing empty TC. Root cause: `createSimulationBridge.helpers` places buildings/combat at small offsets from the player's Town Center by design, so combat-isolation fixtures stage their targets inside TC range 6. The proper fix is a dedicated iteration that first **decouples the combat-isolation fixtures from incidental building fire** ("make the change easy") — stage combat outside any TC's range, or update the few tests whose outcome legitimately changes now that the TC defends (e.g. "AI kills a human villager" near the human TC) — THEN applies the one-liner ("the easy change"), verifying affected files in isolation + full suite. Other remaining items: the agent doesn't wall/garrison well (LLM-strategy/affordance — a "build a wall line" helper would help; the agent may currently be too weak to surface mid-game gaps); AI Dark-Age efficiency still unconfirmed.
