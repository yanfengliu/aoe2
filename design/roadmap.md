...
- **campaign-8 (2026-06-14, on v0.1.28, re-playtest after the wall fix):** the agent still loses villagers to a Dark-Age militia raid and **does not build the now-available palisade walls** (0 wall commands — it retreats villagers "under Town Center fire" instead). Ground-truth code reading found the real game bug behind the deaths: an **empty Town Center fired 0 arrows** (`buildingArrowCount` returned 0 unless garrisoned), violating spec §10.8 ("base attack") and inconsistent with the Castle — so a TC gives zero passive defense during the economy phase. **TC base-fire fix found but DEFERRED (large blast radius):** the one-line fix (empty TC fires 1 base arrow, like the Castle) was implemented and reviewed clean by 3 CLIs on the diff — but the full suite then failed 27 combat-isolation tests across 11 files. Those tests place combatants within the starting TC's range (6) and assume the empty TC never fires, so the now-firing TC perturbs their exact-HP assertions (failures off by multiples of 5, the TC's per-arrow damage). It was reverted to keep the suite green; the correct fix needs a dedicated iteration that first **re-isolates the combat fixtures from TC fire** (spawn combat out of TC range, or give the fixtures no nearby player TC) so the firing TC doesn't contaminate combat measurements — a lesson that this "tiny" change is actually a pervasive behavior change the test suite is coupled against. Other remaining items: the agent doesn't wall/garrison well (LLM-strategy/affordance — a "build a wall line" helper would help; the agent may currently be too weak to surface mid-game gaps); AI Dark-Age efficiency still unconfirmed.
