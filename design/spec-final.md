...
**Redemption ships its siege half only.** The rule also states which BUILDINGS a monk could take — everything except a Town Center, Castle, Wonder, farm, or any wall-line building (wall or gate), which AoE2 puts permanently out of reach because losing one to a single monk would decide a game outright. Nothing targets a building for conversion yet, so that branch is not reachable in play: converting a building means transferring its production queue, population supply, garrison, vision, and occupancy to a new owner, and that is deferred as its own slice. The technology's effect on siege engines is complete.
