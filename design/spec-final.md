...
**Gatherer distribution — food-priority for age-ups (implemented).** The AI spreads its villagers across resources by an age-keyed target weight map: Dark {food 4, wood 3}, Feudal {food 7, wood 4, gold 1}, Castle {food 4, wood 4, gold 3, stone 1}, Imperial {food 4, wood 3, gold 5, stone 1}. Feudal is deliberately food-dominant because the Castle age-up costs 800 food to only 200 gold (4:1). Grounded by replaying an AI-vs-AI match: an earlier Feudal split of {food 5, wood 4, gold 2} left the AI mining gold it barely spent (hoarding ~1500) while its food stayed pinned near zero, so it never gathered the 800 food to advance — the gold sink in Feudal is only the archer (45 gold) plus the Castle's own 200 gold, both of which one gold villager funds comfortably. Weighting food far above gold in Feudal is the supply-side half of reliably reaching Castle Age; the demand-side complement (implemented) is that villager training also respects the age-up reserve — the AI's villager-training gate uses the same `canAffordWithReserve(stockpile, villagerCost, ageUpReserve)` check as its military training, so once the AI qualifies for the next age it stops spending the reserved food on more villagers and the stockpile actually climbs to the age-up cost. The reserve is empty until the AI qualifies (early economy is unchanged) and clears the moment it advances, so the hold is a brief pause at the qualified-but-not-yet-advanced boundary, not a permanent throttle. Without this, a food-banking AI re-spent every surplus villager-cost on another villager and its food stayed pinned near zero through the age (grounded 2026-07-04 by replaying the default-seed corpus).
