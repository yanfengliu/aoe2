...
Each civilization has one or two signature technologies, researched at its **Castle**. Twenty-four are implemented: all nineteen of the original civilizations', which the deferred list emptied in v0.3.70 when Atheism landed on the victory countdowns Spies made meaningful, plus five of the twelve EXPANSION civilizations' (v0.3.170-172). Those twelve — Berbers, Burmese, Ethiopians, Incas, Indians, Italians, Khmer, Magyars, Malians, Portuguese, Slavs, Vietnamese — arrived with 17 unique units and 22 unique technologies, none of them implemented; Shatagni, Recurve Bow, Farimba, Kasbah and Sultans are the first closed — Sultans being the first whose effect is economic rather than martial, which is why it lives in `GATHER_RATE_TECH_FACTORS` with the mining line rather than gaining a field of its own, and `tests/architecture/expansionCivContentGap.test.ts` holds the remainder as a checked ledger rather than prose, and every value in them was verified against the wiki rather than taken from memory (which would have made Shatagni +1 range instead of +2). The table below and `UNIQUE_TECHNOLOGIES` are checked against each other by `tests/architecture/specUniqueTechnologyTable.test.ts` — El Dorado shipped in v0.3.57 and this table did not learn about it for eight versions.
