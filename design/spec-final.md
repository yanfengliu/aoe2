...
**Supported civilization roster (scope decision 2026-07-05).** The game deliberately does NOT implement all 30 civilizations' identities. Civilization bonuses (and, when added, unique units / unique techs) are implemented for a curated roster of FIVE civilizations only: **Britons, Franks, Goths, Aztecs, Mongols.** Every other civilization is selectable (`?civ=`) but plays with DEFAULT stats — no bonuses, no uniques. This caps civ variety at a manageable, well-polished few so effort concentrates on an otherwise complete and polished game rather than spreading thin across 30 civs. Do NOT add bonuses for civilizations outside this roster. (An earlier Slavs "Farmers work 15% faster" bonus, shipped in v0.1.100, was removed in v0.1.101 to honor this cap; Slavs is not in the roster.)
...
- isometric voxel renderer — OPT-IN VERTICAL SLICE (2026-07-11): `?renderer=voxel` renders a real lit Three.js world canvas behind a transparent Phaser input/overlay canvas. The AoE-owned adapter maps projected terrain into 16x16 palette-indexed chunks, supplies deterministic block geometry through a shared geometry resource, and instances original procedural unit/building/resource parts. AoE2 remains the owner of simulation, visual roles, player palettes, fog memory, selection priority, placement, health, commands, animation meaning, HUD, saves, and replays; the sibling `voxel` package exposes only game-neutral voxel-chunk, arbitrary-geometry, and rigid-instance data lanes suitable for City and Townscaper. Empty/absent/invalid renderer values and `?renderer=phaser` use the established Phaser path, which remains the default. The composed voxel slice intentionally flattens terrain and entity elevation because Phaser's overlay/input hit geometry is still ground-plane based; elevation-aware interaction, complete voxel-mode parity coverage, and the promotion gates remain open. Fixed-view before/after/diff evidence, epoch-reset/replay pixel-change checks, context-loss recovery, explicit capture without a preserved drawing buffer, and renderer metrics prove the slice without claiming full migration. Authoritative design and execution ledger: `docs/threads/current/voxel-renderer-migration/DESIGN.md` and `PLAN.md`.
- isometric voxel art direction: the opt-in voxel renderer uses an original procedural Age-of-Empires-style vocabulary rather than generic whole-object blocks. Buildings combine neutral stone/plaster/timber walls, stepped voxel roofs, doors, windows, beams, towers, merlons, banners, awnings, blades, crop rows, and construction scaffolds, with player color restricted to readable accents. Units combine role-specific limbs, clothing, skin, helmets, shields, weapons, mounts, wheels, and banners. Resources use clustered trees, faceted mineral piles, forage berries, distinct animals, and relic ornament; sparse deterministic terrain props add surface detail without changing simulation height or picking. Parts are bounded rigid instances grouped into matte, metal, contact-shadow, and reduced-opacity memory materials. The shared `voxel` package may own configurable target-tracked daylight, but AoE owns every palette, recipe, role, fog cue, and animation meaning. The composed path stays elevation-zero and Phaser keeps overlays/input; true shadow maps, AO, skeletal animation, per-civilization facades, and default-renderer promotion require separate measured gates.
- animated voxel units: live non-memory units in `?renderer=voxel` must animate their existing rigid procedural parts without changing simulation state, saves, replays, hit geometry, selection, fog, health bars, or commands. The first profiles are subtle identity-phased idle breathing/bobbing plus role-readable rigid motion: opposing humanoid limbs, horse legs/tail and rider bob, monk sleeves/staff, and siege mechanisms. Ground contact shadows, fog-memory ghosts, buildings, resources, and terrain stay static. The sibling renderer may own only the game-neutral bounded harmonic transform sampler driven by injected frame time; AoE owns every role, part mapping, amplitude, period, phase relationship, and future gameplay state transition. This first slice is visual motion, not authoritative attack/gather/reload timing. Skeletal animation, clip blending, root motion, projectiles, and event-synchronized attack/gather clips remain deferred.
- speed-matched voxel locomotion: AoE samples the already-interpolated displayed `x/y` path per `id:generation`, advances a wrapped gait phase by displayed distance divided by an AoE-owned role stride, and bakes foot lift/travel, opposing limb pose, cavalry stride, monk sway, and siege wheel rotation into ordinary rigid-part base transforms. The injected clock is simulation display time (`tick + interpolation alpha`), not wall time: it determines displayed speed and a bounded start/stop/turn blend only, never advances phase, and freezes identically during manual or replay pause whether selection forces redraws or not. Direction-aligned pitch and transformed-corner clearance keep feet flexing in the travel plane without penetrating ground. Disappearance, memory projection, generation replacement, bridge replacement, and clock rewind start fresh history. Static and animated surface lanes remain separate so large static scenery does not inherit the active-batch slot ceiling. The shared renderer contract does not gain unit roles, velocity inference, stride policy, foot planting, or mutable gait history; its harmonic lane remains for independent ambient motion.
...
- directional unit animation and gameplay-synchronized attack/gather clips
...
- units behind buildings (occlusion silhouette): a unit that stands BEHIND a building — one that paints over it in the back-to-front (cellX+cellY) isometric draw order — is shown as a WHITE SILHOUETTE of its own role shape (edged in its owner colour, no ground shadow) drawn on a layer above the buildings, so a hidden unit reads as a ghost of where it is rather than vanishing. A unit is occluded when its on-screen centre falls inside a building's drawn silhouette (the footprint diamond extruded by the building's wall height plus any pitched-roof ridge, using the construction stub's reduced height, ignoring last-seen memory ghosts) AND that building sorts in front of it. The cue applies to any unit in the player's vision (own or enemy), tracks units as they move, and is render-only: simulation, hit testing, health bars, selection, fog of war, saves, and replays are unchanged. Shipped v0.1.133.
- building selection by drawn silhouette (M10, v0.1.145): because a building draws as an extruded iso VOLUME, a click on its raised roof or upper walls lands outside the flat ground footprint the pointer inverse-projects to, so the click used to fall through to the empty cell drawn BEHIND the building (clearing the selection). Building selection now tests the click's iso-pixel against the SAME drawn silhouette polygon the occlusion cue uses (`buildingSilhouettePolygon`), and when buildings overlap picks the FRONT-MOST match (largest `cellX+cellY` depth key = painted on top), so selection matches what the player sees. A unit under the click still wins (the existing layer priority — a unit in front of a building is selected before the building). This changes only WHICH existing entity id the click resolves to (selection is render-layer); simulation, saves, and replays are unchanged. The remaining edge case — a click on a roof directly over a unit standing at the ground cell behind it selects that unit — is accepted (the unit is genuinely at that cell).
...
- textured terrain surfaces rather than flat per-kind color fills: terrain cells carry subtle, deterministic per-cell brightness variation (a hue-preserving ±7% jitter; shipped v0.1.30) so same-kind cells do not read as one flat block, AND adjacent cells of DIFFERENT kinds blend at their shared edge instead of meeting at a hard rectangular seam — for each cell edge whose von-Neumann neighbour is a different kind, the renderer stipples a deterministic feather band of small specks on the inner side of the edge, tinted with the per-channel average (blend) of the two kinds' base colors; the neighbour cell feathers back symmetrically so the two bands interlock into a soft, dithered transition (a grass/water shore feathers green into blue, a grass/hill edge green into tan). Shipped v0.1.43. The transition is a pure function of the cell kind, its neighbour kinds, and the cell coordinates (deterministic — no per-frame random or time, so the ground never shimmers), every speck stays strictly inside its own cell square (so unit/building/health-bar/selection geometry is unaffected), and the blend renders under all entity/fog/selection/grid overlays. Interior terrain texture marks shipped v0.1.119: grass/forest/hill diamonds add faint deterministic speckles and water diamonds add short translucent wave strokes, all inside the diamond and render-only. Water shoreline strokes shipped v0.1.124: a water cell bordering land draws a short low-alpha inset line along that diamond edge, over the existing feather/ripple detail, so shorelines read as water meeting land rather than hard blue cutouts; still render-only and deterministic. Elevation-based light/shadow is deferred (the render data does not currently project per-cell elevation; shading by height would require a render-contract change); diagonal (corner) blends and bespoke per-pair transition textures (e.g. a dedicated beach gradient for grass↔water) are also deferred to later M7 slices.
- readable per-type unit silhouettes rather than flat tinted circles: each unit renders as an original procedural shape grouped by RENDER ROLE — villager, infantry (foot melee), archer (foot ranged), cavalry (mounted), cavalry-archer (mounted ranged), siege (machine), monk — so a glance distinguishes the role, while the owner-color tint is kept as the body fill. The shape orients toward the unit's movement FACING (derived render-side from the per-tick projected-position delta; idle units use a consistent resting orientation — no fabricated heading), and every shape stays within the unit's cell-centred bounding circle so the existing health-bar and selection-ring geometry is unchanged. Shipped v0.1.41 (the prior look was one tinted circle per unit). Each unit also draws a subtle translucent ground shadow beneath its body — a flattened dark ellipse (`UNIT_SHADOW_COLOR`/`UNIT_SHADOW_ALPHA`, `unitShadowEllipse`) nudged just below the unit centre and kept within the bounding circle so the health-bar and selection-ring geometry is unchanged, drawn FIRST so the body silhouette sits on top — so a unit reads as standing ON the terrain rather than as a flat shape painted over it; shipped v0.1.102 (buildings are excluded — they fill their footprint, so a shadow would either be hidden under the body or violate the in-footprint primitive contract). Foot humanoids (villager, infantry, archer) draw planted leg strokes and small visible foot marks below the torso so they read as upright people at default zoom; shipped v0.1.118. A per-unit (vs per-role) silhouette, attack lunge, and richer weapon animation are deferred to later M7 slices (the unit death effect shipped v0.1.129 — see below).
- HUD icons authored as ORIGINAL procedural inline SVG (no copyrighted icons, sprites, image files, or icon fonts; every glyph is self-contained hand-drawn `<path>`/`<rect>`/`<circle>` markup stroked with `currentColor` so the existing gold-on-dark-teal palette controls the tone). Shipped coverage: top-bar resource chips and Build buttons in v0.1.39; selected unit/building badges in v0.1.44; Train/Research/Market/Action command-card buttons in v0.1.72-v0.1.76; and selected resource/wildlife/relic badges in v0.1.77-v0.1.78. Icons AUGMENT the labels rather than replacing them: each glyph is a sibling of the element that still carries the meaning (resource value, command text, or selection badge/label), so the existing selection/data hooks and screen-reader text are preserved; decorative glyphs are `aria-hidden`. Remaining UI-icon polish: per-species/per-tech art variants and a clearer market buy/sell arrow distinction.
...
- dynamic, time-based feedback for selection and combat (the first animated render layer; purely visual — it never affects the simulation, saved games, or replay determinism): (1) a SELECTION PULSE — the selection ring around a selected unit or building breathes over a ~1.1-second cycle, modulating its stroke brightness, width, and a small outward radius swell, while the ring's center and base radius (and thus the entity's hitbox/health-bar/footprint geometry) stay fixed; derived from the existing `selected`/selection-set state. (2) a HIT FLASH — when a unit's HP drops between frames, a brief (~0.26 s) bright impact flash (a white-hot core fading to a warm orange rim) is drawn over it, confined to the unit's bounding circle and decaying on its own; derived render-side from a per-frame HP delta tracked the same way unit facing is (no new projected/recorded data). Shipped v0.1.45. Both are deterministic-sim-safe: the pulse phase and flash decay read the render clock only, and no randomness enters the simulation. (3) a UNIT DEATH TELL — when a unit dies, a brief (~0.7 s) collapse animation plays at the cell where it stood: the body tips (its tip direction fixed by entity-id parity, so a battle line's corpses fall varied ways) and wilts to the ground while fading out, under two expanding, fading dust rings. Because a unit vanishing from the render projection is ambiguous (death, fog exit, and garrisoning all remove it), the death cue is driven by an explicit DEATH FEED rather than by present→absent diffing: the simulation records each unit death at its `destroyUnitEntity` chokepoint (id, tick, sub-cell position, owner, unit type, tint, size, and the set of players who could see the cell AT THE MOMENT OF DEATH) into a transient, NEVER-persisted feed bounded to the last ~10 ticks, and each frame surfaces only the deaths the viewing player WITNESSED — i.e. had vision of when they happened, captured before the unit is destroyed and vision recomputed. Gating on at-death visibility (not current visibility) is what makes the cue fog-correct in both directions: a kill that happened in your fog never leaks when you later uncover the cell, and your own lone unit's death still shows even though losing it re-fogs its cell the same tick. A garrisoned unit dying inside a building records nothing (it has no position; the building's own destruction is the visible event). The feed carries no simulation authority: it is absent from saves and from every deterministic hash, so a load simply drops any in-flight death animations and a replay re-emits deaths naturally because `destroyUnitEntity` re-runs. Shipped v0.1.129. Deferred to later M7 slices: gather sparks on actively-gathering villagers (the villager gather state is not on the render-projection today; adding it is a render-contract change), building-destruction rubble, and projectiles in flight (paired with the M2 projectile simulation).
...
- **Model policy: all playtest LLM calls use ONE model** — tactical, strategy, the conformance probe, the auto-fix `submit_fix_diff` call, and the deterministic-loop propose-fix CLI. Do not split tiers across models. **Current model: `claude-fable-5` (2026-07-10 owner directive: everything uses the most advanced model available — the 2026-06-12 Fable ban is lifted; both `claude-fable-5` and `claude-fable-5[1m]` smoke-verified that day). Fall back to the latest Opus (`claude-opus-4-8`) only while Fable is unavailable, and swap back when it returns.** Per-call cost on the claude-code provider is prelude-dominated, so the Fable/Opus rate gap is immaterial (~$0.61 vs ~$0.67/decision observed on campaign-3). When swapping wholesale (unavailability, flagship bump), smoke-test the new id via `echo ok | claude -p --model <id>` first (per the AGENTS.md model-currency rule) and update all call sites (`scripts/playtest-llm.mjs` ×2 plus its cost-note strings, `scripts/playtest-findings.mjs` ×1, `scripts/playtest-llm-auto-fix.mjs`, `scripts/propose-fix.mjs`) plus the `DEFAULT_LLM_COST_TABLE` comment in `src/game/playtest/types.ts`.
