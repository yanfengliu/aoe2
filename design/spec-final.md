...
**Slavs — Farmers work 15% faster (implemented v0.1.100).** A Slavs player's villagers gather from FARMS 15% faster. Same seam as the Britons shepherd / Mongols hunter bonuses: `civGatherRateMultiplier` returns 1.15 only for `civilization === 'Slavs'` and `resourceKind === 'farm'`, multiplied with the tech gather-rate at the villager gather-tick site. Farm-specific — foraging berries, herding sheep, hunting boar, and chopping wood are unaffected, matching the AoE2 "Farmers" wording. Active from the Dark Age with no prerequisite; non-Slavs owners are byte-identical.
