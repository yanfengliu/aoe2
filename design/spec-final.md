...
- External sources resolve ambiguity and identify gaps between the local dataset and live DE. They override a CSV row only where that row disagrees with Definitive Edition and the evidence is in hand (§2.1: DE is the authoritative edition). Each override is recorded where its column is checked, as a named exception in that column's `tests/content/` differential citing the source and its pinned revision, and never as a silent difference between the game and the CSV.
...
**Behavioural parity is the standard (owner directive, 2026-08-23).** The game must FUNCTION IDENTICALLY to the real Age of Empires II — the same costs, gather and creation rates, counters and bonus damage, tech-tree gating, controls, and consequences. Where this repo has no assertion about a value, the real game's value is still the correct one, and a divergence is a defect rather than an open choice. Rules resolve in the §1 source-of-truth order, and **Definitive Edition is the authoritative edition**: the owner's "HD" is shorthand for the real game, `design/stats/` is meant to hold DE's values, and where an HD-era rule differs from DE, DE wins and the choice is recorded where it was made. (On 2026-09-23 the unit and technology costs and times in `design/stats/` were found to differ from current DE on 109 of 230 rows, most of them Conquerors-era values DE has since changed; see "Costs and times gated" in §10.9.) A remembered rule is a hypothesis until an authoritative reference confirms it.
