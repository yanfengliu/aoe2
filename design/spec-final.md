...
- resource visual variation (user, 2026-07-14; shipped v0.2.10): gold mines, stone mines, and trees draw deterministic per-instance variants so adjacent same-type resources no longer read as copies. Trees vary trunk height, whole-tree lean, and — critically — the CANOPY: every crown block's offset, size, and yaw deviates per instance (the pre-v0.2.10 recipe jittered only trunk height, leaving every tree's crown byte-identical, which is what made a forest read as stamped). Mines vary each rock's offset, size, and yaw plus glint/vein placement (previously only per-rock yaw varied). Every variant is seeded by the resource's MAP POSITION — immobile resources never move, so the look is identical every frame, across save/load, and in replay, with no per-frame or per-load randomness. Deviations are bounded so each part stays inside the resource's cell, the part SET is unchanged (the part-derived hit silhouette, selection, and fog memory are untouched), and total height stays within a readable band. Render-only: amounts and gathering are unaffected.
