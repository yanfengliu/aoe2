...
- **No provider is NOT RUN, never a pass (2026-09-24).** `npm run playtest:corpus-llm` with neither `ANTHROPIC_API_KEY` nor a working `claude` plays nothing, prints `NOT RUN` naming what is missing, and exits 2. The nightly `playtest-llm.yml` therefore reads red on a night it cannot play, and its artifact upload fails when there is nothing to upload. It used to exit 0, and the nightly read green on 89 of its 138 scheduled nights since 2026-05-10, the last 19 in a row (2026-09-06 to 09-24), with no artifact. Whether the nightly gets a provider secret or is retired is the owner's decision.
