...
**AI Lumber Camp siting and Dark-Age camp order (v0.3.177).** The AI sites its Lumber Camp on the WORK, not on the tree nearest its Town Center: the anchor is the medoid of the trees its wood-gathering villagers currently target (the targeted tree minimising summed Manhattan distance to the other targeted trees — a medoid rather than a centroid because the answer must be a real node on the woodline being cut, not an empty cell between two woodlines), and the camp is placed at the nearest buildable cell to that anchor. The work anchor is bounded to within 12 cells (Manhattan) of the Town Center; beyond that it is discarded and the camp falls back to the previous rule, the harvestable tree nearest the Town Center within the same radius — a camp further out costs more in the walk to build it and in the exposure of everyone hauling to it than the shorter carry saves. The fallback also applies when no villager is yet targeting wood. Mining Camps keep the nearest-node rule unchanged. Before this, camps were planted a median FOURTEEN cells from the woodline actually being cut (measured over ten seeds); the medoid puts them at 4.5. In the ordinary Dark Age opening (Town Center standing), the AI requests the Mining Camp BEFORE the Lumber Camp; the reverse order let a Lumber Camp placed on the woodline evict the Mining Camp onto the Town Center's shoulder and deadlock every villager (measured: all ten villagers on one cell from tick 9,000 to 20,000, never leaving the Dark Age). The Nomad opening is unchanged — it opens Lumber Camp first, sited by the previous nearest-tree rule, because a wood dropsite is the only road to the Town Center's 275 wood (see Nomad above).
