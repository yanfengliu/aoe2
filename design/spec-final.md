...
- External sources resolve ambiguity and identify gaps between the local dataset and live DE. They override a CSV row only where that row disagrees with Definitive Edition and the evidence is in hand (§2.1: DE is the authoritative edition). The override is recorded with its source and pinned revision: by re-sourcing the row and naming the source in the CSV's provenance header, or, while the row itself is not yet re-sourced, as a named exception in that column's `tests/content/` differential. It is never a silent difference between the game and the CSV.
...
**Behavioural parity is the standard (owner directive, 2026-08-23).** The game must FUNCTION IDENTICALLY to the real Age of Empires II — the same costs, gather and creation rates, counters and bonus damage, tech-tree gating, controls, and consequences. Where this repo has no assertion about a value, the real game's value is still the correct one, and a divergence is a defect rather than an open choice. Rules resolve in the §1 source-of-truth order, and **Definitive Edition is the authoritative edition**: the owner's "HD" is shorthand for the real game, `design/stats/` holds DE's values, and where an HD-era rule differs from DE, DE wins and the choice is recorded where it was made. The unit and technology costs and times in `units.csv` and `technologies.csv` were re-sourced from the DE game data on 2026-09-23, pinned in each file's header; until then they were mostly Conquerors-era values (109 of 230 rows differed from current DE; see "Costs and times gated" in §10.9). A remembered rule is a hypothesis until an authoritative reference confirms it.

Exactly one deliberate divergence exists, about LOOK rather than behaviour, and owner-set: all art, the HUD's included, is original/procedural rather than reproduced Age of Empires assets (§14.5). It does not license a behavioural difference. Until 2026-09-25 there was a second: the HUD chrome was modern rather than AoE2-era. That day the owner put the HUD inside the Definitive Edition target (*"Yes include HUD"*; §14.1, §14.5). Moebius, which the owner kept selectable for the game world beside the DE look (§14.5), is an option rather than a divergence from the target, even while it is the default.
...
**Owner decision, 2026-09-23:** *"No multiplayer, no campaigns."* So "the whole game" that §16.2 sets as the goal means the whole single-player game played against the computer. Multiplayer and campaigns are excluded by the owner, not merely unrequested. The decision says nothing about the other items below, and they stand as they were.

- multiplayer — excluded by the owner, 2026-09-23
- ranked or quick-play flows
- replay system requirements
- campaigns (the campaign story mode) — excluded by the owner, 2026-09-23
- scenario scripting
...
**Every setup control is reachable at any viewport height (v0.3.215).** The setup screen is the only route from a bare URL into a match, so a control it cannot show is a wall rather than a degraded experience. The requirement is behavioural, not a height budget: the heading is fully visible AT REST before anything is scrolled, every control including Start can be brought fully inside the viewport by ordinary WHEELING, each one is the topmost element at its own centre point, and Start pressed at those real coordinates boots the match it configures. The overlay is therefore its own scroll container and the panel is centred by AUTOMATIC MARGINS — centred while the viewport has room, top-aligned and fully scrollable when it does not. `align-items: center` cannot be used for this and the reason is structural: a flex item taller than its line overflows BOTH ends of it, while scrollable overflow only ever extends past the END edge, so the top stays unreachable at every scroll offset. Below about 600px of height the panel used to clip its heading off the top and its Start button off the bottom with nothing scrolling at all, which meant the game could not be started (defect register, 2026-09-06; the browser suite's own 800x600 default cleared the old panel by one pixel, which is why it survived twenty versions).
...
Gather-target assignment: a villager with a gather order is routed to a matching, gatherable resource — preferring the player's own resources first, then neutral resources on the player's home base, then others — and villagers piled onto a single over-subscribed resource fan out across nearby alternatives instead of jamming. Within a preference tier, candidates are ranked by their HAUL — the true walking distance from the cell beside the node to the cell beside the player's nearest standing drop-off for that resource, measured the way units actually move (4-connected, around every obstacle), which is the steady-state round-trip cost a shuttling villager pays repeatedly — with the villager's own distance as a tiebreak; when the player has no standing drop-off for the resource the ranking falls back to plain villager distance. The haul is a WALK and never a straight-line or Manhattan estimate (corrected 2026-09-05): on open ground the two agree, but a forest is a dense block of impassable cells, and ranking by Manhattan distance sent villagers to trees a few cells away as the crow flies whose real walk was thirty-five to forty-five cells while trees a walk of zero to three away sat unused — a near-optimal assignment was modelled at 2.61x the AI's measured wood throughput before the fix, and with the fix the share of wood targets the ranking gets wrong by its own rules falls from as much as seven in ten to a few in a hundred (defect register, 2026-09-01). A node no drop-off can reach has an infinite haul and ranks after every reachable one. The haul is read from one distance field per player, resource kind and movement domain (a breadth-first search from every drop-off of the kind), recomputed only when the map's durable topology or the set of drop-offs changes, so ranking every candidate costs no pathfinding. This round-trip locality keeps auto-assigned villagers cycling on base-proximate resources instead of spiraling outward: a villager that goes idle deep in a far forest re-targets a tree near its lumber camp (one walk back) rather than the tree nearest its current position (a huge round-trip every load, which throttles wood income to a trickle while full trees near the base sit unused). The return leg walks the same field (v0.3.205): a loaded villager descends the field from the cell it stands on — the true shortest route to the walk-nearest cell beside the walk-nearest drop-off — and every carrier on a cell takes the same route, so two groups never walk through each other; the straight-line search survives only as the fallback for a carrier standing off the field (boxed in, or with no drop-off at all). Before this the return leg aimed at the drop-off nearest by straight-line distance and at the first cell beside it a path search happened to reach, the same measurement error one level down: on Nomad it sent fourteen carriers on mirror-image loops through each other for the second half of a match (defect register, 2026-09-05).

Home gather range (implemented v0.3.30): auto-assignment is bounded to resources within a home range of the player's drop-offs for that resource — the same walking haul the ranking uses, to the nearest drop-off (a walk since 2026-09-05; Manhattan distance to the drop-off nearest the villager before that) — so a node no drop-off can reach is never "home", whatever its straight-line distance. Ranking alone only ORDERS candidates, so once the nodes beside a base are crowded the fan-out otherwise runs on to the far side of the map — a villager assigned work forty to sixty cells away walks through an opponent's base and is killed there, which drains a player's economy far faster than the extra income replaces it. The range applies only when at least one candidate of the desired kind falls inside it; a player whose own resources of that kind are exhausted still ranges as far as needed rather than stopping. Unreachable-target recovery (below) probes inside the home range first and widens to the full map only when nothing at home can be reached, so a villager standing beside a reachable tree is never starved by a walled pocket next to its drop-off. This bound governs AUTOMATIC assignment only — an explicit player gather order is honoured at any distance.

Enemy static defences (implemented v0.3.191): automatic assignment never sends a villager to a resource node, drop-off, or hunt target inside an ENEMY defensive building's attack range — every building that shoots, which today is the Town Center, Watch Tower, Bombard Tower and Castle, so a new one is covered the day it gets an attack range. Reach is measured from the building's FOOTPRINT with its actual effective range (arrow technologies, Keep, the unique technologies that reach defensive buildings, and the Korean tower bonus included) plus one cell of margin, because a villager works from the cell beside its node, and it is computed by the same function the arrows fire with, so the RANGE the rule keeps clear of and the range the arrows have cannot drift apart. The two are not the same predicate: the arrows also require line of sight and, until Murder Holes, refuse a target inside the tower's minimum range, and the rule ignores both — so it refuses a slightly larger set than the building actually covers, deliberately and in the safe direction. "Enemy" means a building owned by a player the villager's owner is at war with — an ally's Castle is a building like any other — and a foundation under construction does not count until it is complete. Excluded nodes come back only as the last resort — when no other reachable node of that kind exists anywhere — and the resource-kind fallback (below) runs BEFORE that exception at every site that may change a villager's resource kind — the two idle passes (including the re-assignment a villager gets the instant it has delivered its load), the unreachable-target reroute and the give-up reroute: a villager whose own gold is gone chops wood safely rather than mining under the enemy's Town Center, and accepts a dangerous node only when no kind has a safe one. The one site that must not change the kind — redistributing a villager off an over-subscribed node of the kind it is already on — instead refuses a dangerous node outright and leaves the villager where it is, so a crowded safe node always beats an empty one under enemy arrows. A kind whose safe nodes cannot be reached counts as having none — within the bounded reachability probe, which pathfinds at most sixteen candidates per list, so "cannot be reached" means "no path was proved to the sixteen nearest", not "no path exists". A drop-off inside an enemy defence's reach is passed over for the nearest one outside it, and used only when no other drop-off for that resource stands clear. The drop-off preference applies to EVERY carry, including one made under an explicit player gather order, because the drop-off is chosen by the game and never by the player — the consequence a player sees is a longer round trip from a node beside a camp an enemy tower has just covered. The same choice is the anchor the home gather range is measured from, so a camp that comes under enemy arrows also stops being the centre of that window. The AI's hunting party takes the nearest boar that stands clear of enemy defences, and one inside their reach only when every boar in range is. A villager already committed to a node that has come under enemy arrows — a tower or Castle completed beside it — is re-assigned to a safe node of the same kind on its next decision, keeping any partial load; when the dangerous node is the only one of its kind it keeps working rather than oscillating. The rule governs AUTOMATIC assignment only, for the AI and for the human player's idle-villager auto-gather alike; an explicit player gather order on a dangerous node is obeyed, because the player chose it. The RE-ASSIGNMENT half is narrower than that and knowingly so: it refuses to touch a villager carrying an explicit gather order, and the human slot's auto-gather is itself carried by that same flag, so today a human player's villager is never moved off a node a new tower has just covered — only an AI's is. Separating "the player right-clicked this node" from "the game assigned it and set the flag so it would keep working" needs a second field on the gatherer and is not done. The rule bounds where a villager is SENT, not the route it walks: a path to a safe node may still cross an enemy defence's reach, and nothing here stops it. Measured on the boot map at 45,000 ticks, the same tree with and without the rule: without it owner 2 loses fifty villagers, forty-three of them inside owner 1's reach, in one run of deliveries to gold mines three cells from owner 1's Town Center; with it, seven lost and none under enemy defences. Ages, building variety, unit variety and peak army are identical either way — but those columns are sampled every 250 ticks and are demonstrably insensitive at this scale (two builds whose villager losses differed by nine produced byte-identical columns), so they show that no cost is VISIBLE in them, not that the rule is free.
...
- a farm is walkable ground: every land unit of every player crosses it and takes a real sub-tile slot on it, and nothing may be built on it (implemented 2026-09-10; see the claim-kind paragraph below)
...
**Farm slice 1 (implemented).** A Farm is a building+resource HYBRID. A villager BUILDS it through the normal placement + construction flow as a 1×1 building (cost 60 wood, available in the Dark Age with no prerequisite, 480 HP, build time 15 seconds → 150 ticks). On construction-complete the farm gains a `resource` component (`resourceType: 'farm'`, 175 food — structures.csv "Standard = 175 Food") so the existing villager economy gathers food from it: a food-role villager is routed to the farm, stands on an adjacent cell (1×1 approach footprint, exactly like a berry bush), and harvests food at the sheep/farm cadence of §6.3 (~0.33 food/s; before the v0.3.159 retune this read "berry-bush cadence", when both families shared one hot prototype rate). NOTE (2026-08-27, retune landed v0.3.159): the gather cadence table now meets §6.3 — every family lands within ±5% of the table (berries 0.3125, sheep/farm 0.333, boar 0.408, deep fish 0.5, wood 0.385, gold 0.385, stone 0.357 per second), pinned by `tests/simulation/economyGatherPacing.test.ts`. The original prototype cadences had run 4.5–10x hotter than §6.3 while build/research times matched AoE2 1:1. (The movement-speed half of the pacing landed v0.3.160 — see §12.4.2: the base clock now puts a villager at exactly 0.8 tiles/s, so effective gather rates carry honest walk overhead.) The hybrid claims its cell as WALKABLE GROUND rather than as a building blocker (corrected 2026-09-10, v0.3.221), and selects as a building. Occupancy dispatches on the building component first, so the resource component still adds gather semantics without a second occupancy claim; what the building component records is a CLAIM KIND (`src/game/simulation/passableStructures.ts`), and a farm's is `'farm'` where every other structure's is `'building'`. A `'farm'` claim is held on this side of the engine only: placement counts it, so nothing may be built on a farm, while movement, spawn and wildlife do not, so every land unit of every player walks and stands on one — DE's rule, where a farm is ground with crops on it and only the Mill or Town Centre beside it blocks. The claim kind rather than the passability predicate is what carries this, because the engine's sub-cell grid refuses a unit slot on any cell an entity occupies: a predicate-only fix would have routed units onto farms and then stacked them there as overflow. Gated by `tests/simulation/farmsAreWalkable.test.ts`. When the stored food reaches 0 the farm either auto-reseeds (if its owner can afford the wood — see "Farm reseed" below) or, when the owner cannot pay, is removed from the map entirely — both the resource and the building shell vanish (an un-reseeded, unaffordable farm disappears in AoE2; there is no leftover shell and no refund).
...
Deferred follow-ups (NOT yet implemented): the Mill batch-reseed queue and a per-farm manual reseed toggle (the auto-reseed-if-affordable rule above IS implemented), requiring a Mill anchor near the farm, and a dedicated farm sprite with tiled crop rendering. The AI DOES build farms (corrected 2026-09-10 — this line claimed it "does not place farms yet" long after it did): `targetFarmCount` is 0 in the Dark Age and `max(2, min(8, floor(villagers / 3)))` from the Feudal Age on, so a developed AI base stands on up to eight of them, which is why they were the most numerous wall segment on a map before farms became walkable ground. The farm-upgrade techs (Horse Collar / Heavy Plow / Crop Rotation → 250 / 375 / 550 food) are now implemented (see §6.5 "Farm-food techs").
...
**Tribute (implemented v0.3.67).** A player with a **completed Market** — AoE2's own precondition — can send any of the four resources to any other player in the match, ally or not. The sender pays the amount plus a fee: **30%** base, **20%** once **Coinage** (Feudal, 200 food 100 gold) is researched, **0%** once **Banking** (Castle, 300 food 200 gold, requires Coinage) is — `technologies.csv`'s "Tribute inefficiency" rows, derived from the sender's researched set at execution time. The fee rounds up, so it can never be underpaid; the recipient always receives exactly the amount sent. The transfer is atomic and instantaneous, rides the recorded `tribute.send` command for replay fidelity, and moves nothing on any failed check (no Market, unknown recipient, self-tribute, non-positive or fractional amount, or a stockpile short of amount plus fee).
...
Qualifying building logic must follow AoE2 conventions, not count every structure equally. Advancing Dark -> Feudal requires two of {Barracks, Dock, Lumber Camp, Mill, Mining Camp} — Houses, Farms, and Walls do NOT count, and the Town Center never counts. All five are in `DARK_AGE_PREREQUISITE_BUILDINGS`; the Dock was missing while this section still said "this land-only slice has no Dock", which stopped being true when the naval slice shipped one that §7.3 lists as a Dark Age unlock. The Feudal -> Castle set is {Archery Range, Blacksmith, Market, Stable} and the Castle -> Imperial set is {Castle, Monastery, Siege Workshop, University} — the PRODUCTION and research buildings introduced in the prior age, not every building unlocked in it: walls, gates and towers are Feudal and Castle unlocks and none of them qualifies, which a looser phrasing here ("the non-Town-Center buildings introduced in the prior age") wrongly implied. Castle -> Imperial additionally accepts a single Castle in place of the pair (`canAdvanceToImperialAge`) — a Castle costs more than two of anything else in its tier, which is why AoE2 lets one stand alone.
...
**A building under construction has no health of its own — its health IS its build progress (v0.3.220).** A foundation stands up on 10% of the building's full hit points (never zero, since a building on 0 HP is a destroyed one) and rises linearly with `buildProgressTicks`, reaching exactly full on the tick the build completes. So a full health bar means a finished building and can mean nothing else, and a bar read half full is a site that is half built, whatever the crew size. Health is therefore never a second accumulator advancing on its own clock: build work moves progress, and progress is what health is read from, with the earned value as a ceiling that health can never run past and the same work converted to hit points as the step, so a foundation an enemy chipped mends at the build rate rather than snapping back onto the curve the moment a villager touches it. The 10% floor is defined in one place and read from there by every writer. This matters beyond the bar: the age advance counts COMPLETED buildings, so a player reading full bars on unfinished sites is told they own none of the buildings they are looking at.
...
Implemented model (buildings): right-clicking a friendly, COMPLETE, damaged building with a villager repairs it. The villager walks adjacent (like construction) and restores HP over time at the building's build rate (`maxHp / buildTimeTicks` per tick) until full. The cost is a fraction of the build cost proportional to the missing HP — `ceil(0.5 × buildCost × missingHp / maxHp)` per resource — so a full repair from ~0 costs about half the build cost. From v0.3.122 the cost is charged CONTINUOUSLY as the hit points return, AoE2's own model: starting a repair is free, fractions accrue per repaired target in integer hundredths (the `aoe2.repairAccrual` codec, so a save mid-repair carries the sub-unit remainder exactly), whole resource units are charged the tick they fall due BEFORE that tick's HP applies, an interrupted repair has paid only for what was mended, and an owner who cannot cover a due unit STALLS — the villager stands at the wall mending nothing for free until the stockpile recovers. A finished repair drops its sub-unit carry. The same model charges mechanical-unit repair (§v0.3.107). Mechanical-unit repair shipped v0.3.107: a villager right-clicking a friendly DAMAGED siege engine or ship (anything carrying the siege or ship armor class — the taxonomy is the source) repairs it under exactly the building model — half the unit's TRAINING cost pro-rata the missing HP charged up front, pursuit to melee range (the target may move; the villager follows), then restoration at the unit's own training rate (`maxHp / trainingTimeTicks`). A full-HP or organic target falls through to the ordinary move — Monks heal flesh, villagers mend machines. The command reuses the 'repair' word with a unit target ref, so saves and replays carry it unchanged. Continuous charging shipped v0.3.122 (see §8.1 — the same accrual codec charges units and buildings). The old "repair-speed modifiers" deferral is RETIRED as not-a-feature: AoE2 DE has no repair-speed technology (Treadmill Crane accelerates construction only, which `buildRateMultiplier` already models), and multiple repairers already stack naturally — each assigned villager runs its own repair tick and pays its own accrual, which is DE's behaviour.
...
Each civilization has one or two signature technologies, researched at its **Castle**. Twenty-five are implemented: all nineteen of the original civilizations', which the deferred list emptied in v0.3.70 when Atheism landed on the victory countdowns Spies made meaningful, plus six of the twelve EXPANSION civilizations' (v0.3.170-173). Those twelve — Berbers, Burmese, Ethiopians, Incas, Indians, Italians, Khmer, Magyars, Malians, Portuguese, Slavs, Vietnamese — arrived with 17 unique units and 22 unique technologies, none of them implemented; Shatagni, Recurve Bow, Farimba, Kasbah, Sultans and Carrack are the first closed — Sultans being the first whose effect is economic rather than martial, which is why it lives in `GATHER_RATE_TECH_FACTORS` with the mining line rather than gaining a field of its own, and `tests/architecture/expansionCivContentGap.test.ts` holds the remainder as a checked ledger rather than prose, and every value in them was verified against the wiki rather than taken from memory (which would have made Shatagni +1 range instead of +2). The table below and `UNIQUE_TECHNOLOGIES` are checked against each other by `tests/architecture/specUniqueTechnologyTable.test.ts` — El Dorado shipped in v0.3.57 and this table did not learn about it for eight versions.
...
| Berbers | Kasbah | Castle | Castles work 25% faster (AoE2 gives this to the whole team; here, to the researching player) |
| Indians | Sultans | Castle | Villagers gather gold 10% faster (stacks with the Gold Mining line, as AoE2's gold technologies do) |
| Portuguese | Carrack | Castle | All ships gain +1 melee and +1 pierce armour |
...
**AI age progression (v0.3.169).** The AI reserves the wood its next age-up PREREQUISITE building costs before it trains military — but only once it already holds three units, so the reserve can never become a training ban — and its Feudal villager split puts 6 parts in 15 on wood rather than 4 in 13. The two are one rule and must move together: a spearman costs 25 wood, so military and construction bid for the same resource, and with nothing arbitrating the AI could never accumulate the 150 for a first Blacksmith — it never qualified for the Castle Age, and because the age-up reserve is empty until an owner qualifies, it also banked food and gold it had no way to spend. Measured before the change, over three seeds and 40 minutes of game time: no player reached the Castle Age, and a match exercised four unit types and eight building types. After: the boot map becomes ELIGIBLE for the Castle Age at tick 23,500, having never qualified at all before, with nine building types against eight and five unit types against four; `default-seed` reaches the Castle Age itself at 24,000 with twelve building types against seven. Across three seeds this costs no army — total peak military is 37 either way — which is what the three-unit floor is for: an earlier version without it blocked all military training on wood-poor seeds and cost 21% of peak military across eleven seeds while still failing to afford the building it was saving for. The reserve is scoped to Feudal, the transition it was measured on, and lapses the moment an owner holds the two qualifying buildings.

**Team bonuses (v0.3.81).** The `team_bonus` column is live for every line the seams can express — active for an owner when the owner **or any ally** plays the civilization (a side of one still enjoys its own, AoE2's rule). Economy and production: Aztec relics pay a third more (one extra gold per relic every third tick — deterministic), Spanish trade returns +25% gold at the route's load-time pricing (sourced v0.3.144), Chinese-team farms hold +10% food (reseeds included; sourced v0.3.144), Slavic-team military buildings each shelter +5, and Britons speed their Archery Ranges 10%, Goths/Huns their Barracks/Stables 20%, Celts their Siege Workshops 20%, Turks gunpowder units 25% anywhere, Magyars mounted archers 25%, Portuguese every technology 25%, and Malians University research 80% (all sourced v0.3.144). Incas-team farms build 50% faster (sourced from `civilizations.csv` rather than live DE, which raised it to 100% and later replaced the bonus with a Spearman/Skirmisher sight line — `design/stats/` is the authority here and the divergence is recorded so it is not read as a typo; through `buildRateMultiplier`, which now takes what is under construction so a bonus scoped to one building can be expressed there at all). Costs: Mayan-team walls at half stone, Viking-team Docks 15% off (sourced v0.3.144) — through the same `ownerConstructionCost` seam every charge and display reads. Support: Byzantine-team monks heal 100% faster (sourced v0.3.144), Teuton-team units resist conversion at half rate (stacking with Faith to a quarter, AoE2's composition), and a Portuguese ally speeds every technology the side researches by 25% (current DE's team bonus — it replaced free Cartography when DE retired that technology; shared sight is simply the default for allies since v0.3.140). The combat-stat lines followed in v0.3.82: Frankish-team knights, Mongol-team scouts see farther (+2; the Magyar foot-archer sight line is DE-dead — their team bonus is now the mounted-archer train speed), Japanese-team galleys +4 flat, and Korean-TEAM villagers +3 (DE lists it as their team bonus); Khmer-team scorpions reach one cell farther at the combat-state factory (the Korean mangonel line's team range is DE-dead); Saracen-team foot archers (+2) and Hindustani-team camels AND scout line (+2) punish buildings, and Persian-team knights strike archer-class targets for +2 — the last three computed by the command loop and threaded into the delivery paths. **Civ attack-speed bonuses (sourced v0.3.145)** ride a per-civilization reload hook at the combat-state factory, re-derived by ratio on age-up: Japanese infantry +33% from Feudal, Ethiopian foot archers +18%, Mongol cavalry archers +25%, Celt siege +25%, Saracen galleys +25%, Byzantine fire ships +25%, Spanish gunpowder +18%, Hindustani camels +20%. The Britons' +1/+2 foot-archer range in Castle/Imperial rides the same factory + age sweep, and Incas' military units cost -15/20/25/30% FOOD by age through a food-component cost rule. Fixing the live Fletching case to a DELTA (it re-derived range from the base table, erasing carried bonuses) was the enabling repair. **Civ research-cost bonuses (sourced v0.3.147)** price through one `effectiveResearchCost(civ, age, tech)` at the charge, the validator, the rejection detail, the agent card, and every AI budget site (including the age-up reserve): Chinese -5/10/15% by age over everything, Italians -25% dock/university and -15% age advances, Byzantines -33% Imperial, Turks -50% gunpowder technologies, Spanish blacksmith upgrades gold-free, Vietnamese economic upgrades wood-free — building scopes derive from the hosting table so no second list exists. The Ethiopian-team tower/outpost sight followed in v0.3.84 through the building-vision seam (below). Listed as the open remainder: the lines needing absent content (Berbers' Genitour, Italians' Condottiero, the Vietnamese Imperial Skirmisher, Burmese minimap relics). The Incas farm line was missing from BOTH the implemented set and this remainder for several versions — an audit against `civilizations.csv` found it, which is the argument for auditing a table against its source rather than against the list of things you remember deferring.
...
**Garrison is Alt+right-click** on an owned garrisonable building, using the same eligibility rules as before. **Ctrl+right-click is the explicit ATTACK order (v0.3.102)** — the deliberate command that can target an ALLY's unit or building; a PLAIN right-click on an ally is always a walk to them, never friendly fire (before v0.3.102, teams made a plain click on an ally's unit an attack — a misclick war). Enemies are attacked by plain click as always; trade routes to any Market and the never-attack rules are unchanged. Seam note: the pointer→view wrapper used to silently drop the iso pick and the Alt flag, so LIVE Alt+garrison had never actually worked — every modifier now flows through, and the seam carries tests. **The idle villager bell (v0.3.103).** AoE2's answer to "who is standing around?": a bottom-left HUD button (always in the DOM per the countdown-chip no-reflow rule; dimmed at zero) shows the count of the human's villagers with no command, an idle gather task, and no shelter; clicking it — or pressing `.` — selects the next one ROUND-ROBIN (next-greater id, wrapping, stable as the set changes) and centres the camera on them. Bridge surface: `countIdleVillagers()` / `selectNextIdleVillager()` (`idleVillagerOps.ts`); the browser spec clicks the REAL DOM button, per the unwatched-seam lesson. LAYOUT RULE (v0.3.186): the bell stands 8px ABOVE the bottom bar — its offset is measured from the bar's top edge every frame rather than fixed against the viewport, because the bar's height depends on the selection and the window size, and a fixed offset put the bell ON the bar, over its SELECTION label, whenever a villager was selected (defect register 2026-09-02); the speaker toggle stacks 10px above the bell the same way. Because the bell follows the bar it moves when the bar's height changes with the selection — only at window widths of 1120px and below, where the bar wraps (37px on a villager selection at 800x600; 1px above 1120px); a fixed offset was rejected because it would float the bell above an empty bar, and a selection-independent bar height is the standing follow-up. No floating HUD control may overlap the bar's heading or another floating control at any window size — `tests/browser/hud-floating-controls.spec.ts` holds that class at 800x600, 1280x720 and 1440x900, empty and with a villager selected. **Control groups (v0.3.104).** Ctrl+digit binds the current selection to that digit; the digit recalls whoever still stands (selectByRefs prunes the dead — an all-dead group is forgotten, as in AoE2); a quick second tap (within 450 ms) centres the camera on the group, and like the double-click that window runs between the two keydowns' own input timestamps, never on a clock read while the key is being handled. Groups PERSIST through save/load (v0.3.112, the DE behaviour): they live in the `aoe2.controlGroups` codec, so the save carries every digit's refs and the loaded game recalls the same groups — the generation-aware recall prune also drops members who died before the save. Saves from before v0.3.112 simply carry no groups. Bridge surface `assignControlGroup`/`recallControlGroup` (`controlGroupOps.ts`); the browser spec presses the REAL keys. **The Delete key (v0.3.114).** Pressing Delete removes the PRIMARY selected entity if it is the player's own unit or building — one entity per press (never the whole group, so a misclick cannot erase an army), no refund, enemies/resources/empty selections refused. It flows through the recorded `entity.delete` command (validator enforces ownership), so replays reproduce deletions and the same destroy ops as combat settle population, refs, and rally points. AoE2's own affordance for freeing population space or clearing a misplaced building. **Shift-queued waypoints (v0.3.125).** Holding Shift on a ground right-click APPENDS a movement leg instead of replacing the order — AoE2's waypoint chain. The chain lives on the move command itself (`queuedTargets`, capped at 8 so a click storm cannot grow an unbounded save field) and the arrival handler starts the next leg instead of clearing, so the chain composes with the §12.7 arrival rules and survives saves. The append rides the RECORDED `unit.move` stream (a `queue` flag; commands process in submission order, so the plain click's move has landed by the time its shift-click follows) — which is what makes replays reproduce chains. A plain click replaces the whole chain. **Shift-queued ENTITY orders (v0.3.141).** The same held Shift on an ENTITY appends a context order to the unit's chain — gather after gather, kill after kill, repair after walk — stored per unit in the serialized `queuedEntityOrders` codec (generation-safe refs, capped 8) and routed, when its turn comes, through the SAME context router a live right-click uses, so a queued click can do everything a plain click could. Completion is watched two ways: a generic system fires the next entry for any unit whose whole order stack emptied (attack target dead, move arrived, repair done), and the villager economy pops in-pass when a gatherer idles — with the explicit-order flag dropped at the moment the CLICKED resource depletes while a chain waits, so the same-type auto-rotate stands down for exactly the DE rule: the queue outranks auto-continue, and auto-continue still serves villagers with no queue. A plain explicit order of any kind (move, attack, gather, patrol, attack-move, attack-ground, cell context) wipes the unit's chain at the recorded-handler layer; death and garrisoning end it too. Shift on an IDLE unit acts immediately, as in AoE2. **Shift-queued construction (v0.3.126).** Holding Shift when confirming a building placement stamps the foundation, KEEPS placement mode armed (stamp the next without reopening the menu), and — for a builder already mid-build — APPENDS the new site to his chain (`queuedBuildRefs` on the build command, capped 8, serialized additively) instead of tearing him off his current roof. On each completion the finalize path advances to the next still-in-progress queued site, dropping refs whose site vanished or finished (another villager may have built it); the queue flag rides the recorded `building.placeConfirm`, so replays reproduce chains. A builder with no standing build takes the first placement as a normal assignment even under Shift. **The town bell (v0.3.115).** The Town Center's command card carries "Ring Town Bell": every villager runs for the NEAREST owned shelter with free room (any complete villager-admitting garrisonable — TC, towers, castle; assignment fills capacity greedily so an overflowing house sends the rest next door), walking in through the ordinary garrison-walk order. "Back to Work" (offered once anyone shelters anywhere) releases VILLAGERS ONLY from every owned building — a deliberately-garrisoned soldier stays on the wall. Both are `building.action` commands, so replays reproduce the alarm; the bell acts on the rung building's OWNER, not just that one building. The modifier is read from the pointer event, so there is no armed mode to enter or cancel and the target building is still chosen by pointing at it. Alt+right-click on anything that is not an owned garrisonable building falls back to the ordinary right-click behavior for that target.
...
**Right-click on an OWN foundation is a build order, on both of right-click's routes (v0.3.208).** A villager right-clicked onto an own building that is still under construction goes to work on it — AoE2's way of rescuing an interrupted build and of adding builders to speed one up, and the one case where a right-click on an owned building is not a move. Any cell of the footprint counts, not just the anchor, because that is what the player clicks, and there is no visibility gate: an owner always knows where its own foundation is, as with Alt+right-click garrisoning. The branch is villager-only and precedes the garrison branch, since a foundation cannot be garrisoned; on any precondition miss — a non-villager, another owner's site, a building that is missing or already complete — the click falls through to the ordinary routings above. Right-click reaches the simulation by TWO routes, `unit.contextAtEntity` when the entity hit test claims the click and `unit.context` when it does not, and a rule belonging to right-click holds on both: the entity route has carried this since 0.1.17 (multi-villager construction) and the cell route had no build branch at all until v0.3.208, so the click fell through to a plain move onto a cell the foundation itself blocks and the villager arrived at the site and stood there. `tests/simulation/buildExistingFoundation.test.ts` holds the cell route and asserts build PROGRESS rather than that a task was assigned, because `issueContextCommand` returned true throughout the defect.
...
- click-select
- drag-select
- double-click same-type on-screen select. Two clicks in quick succession on a friendly unit select every unit of that type on screen, and this OUTRANKS the stacked-entity cycle: both gestures are "click the same place again", so the 300 ms window is what tells them apart, and a slower repeat click still walks the stack of entities under the cursor. It holds whatever was selected before: the pair selects every unit of the type of the player's own unit (or owned sheep) on top at the cursor, which is what a plain click there selects, even when the pair's first click walked the stack because that unit, or something an earlier click walked to, was already selected; a click on anything else between the two breaks the pair (v0.3.231). The window is measured on the INPUT clock — between the two clicks' own input timestamps (the pointer events' `timeStamp`) — never on the rendered-frame clock and never on a clock read while the click is being handled: a busy page handles a click late, so a handler clock stretches a quick double-click past the window, and a gesture that stops working when the machine is busy is not a gesture. Only a pointer click can be half of a double-click.
- control groups
- shift-queued orders
- DE muscle-memory hotkeys (v0.3.155): `H` selects the Town Center and centres (round-robin over multiple TCs), `,` cycles idle MILITARY (the '.' villager bell's companion, same round-robin over the human's non-economy units with no command/task/shelter), `F3` toggles the same manual pause the game menu preserves, `+`/`-` step the in-match game speed through the §4.5 ladder (1.0/1.5/2.0), and `Space` (v0.3.156) jumps the camera to the last town-under-attack event — the horn's companion, remembered by the audio controller's own home-hit detection; in replay mode Space stays the replay play/pause toggle (the boot binding arbitrates, since the registry is first-match). Arrow keys and WASD pan the camera continuously and pointer edge-scroll pans after a short hover delay (both pre-existing); wheel zooms; middle-drag pans; the minimap clicks to pan. The camera BOOTS at a default zoom that frames the whole opening base plus its starting resource ring — berries, wood line, gold and stone, the herd — the way AoE2 DE's default framing does (1.2 of the 0.7–2.4 wheel range, v0.3.158; the old 2.0 default filled half the screen with the Town Center alone and pushed parts of the starting resource ring off-frame or to the screen edges beside the HUD chrome, where several playthrough-probe clicks aimed at them never reached the canvas).
...
- The mangonel line has NO anti-infantry attack bonus — its effectiveness vs massed infantry is BLAST/splash (§10.7). Mangonels keep +12 vs the `siege` class. ATTACK-GROUND shipped v0.3.117: G arms it and the next left click names a CELL; every selected blast-capable unit (the validator gates on the unit's own blast radius — the area effect IS the attack) walks into range of that cell and bombards it on its ordinary reload, with the shot flying as a dead-target area projectile so the impact resolves as pure blast at the aim point — friend and foe alike, targets or no targets. The order never self-clears (ground cannot die), it silences auto-aggression like any standing order, it survives saves ('attack-ground' joined the serialized command union), and replays reproduce it through the recorded `unit.attackGround` command. Ineligible units in a mixed selection are skipped.
...
| Treadmill Crane | Castle | 200 wood, 50 stone | Builders work 20% faster |
...
Implemented model (mangonel line): a blast unit's attack lands full damage on its primary target, then deals area damage to EVERY OTHER unit (enemy AND friendly — friendly fire) within the unit's `blast_radius` (design/stats/units.csv) of the impact cell. Each splashed unit's damage is computed the same way as the primary — attack type (melee/pierce) + class bonus, reduced by that unit's own armor, floored at 1 — so a splashed spearman takes the mangonel's base pierce damage (the mangonel line has no per-target anti-infantry bonus; its power against massed infantry IS this area effect, restoring the AoE2 counter). The attacker itself is never splashed. Blast is DERIVED from the attacker's unit type at the damage site (no persisted state, no save-format change) and applies in a deterministic order (by entity id). The mangonel-line `blast_radius` follows units.csv exactly: mangonel 1 and onager 1.25 reach the four orthogonal neighbours (a diagonal sits √2≈1.41 away), and SIEGE ONAGER'S 1.5 (fixed v0.3.128 — the table had carried a stale "off-roster" note from before the upgrade line shipped, so the Imperial upgrade blasted no wider than an onager) is the first radius that also catches the diagonals, which IS the upgrade's identity at this grid resolution. Distance-attenuated blast for non-siege attacks remains deferred with elevation (§10.5).

#### 10.7.1 Siege Engineers (siege-range tech)

Siege Engineers (implemented v0.1.62) is a researchable tech that gives the owner's SIEGE units +1 attack range. A "siege unit" is any unit in the `siege` armor class (§10.3) — the mangonel line, the scorpion line, the ram line, the Bombard Cannon, and the Trebuchet; non-siege units (archers, cavalry, infantry, monks, villagers) are unaffected. Cost 500 food / 600 wood, 450 ticks (technologies.csv 45 s), Imperial Age. The +1 is applied in TWO places, mirroring the Blacksmith Fletching pattern: DERIVED at unit creation (`createCombatState` adds +1 when the owner has the tech) for new siege units, and imperatively in `applyTechnology` (a per-unit loop over the owner's existing siege units) the moment the research completes. The already-researched guard keeps it single-applied (no double stacking). No per-entity state, no save-format change. Hosted at the UNIVERSITY as technologies.csv places it (v0.3.131 — the old "University does not exist yet" divergence note had outlived the University by many releases; the absence-claim audit retired it, together with the tower techs' twin note in §10.8). Offered as an Imperial-Age University research option; drops once researched.

#### 10.7.2 Sappers (villager anti-building tech)

Sappers (implemented v0.1.64; corrected v0.3.132) gives the owner's VILLAGERS +15 attack against buildings — technologies.csv's applies-to says Villagers, the siege-crew reading of the tech, and the CSV is the spec of record; the original slice had read it as an infantry tech. Non-villagers (infantry included) are unaffected. Cost 400 food / 200 wood, 100 ticks (technologies.csv 10 s), Imperial Age. Unlike Siege Engineers, Sappers is applied purely DERIVED at the single unit-to-building damage site: the per-hit building damage becomes `attackDamage + siege-anti-building-bonus + sappersBonus`, where `sappersBonus` is 15 when the attacker's owner has researched Sappers AND the attacker is a villager, else 0. The building-damage site applies no armor reduction, so the +15 lands raw. There is no per-entity state and no `applyTechnology` mutation — the bonus is read fresh from the owner's researched-tech set on every hit (the buildingArrowTechEffects pattern), so it applies uniformly to existing and future villagers with no double-application risk. No save-format change. Hosted at the CASTLE as technologies.csv places it (v0.3.132; the old note parked it at the Blacksmith under the same dead "University does not exist" claim the audit retired — and the CSV never put it at the University either). Offered as an Imperial-Age Castle research option alongside Conscription/Hoardings/Spies; drops once researched.
...
Defensive tower-upgrade techs (Guard Tower → Keep, implemented v0.1.57). The Watch Tower ladder mirrors AoE2's Watch Tower → Guard Tower → Keep progression, but SCOPED to attack + range: Guard Tower (Castle Age, no tech prereq, 100 food / 250 wood, 300 ticks) gives the owner's Watch Towers +2 attack; Keep (Imperial Age, requires Guard Tower first, 500 food / 350 wood, 750 ticks) gives +2 more attack (so 0 / +2 / +4 across none / Guard Tower / Keep) AND +1 range (base 7 → 8). Hosted at the UNIVERSITY as technologies.csv places them (v0.3.131; the old Watch-Tower hosting and its "University does not exist yet" note were retired by the absence-claim audit). The bonus is DERIVED (pure) from the owner's researched-tech set at the tower's fire site (towerTechEffects, no per-building state, no save-format change) — a Watch Tower whose owner has the tech fires harder/farther with no per-entity mutation, exactly like the derived economy techs. The bonus applies ONLY to Watch Towers; Town Center and Castle defensive fire are unchanged. The hit-point half ships through the Masonry seam (§11: Guard Tower 1020→1500, Keep →2250) — an older DEFERRED note here outlived that mechanic too.
...
- **Careening** (Dock, Castle Age, 100 food + 200 gold, 50s): +1 PIERCE armour and no melee armour — technologies.csv says "+0/+1", making it the only armour technology in the game with no melee half, so it bumps the pierce-only accumulator directly instead of going through the shared armour path. It also raises a Transport Ship's capacity by 5.
- **Dry Dock** (Dock, Imperial Age, 200 food + 400 gold, 60s): every ship moves 15% faster, and a Transport Ship carries 10 more.
- **Shipwright** (Dock, Imperial Age, 1000 food + 300 gold, 60s): a ship's WOOD cost falls 20% (a Galley's 30 gold is untouched) and its build time falls 35%. The cost discount is applied at `effectiveTrainingCost`, the single seam every charge and every affordability check already shares, so the price a player is quoted, gated on and charged cannot disagree.
...
**Costs and times gated (v0.3.226).** The fifth content differential: every unit's cost and training time and every technology's cost and research time, against `units.csv` and `technologies.csv`, in `tests/content/trainAndResearchCostsAndTimes.test.ts`. Nothing had compared those four columns before, and 38 rows disagreed in 52 places: the Onager upgrade charged 500 wood where the CSV asks 500 gold, Guard Tower and Keep were priced in gold at a fraction of their cost, Paladin researched in 60 s against 170, Forging, Padded Archer Armor and Scale Barding Armor charged 50 gold the CSV never asks for, and the Longbowman cost food instead of wood. Where the two disagreed and the CSV's value is not DE's either, the game charges DE's value and the gate names the difference: thirteen named exceptions on twelve rows on 2026-09-23, each citing the aoe2techtree dataset generated from the DE game files, pinned at `3bb43b14` (DE update 185872). Three are CSV errors the game already charged DE's price over (Man-at-Arms upgrade 100 food + 40 gold, Chain Mail Armor 200 food + 100 gold, Spearman 25 wood). Ten are fields where neither side was DE's and the game moved to DE: the Arbalester and Champion upgrade costs, the Champion, Two-Handed Swordsman, Iron Casting, Cavalier and Heavy Camel research times, and the Heavy Cavalry Archer and both Longbowman training times. With no DE value to set against it, the CSV wins outright: Berserkergang charges the CSV's 500 food + 850 gold, because DE has no Berserkergang (Bogsveigar holds its slot, and Vikings keeping Berserkergang is a content gap, not a price). An exception pins both sides, so it goes red when the CSV row changes under it or comes to agree with the game. The CSVs are mostly Conquerors-era for costs and times: when this gate landed, 97 rows on which the game follows the CSV in full still differed from current DE, 117 fields in all. Re-sourcing those columns from the DE dataset, as `tech-tree.csv` was at v0.3.139, was the next step, and it dissolves these exceptions. **Re-sourced (v0.3.228).** It landed the same day: both files' cost and time columns now hold the DE dataset's values for every row DE prices, pinned in each file's header, 108 rows in all (57 technologies, 51 units). The thirteen exceptions dissolved, each going red as DISSOLVED as designed, and the gate is now a plain row-by-row equality. Rows DE has no price for keep their earlier values and are named in the header: Treason, the Cannon Galleon technology, Bearded Axe, Berserkergang (Bogsveigar holds its slot), El Dorado, Mahouts, Zealotry, the King, and the Fast Fire Ship upgrade, which DE folds into its Galleon technology while this game keeps the two separate. **Structures gated (v0.3.137).** The fourth content differential: structures.csv hit points (as the Imperial top step of the per-age ladder) and garrison capacities against the building tables, with the Guard Tower/Keep rows checked as UPGRADE rows through the Masonry-seam multiplier chain — clean at 28/28 rows on arrival, so this one is pure insurance. All four content dimensions (tech hosting, attack bonuses, armor bonuses + base stats, structures) are now named-failure differentials in `tests/content/`. **Base stats corrected and gated (v0.3.136).** The third differential — every units.csv numeric column against the stat tables — found and fixed: the militia line's HP running 5 high across three tiers (Man-at-Arms 45, Long Swordsman 55, Two-Handed 60), the Elite Longbowman two range short of its identity (now 8 — with Fletching it outranges a Guard Tower), the Siege Ram an attack point light (4), Petard and Trade Cog armor TRANSPOSED between the melee and pierce tables, and the Scout Cavalry's hidden Feudal buff (units.csv gives the SAME scout 3 attack in the Dark Age and 5 from Feudal — now modeled through the age-scaled seam: creation reads the owner's age, the age-up sweep bumps standing scouts by the difference, civ-independent). Conventions the model owns are encoded in the gate rather than silently excepted: melee rows' range 0 is 1 in our grid (adjacent reach), fractional ranges floor, the melee-armor table holds the EFFECTIVE value (base plus the armor_bonus column's −N melee), and age-conditional attack compares the top-age value. `tests/content/baseStats.test.ts` enforces all of it by name. **Class armor — the defensive half (v0.3.135).** units.csv's `armor_bonus` column is class-specific RESISTANCE, now modeled: bonus damage per class is max(0, attack bonus − the target's class armor), summed over matching classes with a PER-CLASS floor (a surplus in one class never eats another class's bonus) — AoE2's own damage model. The entries: Cataphract +12/+16 vs the cavalry class (a Halberdier's +32 lands as +16 on the elite — the unit's whole identity), Mameluke +11/+11, Turtle Ship +8/+11 vs ships (+1 turtle-class on the elite) — the galley line's bonuses nearly cancel on the Korean shell — fire/demolition hulls +3..+7 vs ship/camel bonuses, and the upgraded rams' +1/+2 ram-class armor. The ram line's famous NEGATIVE melee armor (−3) lands in the base melee-armor table — swordsmen chop rams 3 harder per hit, the classic answer. Siege Engineers' true effect also corrected here: the CSV ram rows' larger building figures are the tech's ×1.2 multiplier (now applied to a siege attacker's whole per-hit building damage at the delivery site) and the fully-garrisoned variants (garrison-adds-damage stays deferred) — the plain bases are 125/150/200, which v0.3.134's first parse had over-read. Both columns are gated: the attack-bonus and armor-bonus differentials in `tests/content/attackBonuses.test.ts` fail by name on any drift. **The attack-bonus differential and its gate (v0.3.134).** The audit's systematic form found FORTY-SIX more silently-dropped units.csv attack_bonus entries and corrected them all: the entire NAVAL bonus fabric (the galley line's +8/+9/+11 vs ships and camels with +3/+4 vs rams — naval combat's defining numbers; the fire-ship line's +3/+4 plus a new `turtle-ship` class for its +2/+3 against the Korean shell; the cannon galleon's +15 infantry/archer/cavalry, +40 siege, and +200/+275 buildings), the camel line's +5/+9 vs ships, the spear line's missing ship halves (+7/+11/+16), the Hand Cannoneer corrected to +1 spearman/+2 RAMS (it had a +2 'siege' the CSV never gave it), the Elite Skirmisher's +6 against a new `cavalry-archer` vulnerability class carried by every mounted archer, Petard +60 siege, the last four anti-eagle rows (Woad Raider, Throwing Axeman and elites), and the ram-line building values (Battering 150, Capped 180, Siege 240) with siege-onager +60, demolition 220/280. ENFORCED: `tests/content/attackBonuses.test.ts` parses every CSV row's attack_bonus into class expectations (named token map), compares both directions against `UNIT_ATTACK_BONUSES` and the building table, and carries a named deferred-token list (building-KIND bonuses per §10.6, '+N melee' notes) plus one documented exception (the Bombard Cannon's self-contradictory "+40 siege/camels;+20 siege" row — we model +40 siege). **The audit's harvest (v0.3.130).** The same off-roster-comment audit closed three more silently-dropped stat families: the WAR-ELEPHANT class (stacks with cavalry under the spear line, AoE2's rule — a Halberdier lands +32 cavalry +60 elephant = +92 on one swing; the scorpion line's +6/+8 rides along), the CONQUISTADOR joining MOUNTED_UNITS (technologies.csv's applies-to names it outright — Bloodlines and Husbandry now reach both tiers), and the SCOUT LINE'S ANTI-MONK ladder (+6/+10/+12 — its CSV identity, deferred back when no 'monk' class existed). The conversion-vision fixture swapped its scout for a militia: the scout's new +6 wins the melee race against a conversion, and that fixture's contract is the vision flip, not the matchup. **The eagle armor class (v0.3.129).** AoE2's famous inversion, restored: the Eagle line's vulnerability class is `eagle`, NOT `infantry` — a Hand Cannoneer's +10 infantry (and a Jaguar Warrior's +10 infantry) do nothing to an Eagle — while the swordsman line's +2/+4/+6/+6, the spear line's +1s, and the unique-infantry answers (Jaguar +12, Teutonic Knight +4, Berserk/Huskarl/Samurai +2–3) land as units.csv writes them. Eagles still take the infantry TECH scope (blacksmith, Squires, infantry civ bonuses) via `isInfantryUnit`'s second clause — upgrade scope and vulnerability are different taxonomies, the War Wagon rule applied again. The Eagle line also gained its own missing CSV bonuses (+2/+4 cavalry, +1/+2 camel and ship). Found by auditing every "off-roster" comment after the Siege Onager case (v0.3.128): fourteen CSV rows carried anti-eagle values that had been silently dropped since before the Eagle line shipped. **El Dorado (implemented v0.3.57).** The Mayans' Imperial unique technology, Castle, 750 food + 450 gold, 50s: +40 hit points to the Eagle line. It was deferred with the note "Eagle Warriors are not on the roster" — true when written and false from v0.3.48, which is the argument for re-reading a deferral list whenever the roster grows rather than trusting it.
...
**Cartography (implemented v0.3.63, RETIRED v0.3.140).** Current DE removed Cartography from the game and made allied shared sight the default, and this game follows: from the first frame of a match every player's projected frame unions its allies' visible AND explored cells with its own (mutually — DE's modern rule), a projectile over an ally's ground draws for the same reason, and the Market opens on the tribute technologies instead. The one-way Feudal research described here for v0.3.63-v0.3.139 no longer exists.

**Shared sight shows everything in it (v0.3.230).** A player sees through its SIGHT — its own vision plus every owner whose vision it shares (its allies, and with Spies every other player) — and every human-facing question is answered by that sight, never by the player's own vision alone: every live unit, building, farm, tree, berry bush, gold and stone mine and animal standing on a cell some owner in the sight can see is drawn, exactly as the lit ground under it is; fog memory records what the sight saw, so an ally's base and an enemy building an ally spotted stay on the map as last seen; a click can select an ally's unit or building — the selection panel names its faction Ally, not Enemy — and can target an enemy that only an ally sees; and an attack swing, death or projectile an ally witnessed animates. Anything no owner in the sight can see stays hidden — an enemy base nobody on the team has scouted is not drawn. A plain right-click never attacks or converts an ally, on the by-entity route and the ground route alike: a soldier, villager or Monk walks to the ally's unit or building, and attacking an ally takes the explicit Ctrl force-attack (v0.3.102). The trade rule of §6.7 is unchanged — a Trade Cart on another player's Market, or a Trade Cog on another player's Dock, opens a route whether that player is an ally or an enemy. A Monk sent to convert an enemy only an ally sees finishes the conversion: whether a conversion progresses follows the Monk owner's sight, for a unit and (with Redemption) for a building, because DE lets a player act on what its allies see. Most other rules of play still ask the acting owner's own vision — a tower's target, a unit's automatic target, the AI's searches — and the per-owner records stay per-owner: the visibility map and the attack and death witness lists are written for each owner separately. What follows the sight in world state is therefore the human's fog memory (saved and replayed), the target a ground right-click picks, and a conversion's progress. All of it reads teams and technologies from world state, so a replay re-simulates it exactly; a recording of a team game (or of one where player 1 had Spies) made before v0.3.230 was made under the old rules, its fog memory differs, and its right-clicks and conversions are re-decided, so it may not re-simulate to its recorded state at all. One helper answers every such question (`src/game/simulation/bridge/humanSight.ts`); `tests/simulation/sharedVisionEntities.test.ts`, `tests/simulation/sharedSightAnimations.test.ts`, `tests/replay/makeReplayBridge.sharedSight.test.ts` and `tests/architecture/humanSightQueries.test.ts` hold the rule (register 2026-09-24: an ally's ground was lit and its whole base missing).

The owners whose vision is shared come from `sharedVisionOwners` — the allies, or every other player once Spies is researched, and empty in a free-for-all without Spies, so such a match projects exactly the cells it always did, in the same order. It is read per FRAME rather than cached, because researching Spies mid-match has to take effect on the next frame rather than the next reload.
...
Ordering your own units to attack an ally takes the explicit Ctrl force-attack (v0.3.102); a plain right-click on an ally never attacks it (see "Shared sight shows everything in it" in §10.9). Still-open team work, listed rather than assumed: a Monk heals only its own owner's wounded units, while DE's Monks heal an ally's too.
...
- healing damaged friendly units — in this build the Monk's own owner's units only; healing an ally's wounded unit, as DE does, is open (register 2026-09-24)
...
**Research hosting follows the CSV (v0.3.131-133).** Every technology researches at the building technologies.csv's Building column names — the audit that restored Guard Tower/Keep and Siege Engineers to the University, Sappers to the Castle (and its scope to villagers), and Chemistry to the University (its gunpowder gating and archer +1 are unchanged; only the door moved). ENFORCED as a differential gate: `tests/content/technologyHosting.test.ts` compares every CSV row's building against `RESEARCHES_BY_BUILDING`, with a named alias table for slug differences and a named intentional-absence list (Treason — Regicide-only, a mode this game does not play), so a hosting that drifts from the CSV fails by name. Free technologies (below) ask the research MENU, so they follow a hosting move automatically — a Korean player's free Guard Tower now waits for a University, exactly as in AoE2. **Free technologies (v0.3.78).** Twelve civilizations' "X free" lines are live: the technology researches itself, instantly and at no cost, the moment the owner could legally research it — its age reached and a completed building standing whose research MENU offers it (the grant asks `getResearchOptions`, so every prerequisite rule lives once and the free path can never reach something a player could not). Aztecs Loom, Byzantines Town Watch, Franks farm upgrades (Mill required, as in AoE2), Koreans Guard Tower and Keep, Teutons Murder Holes, Turks Chemistry and the light-cavalry upgrades, Vikings Wheelbarrow and Hand Cart, Burmese lumber upgrades, Ethiopians spear-line upgrades, Magyars the forging line, Malians gold mining, Slavs Tracking.
...
- **unexplored ground is drawn BLACK in every art style, and nothing about an unexplored cell may reach any picture the player sees.** This is fog of war, not a look (defect register, 2026-09-23: unexplored ground was drawn at 12% of its colour, and a contrast stretch showed every lake and forest on the map; Definitive Edition draws it black). Three drawing paths leaked, and each is closed: the ground itself; the edge of the explored area, where the terrain colour pipeline blends each known cell toward neighbours of a different kind and must treat an unexplored neighbour as if the map ended there; and the minimap, which fills the map with one opaque black and never draws an unexplored cell's terrain. Explored-but-unseen ground is dimmed to about half its brightness, at the art style's own level (§14.5). Gated by NON-INTERFERENCE: changing any unexplored cell's terrain must leave the world's render snapshot and the minimap's draw calls identical (`tests/rendering/aoeVoxelFogFairness.test.ts` for each style, `tests/ui/minimap.test.ts`). `tests/browser/art-style-setting.spec.ts` also reads the real canvas over unexplored ground in both styles and requires one colour, black in the DE style. **Known open gap:** the building-placement ghost still breaks this rule, because its valid/invalid colour over unexplored ground follows the terrain and occupancy there, so sweeping it across black ground maps the lakes and forests (defect register, 2026-09-23, OPEN). Whether Definitive Edition lets a building be placed on unexplored ground is not yet verified against a reference.
...
- **Town Patrol** — Town Center research, Castle Age, 300 food + 100 gold, requires Town Watch. Grants a further +4 building line of sight (stacks with Town Watch for +8 total).
- **Tracking** — RETIRED v0.3.140, as in current DE: the technology is gone and the infantry line's units.csv line-of-sight values (sourced from the DE game data) carry what it used to grant. The Barracks hosts no line-of-sight research.

Each tech applies its bonus imperatively to the owner's existing entities the instant it completes (the vision radius is bumped in place and the fog re-reveals on the next tick) and is derived at creation for entities produced afterward, so a building finished or a unit trained after the research sees the same distance. The Town Center building-LoS techs are player-driven (mirroring the AI's hands-off stance on the other Town Center economy techs, which keeps its age-up timing protected).
...
| Technology | Age | Cost | Effect |
| --- | --- | --- | --- |
| War Galley | Castle | 150 wood, 100 gold | Galley -> War Galley |
| Galleon | Imperial | 400 wood, 315 gold | War Galley -> Galleon (requires War Galley) |
| Fast Fire Ship | Imperial | 280 wood, 250 gold | Fire Ship -> Fast Fire Ship (DE folds this into Galleon; the price is the earlier one) |
| Heavy Demolition Ship | Imperial | 250 wood, 300 gold | Demolition Ship -> Heavy Demolition Ship |
...
A fractional percent is made real by a per-unit carry accumulator (`moveCarryHundredths`) that banks the unconsumed entitlement across ticks and per-cell waypoint clamps; rounding the step instead would silently drop everything under a whole fine unit. The per-tick entitlement is the ROUNDED product of base and percent, a deliberate integral-bank bias of under half a percent (112% plays as 112.5%, 110% as 109.4%). The carry is bounded PROPORTIONALLY — 1.5 ticks of entitlement, floored at one fine step plus a tick so fractional rates can always afford their next step — which caps a long-blocked unit's release burst at roughly one fine step; in an unobstructed walk the bank self-regulates below the cap.

Landed v0.3.160: the BASE clock itself is honest — 0.32 fine units/tick (subgrid 4, TPS 10) puts the 100% villager at exactly 0.8 tiles/second, pinned by `tests/simulation/movementSpeedPacing.test.ts`. Before the retune the base step was 2 fine units/tick (5 tiles/s, 6.25x DE) and every walk in the game read as teleporting. Because 0.32 is fractional, EVERY unit now rides the carry accumulator — there is no whole-step fast path any more. Honest walk speed exposed three systemic defects the hot clock had been hiding, all fixed with it: the AI's placement connectivity guard was margin-boxed and approved farm walls that collectively sealed its own drop-off corridors (now a whole-map flood); carriers stuck behind such a seal re-ran full-map failing path searches per approach candidate every retry (now cached against the world occupancy's structural revision — unit churn never changes cell reachability, only building/resource/terrain changes do); and the traffic election's recursive dependency closure copied its visited-set per branch, going combinatorial once a dozen carriers shared a door's cells (now one iterative BFS). Profiled on the feudal-stone fixture: 12.3 seconds per tick at tick 8,340 before the fixes, ~2 ms after.
...
- A converted unit is judged by its NEW owner at every gate from the moment it flips, including on a route it had already planned: a villager converted beside its old side's gate does not walk through that gate, and stands outside it under the ordinary unreachable-target rule instead (v0.3.183 — a remembered route is invalidated by the flip exactly as by a topology change).
...
**Starvation relief in a closed jam (implemented v0.3.175; clock corrected v0.3.183).** Lowest entity id stays the ordinary rule for a closed directed cycle, because replacing it outright perturbed maps that never jam. It yields only to a member that has been WAITING for more than 750 ticks — asking to move on every tick and not changing cell — and among such members the longest wait goes first, ties to the lower id. The clock measures continuous contested waiting and nothing else: it restarts when the unit changes cell (progress) and when it resumes asking after a gap of more than one tick, so a builder at its site, a garrisoned unit or a parked army accumulates no claim while standing still and cannot jump the queue on its first step. Every member of a jam reads every member's clock from the same tick-start snapshot as the rest of the election's inputs, so all members elect the same winner whatever order the systems consult the arbiter in. Persisted per unit as the last progress tick and cell; a save from before the rule reads as "moving" and starts the clock at its first contested attempt.
...
- Buildings, resources, impassable terrain, and out-of-bounds claims block whole tiles; units cannot share a whole-cell-blocked tile. ONE exception, and it is a claim kind rather than a special case (2026-09-10, v0.3.221): a FARM claims its footprint as walkable ground, so it blocks no tile and units take ordinary sub-tile slots on it — see §6.6. The divergence is stated once, in `worldOccupancyCells.blocksWholeCell`, which spawn passability, wildlife passability, the spiral search and the group allocator all read; placement counts every claim, so a farm still refuses further building.
...
**An order the game cannot carry out says so (v0.3.219).** When the ordered cell cannot be reached, the unit still walks as far toward it as it can — to the reachable cell closest to the order, chosen by smallest `(manhattan distance to the ordered cell, y, x)` — and the player is told, once per order gesture rather than once per unit, on the same rejection-toast surface the placement validator uses: what happened, which cell was ordered, what is in the way, and what would satisfy it ("Cannot reach (55, 30): forest and a town-center (building) block every route out from where all 4 selected units stand. Clear a path, or pick a cell on their side of it."). Reachability is answered by a flood out of the ordered cell over the same passability predicate the pathfinder walks, so the message and the movement can never disagree; a selection is partitioned by unit DOMAIN, since a ship and a soldier disagree about every cell. A command that is accepted and then silently dropped is not an acceptable outcome for any walk order.
...
**The AI trains the unit TIER it holds (v0.3.199).** The AI's unit mix names the top of each line for the age — Imperial asks for Halberdier, Arbalest, Cavalier, Siege Ram and Onager — while a production building only offers the tier its owner has actually RESEARCHED. Those two are reconciled by unit LINE, not by name: asked for a Halberdier at a Barracks offering Pikemen, the AI trains the Pikeman, which is what a player does while an upgrade is unresearched. It resolves in both directions, so an owner who has upgraded PAST the named tier — a Dark-Age mix asking for Militia at a Barracks offering Man-at-Arms — trains the better unit rather than none. Lines are derived from the unit-upgrade table itself so the tiers cannot drift from the upgrades that create them, and a resolution never crosses lines: Scout/Light Cavalry/Hussar and Knight/Cavalier/Paladin share the Stable and stay distinct. Before this, the two were matched by exact name, so an AI that reached the Imperial age without those upgrades — the default path, since reaching an age researches nothing — matched NOTHING at any building and stopped training units permanently: measured on `fortress` at 60,000 ticks, both owners held five military buildings idle in 100% of samples with armies of 2 and 0 while holding 1,231 wood and 595 food, enough for seventeen Halberdiers. After: that owner's army goes 0 to 3 and it destroys its opponent outright, and `gold-rush` resolves at tick 34,704 instead of 41,967.

**AI placement never seals its own ground (implemented 2026-08-23; the unguarded fallback removed 2026-09-08).** The AI's ring search skips any anchor whose footprint would cut the free ground beside it in two: the free cells orthogonally touching the building must still reach one another without passing through it. The check is a WHOLE-MAP flood, not a local window — a three-cell margin box approved every individual farm of a wall whose COLLECTIVE effect sealed the drop-off corridors three cells further out (§12.4.2), so it was widened to the whole map at v0.3.160 and the "window three cells wider than the footprint" this paragraph used to describe has not been the rule since. Walls and gates are exempt, because sealing ground is what they are for. The search runs rings 2..12 under the guard, then rings 13..24 under the same guard, and then RETURNS NOTHING: there is no unguarded pass. From 2026-08-23 to 2026-09-08 there was one — when no anchor in reach survived the guard the search ran again with it off, on the reasoning that a sealed pocket beats an AI that stops building — and on a map whose only building ground is a corridor that filled the corridor, every time. Measured with an independent flood over terrain plus building and resource footprints: on `black-forest`, with the human idle and only the AI building, the enemy units reachable from the human's own ground went from 4/4 at tick 0 to 0/26 at tick 20,000, and with both sides played by the AI the human's units were split into 50-, 43- and 2-cell pockets by tick 15,000. No battle had ever happened in this game on any seed. Age of Empires II: Definitive Edition's AI never seals itself in — it places elsewhere, or it waits — so the fallback is gone and the wider guarded pass stands in for it; a stalled AI can still fight with what it has, and a sealed one never can. Removing the fallback was measured NOT to cost the AI its development, which is what a rule like this usually costs: run twice in one process on one tree under the coverage lab's own configuration (§15.8), the shipped search reached 15 building types, 17 unit types and 29 technologies with both seats in the Imperial Age, against 15, 14 and 23 with one seat left in the Castle Age when the pre-2026-09-08 search was restored — and it ran 149 empty site searches against that arm's 469, in 11.8 seconds of search time against 18.6. A sealed map is worse ground to search than an open one, so the fallback did not even buy the thing it was there for. The guard also counts what the AI asked for a moment ago: one decision can push a watch tower and a next build, both judged against the world as it currently stands, so every pending `building.placeConfirm` footprint counts as blocked ground — for the footprint test and for the guard, whichever owner asked for it, because two AI seats decide on the same ticks. Two buildings neither of which seals alone can otherwise seal together with each one's guard approving. The guard asks the same passability predicate the pathfinder and the walk-order flood ask, about a probe that is no unit at all, so it can never hold a second definition of "walkable" that movement disagrees with; one consequence is that a farm, which is walkable ground, can neither seal a corridor nor be counted as sealing one, while the placement validator still refuses to build ON it. The original grounding stands: on the default map the AI had packed its buildings into a solid mass ten cells wide across its own base, sealing its sheep, its boar and the forest behind it into a pocket its villagers could not enter, and six of them held that one unreachable sheep while the economy read food 19 / wood 19 / 15 villagers, byte-identical from tick 17000 to tick 30000.

**A gatherer with no reachable resource of its kind takes another kind (implemented 2026-08-23).** When the reachability-aware reroute cannot find any resource of the villager's desired kind that it can actually path to, the villager falls through the kind order (wood, food, gold, stone) with the same reachability requirement rather than going idle. Without this the villager was left idle and the ordinary idle→assign — which does not check reachability — handed the same unreachable resource straight back on the next tick, forever. The AI's periodic villager rebalance (above) still governs the long-run distribution, so a fallback is a stopgap and not a permanent role change. Since v0.3.191 the same walk runs safe-first (§6.4, enemy static defences): a kind whose only remaining nodes stand under an enemy Town Center, tower or Castle counts as having none until every kind has been tried, and only then is one of those nodes taken. It runs at every site that assigns a villager work on its own AND may change its kind — both idle passes, the unreachable-target reroute and the give-up reroute. The post-delivery idle pass was missing until v0.3.191 and was the whole of the residual: a gold villager was handed the mine under the enemy Town Center again on every delivery, nine deaths in four hundred ticks on the boot map. The fifth site, the over-subscription redistribution, deliberately does not walk the kinds — see §6.4.
...
**AI Lumber Camp siting (v0.3.177, REVERTED v0.3.178; radius raised v0.3.200).** The v0.3.177 rule — Lumber Camp on the medoid of currently-targeted trees, Mining Camp requested first in the Dark Age — is withdrawn: at match resolution on the boot map it lost the game the previous behaviour wins, because its larger economy thinned the home woodline sooner and villagers were then sent to gather inside the enemy Town Centre's range (thirty killed between ticks 35,750 and 45,000). The defect that exposed — automatic gather and hunt targeting ignoring enemy static defences — was the stated precondition on any retry, and it landed in v0.3.191 (§6.4). Camps are sited beside the harvestable tree nearest the Town Centre, and the Dark Age requests the Lumber Camp before the Mining Camp, as before. Nomad is unchanged.

The radius was 12 and is 20 (v0.3.200). Twelve could not reach the woodline of the maps that ship: `paintWoodlines` holds every patch at least 14 EUCLIDEAN cells clear of every start so it cannot punch a hole in Arena's ring or Fortress's square, which is up to 20 in the manhattan metric the anchor measures in — so on `arena`, `fortress` and `gold-rush` there was no tree within reach of EITHER start, the anchor returned null at both, and the camp went up beside the Town Centre with the nearest tree fifteen cells away. Measured on `arena` owner 1 over a 60,000-tick match: 19.6 villagers assigned to wood and 3.01 of them chopping at any moment, the rest walking. The two bounds are held together by `tests/simulation/dropOffAnchorReach.test.ts`, which derives its floor from `MIN_START_GAP` and walks every map in `PLAYABLE_MAPS`, so it goes red when either constant moves.
...
**The plan outranks the queue in the Imperial Age (implemented v0.3.222).** The rule above has exactly one trigger — qualifies for the next age, cannot afford it — and there is no next age in the Imperial Age, so from the moment a player reaches it the AI's Market goes dead and stays dead. Every Imperial unit but the Halberdier is priced in gold, so an Imperial AI whose gold runs out loses its wood and food sinks at the same moment and banks both instead, while a seat that banks gold cannot convert it into the food and wood its units also need. So in the IMPERIAL AGE only, three things change. The AI knows what its build order is STUCK on, which is a different building from the one it will start next whenever a cheap entry is affordable and a dear one above it is not — it keeps replacing 60-wood farms while the 200 wood for a Siege Workshop never forms. It emits one Market trade per decision cycle aimed at that blocked building, or at a technology standing unbought at one of its idle producers, and only at a unit its queue wants when neither of those is waiting — a case that, measured over 1,159 trades on two configurations, has never yet arisen, because in the Imperial Age some producer always has a technology it cannot afford: short of gold it sells whichever commodity has the largest surplus above the want's cost plus a working reserve, and holding gold it buys whichever commodity the want is short of. And it buys an affordable upgrade BEFORE the next unit rather than after it, because both are charged from one stockpile in the order they are asked for, and a queue that asks every cycle will always outbid an upgrade bought once. The working reserve is four Market batches of food and of wood and NOTHING of stone or gold: no unit is priced in stone, so a stone reserve is only a hoard with a smaller number on it, and gold is the medium the trade works in. The Imperial restriction is load-bearing on all three parts and was measured: run earlier, the trade spends the gold the AI is saving for its next age and cost one seat both the Imperial Age and 37 of its 38 villagers over a 45,000-tick match, and the upgrade-first ordering outranks the age-up itself and left another seat in the Castle Age at the horizon. A military reserve for the blocked building was also measured and is NOT used: every Imperial unit costs wood, so reserving a building's wood freezes the whole army.
...
**Finishing an enemy that has nothing left (v0.3.200).** Two rules govern what the AI's army marches on when the enemy holds only buildings.

A target that cannot be ENGAGED is not a target. The AI prefers to hunt the target enemy's villagers, and a GARRISONED villager is alive but has no position — nothing can walk to it or hit it, and the attack order is dropped silently. The preference therefore only ever names a unit standing on the map. Measured on `gold-rush`: one hidden villager held an army of 151 idle for 30,000 ticks, 45 cells from an opponent with fourteen buildings and no unit on the map, and the match could not resolve. That reading was taken with the camp-anchor radius already at 20 — the two changes shipped together — so it is the symptom of an army this AI can now field, not of the one it fielded before either change.
...
**HUD chrome: the target is Definitive Edition's HUD (owner decision, 2026-09-25), and there is one HUD (the coordinator's reading).** Asked whether the Definitive Edition look target (§14.5) covers the HUD, the owner answered *"Yes include HUD"*. The coordinator's reading, recorded as the coordinator's and not as the owner's words: the HUD becomes Definitive-Edition-like, and there is ONE HUD, which does not change with the art style, so Moebius stays selectable as an art style for the game world only. This SUPERSEDES the modern direction of 2026-08-18, when the owner had asked for "a modern looking UI". The HUD's art stays original and procedural, like all art here (§2.1): DE's panels, emblems, portraits and cursors are copyrighted, so the HUD evokes them and reproduces none. The gaps and the steps that close them are H1–H7 of the de-look plan (`docs/threads/current/de-look/PLAN.md`, section 7), ranked there with the world steps: the chrome, the layout, portraits, the resource bar, type, the minimap, and cursors with in-world selection marks. Until a step lands, what it replaces keeps shipping and keeps its gates.

**The chrome that ships until H1 and H2 replace it (modern, 2026-08-18).** The major HUD container panels — the top resource bar, the command/selection panel, the minimap frame, the game menu, dialogs, tooltips, and the replay timeline — present as dark-glass surfaces: a translucent slate face over a background blur, ONE hairline border, a bright inset rim along the top edge, and a two-part elevation shadow (a tight contact shadow plus a wide soft one). The minimap is the deliberate inverse — a sunken well rather than a raised face — because it is a viewport into the world rather than a panel on top of it. Resource chips carry no boxes of their own — spacing and the label/value type contrast separate them, and a hover surface marks them as interactive. The single warm accent is reserved for interactive state (focus rings) and for production progress, the one continuously-changing quantity on the panel. A viewer who prefers reduced transparency gets opaque panels with no blur, which doubles as the low-power path. Still 100% procedural CSS: no image assets and no reproduced Age of Empires art. This chrome replaced a framed wood-plank/chiselled-stone treatment (2026-06-16) whose beveled edges and grain read as a plaid moire at the sizes the HUD actually renders, so a textured DE-like panel is judged at those sizes before it ships.

**What holds for any chrome.** Every colour comes from the token palette declared in `styles.css:root` (`--hud-surface*`, `--hud-hairline*`, `--hud-text*`, `--hud-accent`, one hue per resource, and the radius and elevation scales); no HUD rule may reintroduce a raw colour literal. Text contrast is held against the worst realistic backdrop: primary text at 4.5:1 and labels at 3:1, enforced by `tests/ui/hudPalette.test.ts`, which composites today's translucent panel over sunlit grass. Focus is a real 2px outline (never a box-shadow ring) so it is both visible and assertable. The command card inside the chrome is a grid of square icon tiles sized from one token pair (`--hud-command-tile`, `--hud-command-gap`), which is what lets a narrow window shrink the whole deck at once; its pages, its tiles and the rule that it is never clipped are §14.2's. `hudPalette.test.ts` also fails on the 2026-06-16 wood-and-stone colour literals and requires the reduced-transparency fallback. Both checks guard the modern chrome, and the step that replaces it (H1) revises them, not an earlier change.

**Opaque HUD surfaces swallow the pointer (v0.3.209, 2026-09-05).** A HUD surface that paints an opaque background swallows the pointer: a click anywhere on it is consumed by the HUD and never reaches the world beneath, while the transparent regions of the HUD grid pass every click through to the map. The two halves are equally load-bearing. `#hud-root` stays `pointer-events: none` so that the empty parts of the HUD's layout grid — the space beside a bar narrower than its row, the gaps between panels — remain live map: a bar that is 330px wide in a 1,256px row must not claim the other 926px. What takes the pointer is the PAINT, so the gate lives on the same rule that sets the surface's `background-color` (`.hud-bar, .hud-panel, .hud-footer` in `hudChrome.css`) rather than on the elements that lay the grid out or on a list of interactive children by name — a painted surface can then never be added without it, and growing what a panel paints cannot silently grow a hole through it. Interactive children inherit the gate instead of opting back in one at a time. The one deliberate exception is the cursor-following tooltip (`#hud-tooltip`), which stays click-through because it chases the pointer and would otherwise fight the mouse it is following. Gated by `tests/browser/hud-opaque-surfaces-swallow-clicks.spec.ts`, which enumerates the painted surfaces at run time by measured opacity rather than by selector, holds every point inside each one against the browser's own hit test, and carries the opposite control — a click on open world and one on the transparent part of the bottom row must both still reach the map, so an over-fix that claimed the whole grid row goes red too.
...
**Top panel stability + game menu (v0.1.95; dedicated action icons v0.3.8).** The top resource bar's button position is FIXED and never shifts for any reason. The resource/status chips (food, wood, gold, stone, age, pop, time, plus a Wonder/Relic countdown slot) live in an independently-scrolling region, and a single ☰ menu button is pinned to the right of the bar OUTSIDE that region — so more digits in a resource value, a longer Age label, an appearing countdown, or a narrow window can never move the button. The countdown chip's slot is permanently reserved (always in the DOM; its content fades via an active/inactive flag) rather than being inserted/removed, so it cannot reflow the bar either. Save / Load / Replay are no longer on the bar: they live in a game menu opened by the ☰ button OR the Esc key. The menu overlays the match and contains Resume, Save game, Load game, Watch a replay…, Restart match (restarts the current scenario), Quit to title (returns to the match setup screen — quitting strips the URL params, and a bare URL is the setup screen's trigger), and a Settings section (currently a debug-overlay cycle). Resume, Save, Load, Replay, Restart, Quit, and Debug overlay each use a distinct ORIGINAL procedural inline-SVG icon as the visible menu-item affordance instead of an action-name label; the Menu and Settings headings plus the live debug mode remain visible text. The native buttons preserve the complete action names, hover/focus tooltips, keyboard order, focus ring, minimum 44-pixel targets, existing hooks, and all behavior, and active menu tooltips must stack above and avoid covering the modal panel. Keyboard focus wraps within the open modal; closing the nested Load surface by Cancel or successful Restore returns focus to the Load action so subsequent navigation remains inside the menu. Esc yields to whatever is ON TOP, by the ordered layer stack in the rule at the end of this section (v0.3.223) — a native modal dialog, then a reference panel, then the game menu, then an armed building placement, then replay mode — so it opens the menu only when nothing above the menu is up, and in replay mode with nothing else up it EXITS replay. Opening the menu PAUSES the match and closing it (Resume, the backdrop, or Esc) resumes — but the prior pause state is preserved: if the player had already paused the game before opening the menu, closing it leaves the game paused rather than resuming it (implemented v0.1.97; auto-pause was deferred in v0.1.95). Styling is original/procedural (no image assets or external fonts).

**A match that cannot continue SAYS SO (v0.3.211, 2026-09-06).** Paused, finished and broken are three different states, and the player must be able to tell them apart without reading a console. A paused match says so in the menu and is the player's to resume; a finished one names a winner on the post-game card; a match that has STOPPED — one that no action of the player's can restart — must announce itself the moment it stops, name the cause, and offer an action. There is more than one way to stop, and every one of them is covered by this rule, not just the one that was found: a throw inside the browser frame loop, and an engine tick failure the simulation catches for itself, both reach the same surface. Silence is the defect. Until this rule, a stopped match was pixel-identical to a running one — the HUD went on drawing resources, age, population, the minimap and a running match timer over a world that had been dead for minutes, and the only exit was reloading the page (defect register, 2026-09-06). The notice must therefore state that the match cannot continue, say in one line what failed (plus the tick, phase and code where the engine supplies them), tell the player that nothing they did caused it, and carry a control that gets them out — a Reload. It is deliberately a FULL-SCREEN MODAL, on the same precedent as the game menu (§14.1) and the technology tree (§14.4), and the local rule that nothing may float over the minimap does not apply to it: that rule governs chrome laid over a LIVE game, where covering the minimap costs the player information they are still using, and this game has stopped, so there is no live information left to cover. The first cause wins and is never overwritten by a later failure, so the player reads the thing that actually broke. Simulation state is NOT stepped after the stop — a world left part-way through a turn is not safe to advance — while the camera and renderer keep running, so the frozen match stays lookable underneath the notice. This adds a report and changes no rule: nothing about pausing, resolution, saves or replays moves, and a healthy match must show no trace of it (the notice's own dim and blur over a running game is exactly the regression this shipped with and caught, at 99.98% of the frame).

**A finished match NAMES the outcome and the winner (v0.3.214, 2026-09-06).** How a match was decided and which way it went are two different facts, and the player is owed both. A finished match must state, in words the player reads rather than in a field the code holds, whether they WON, LOST or DREW; how it was decided (conquest, wonder, relic, score); and who won, wherever that is knowable. The outcome word leads — it is the one thing the player scans for — and every label below it is formed with the outcome in hand, so a win and a loss can never print the same string. Until this rule the HUD built its only end-of-match label out of the win condition alone, so a player who had just lost by conquest read "Conquest Victory" in 13px translucent grey in the bottom-left corner (defect register, 2026-09-06). The announcement is a dimmed FULL-SCREEN MODAL on the same precedent as the engine-halt notice above: a stopped match and a finished one are the same kind of event — the world will not change again, and the player must be told where they stand and given one thing to do about it — so it takes that surface's shape (one panel, one control, focused on open) rather than a third modal idiom, and the rule that nothing floats over the minimap does not apply to it for the same reason it does not apply to the halt notice. The post-game footer card remains as the RECORD that stays after the overlay is dismissed, and it carries the same corrected words. **The winner is derived honestly or not at all.** `MatchState` records the outcome from the human player's point of view and stores no winner, so the winner is named only where naming it is sound: a victory is the player's own; a draw has no winner and says so rather than promoting the top score; a SCORE defeat names the top scorer, because "highest score wins" is that condition's own rule rather than an inference; a conquest, wonder or relic defeat against a single opponent names that opponent, because nobody else can have won. A conquest defeat with several opponents names the winning SIDE and no individual. Score must never stand in for the winner generally — a conquest is decided by what is left standing, and the player can lose it while holding the higher score (the shipped conquest fixtures do exactly that, 50 against 10). The open gap is recorded rather than guessed around: `MatchState` has no `winner` field, and until it does, the multi-opponent conquest case is the one place this announcement is vaguer than DE. A healthy running match must show no trace of the overlay — the announcement surface is `[hidden]` with the compound `display: none` rule the halt notice already pays for, since an author `display` outranks the user agent's.

**Escape belongs to whatever is ON TOP (v0.3.223).** The dismissable layers are an ordered stack — a native modal dialog, then a reference panel (technology tree, civilizations compendium), then the game menu, then an armed building placement, then replay mode — and Escape dismisses the first one that is up; with none up it opens the game menu. A panel that opens OVER the menu therefore takes the key before the menu does, and the menu cannot close (and the clock cannot resume) underneath a panel the player can still see.
...
**Readable command deck and build palette (v0.3.7).** Populated commands are grouped under visible semantic headings such as Orders, Train, Trade, Research, and Build instead of forming one undifferentiated strip; empty groups are omitted without changing the authoritative option order or availability supplied by the simulation bridge. A villager's build palette is DE's TWO-PAGE command card (v0.3.189): exactly two pages, **Economic Buildings** (House, Mill, Farm, Lumber Camp, Mining Camp, Market, Dock, Fish Trap, Town Center, Wonder) and **Military Buildings** (Barracks, Archery Range, Stable, Siege Workshop, Blacksmith, Monastery, University, Castle, Outpost, Watch Tower, Bombard Tower, Palisade Wall, Stone Wall, Palisade Gate, Stone Gate), with a toggle in the Build heading between them. Every buildable building type belongs to exactly one page — a building on neither page is a building no player can place, so the split is exhaustive by construction and gated over the union type. One page is on screen at a time; the shorter page is padded with empty slots so both pages fill the same rows and neither the palette nor the toggle above it moves when the player switches. The toggle LEADS the Build heading row, so the title and count stepping aside for the placement pill cannot move it either. A placement in progress SEEDS the page — the pill names a card, and that card has to be under it — but a page the player picks afterwards outranks it, because every build click enters placement and a toggle placement always won would be a dead control in the commonest state. The chosen page resets when the selection changes, so a new unit's palette opens on its own first page. Build cards are square ICON TILES on a fixed five-column grid, as DE's are: the original procedural glyph carries the identity, and no two buildable buildings may share a glyph — with no visible name on the card, two buildings drawn the same are two buttons the player cannot tell apart, the price and the build description are in the tooltip and the accessible name, and an unaffordable card is dimmed rather than annotated. Their name, Ready/Short readiness and per-resource cost chips remain in the DOM for assistive technology and for panel readers. Short cards remain actionable: selecting one still enters the existing placement flow, whose authoritative validator owns the eventual affordability and obstruction result and its player-facing rejection feedback. Active placement highlights the matching card and names the building plus the next clear-tile action. The Orders, Stance and Formation groups are square ICON TILES too (v0.3.189) — a compact grid with the meaning in the tooltip, the DE shape — each with its own hand-authored 24x24 `currentColor` glyph, an `aria-label`, and its text label kept in the DOM but not drawn. Train, Research, Trade and Tribute keep visible text, because their options are told apart by their NAMES and would be indistinguishable under one shared glyph. Every command is a native button with keyboard focus treatment and the same delegated tooltip on pointer hover or keyboard focus; the active trigger carries the shared tooltip as an accessible description, mixed pointer/focus ownership keeps it open until both release it, and replacing a trigger clears stale tooltip state. Resource-only updates refresh affordability without dropping logical command focus or recreating cards whose cost coverage did not change. **The command bar never hides what it holds (v0.3.189, play-test finding F5).** The smallest window the HUD is held to is 800x600. Two rules, and the second is the one with no exceptions. (a) The bar NEVER runs off sideways, at any window size: its content never exceeds its own scroll width, because below 1120px there is no horizontal scroll at all and a group that ran off the side would be unreachable by any means. Every group is therefore shrinkable and its buttons wrap inside it. (b) Anything the bar cannot show is REACHABLE: a wheel over the deck scrolls the bar, and scrolling it brings the whole card into view. On top of those, the common cards FIT outright — a villager's palette on both pages, a military unit's stance and formation, a Town Centre, a Blacksmith, a Monastery — at 800x600 as well as above it, with every command group, every button, the build page toggle and every selection stat inside the panel's visible box. What pays for that is the bar's own geometry rather than a scroll region: the tiles shrink below 1120px, the selection summary lays its portrait, name and stat chips ACROSS the bar instead of down a column, the bar is only as wide as what it holds, and its fixed height (the next rule) is enough for the content. The biggest late-game cards — an Imperial Castle's or a Market's, whose research and tribute rows a boot scenario cannot reach — still exceed an 800x600 bar and are held to rule (b) alone. DE never clips the command card, and the version this rule replaces did: the whole BUILD group sat at y 647-767 below a panel clipped at 588 with `pointer-events: none` over it, so no wheel could reach it, and it was reachable in the test suite only because Playwright scrolls hidden overflow. The bar creates no document-level horizontal overflow, must not overlap the minimap, and must not reach the CENTRE of the canvas: every "go to X" affordance — the Town Centre key, the idle bell, a minimap click — puts what it found in the middle of the screen, and a bar tall enough to cover that point swallows the thing the player just asked to see (measured: a 276px ceiling at 800x600 put the bar's top edge at y 312 and hid a Dock's newly trained Transport Ship at y 319). When the replay timeline is visible, its reserved height also constrains the command panel so the panel stays between the top resource bar and timeline at both desktop and narrow breakpoints. **The command bar keeps ONE height, whatever is selected (v0.3.207, 2026-09-05, owner: "the info panel at the bottom should not have different heights for different units. This is visually confusing when switching between units").** The bar is a fixed-height BAND, as DE's is, not a box as tall as what it holds. Its height is one layout token (`--hud-selection-bar-height`), with one value per viewport regime — the unwrapped bar above 1120px and the wrapped bar at 1120px and below — and nothing about the selection changes it: nothing selected, a villager on either build page or placing a building, a military unit, a Town Centre with or without a production queue, a foundation, a sheep, a tree, a berry bush, a mine, every villager, a mixed box selection — same height, same top edge. Everything that varies (the portrait, the stat chips, the command groups, the queue, the placement pill) lives INSIDE the band, packed to its top so the heading, the portrait and the first row of tiles sit at the same place for every selection, and content the band cannot hold scrolls inside it under rule (b) above. The token is sized to the tallest card the boot scenario shows plus 2px — a Town Centre's above 1120px (its name wraps to two lines beside its portrait and its seven stat chips take four rows: 242.8px, 244 with three queued entries, so 246px), every villager selected or a queued Town Centre below (232-234px, so 236px) — so none of the common cards scroll; the bar stands 39px taller than the minimap panel above 1120px; the viewport ceilings above remain the screen's own limit and bite only on a window shorter than the band needs. Before this the bar was content-sized and bottom-anchored, so its top edge moved on every click (measured at 1280x720 on 2026-09-05: 72px with nothing selected, 185.6 with a villager, 242.8 with the Town Centre), which is the cure the 2026-09-02 idle-bell register entry predicted. Gated by `tests/browser/selection-panel-height.spec.ts`, which cycles those selections at 800x600, 1024x768, 1280x720 and 1920x1080 and holds the bar's height and top edge equal across them, each state inside the band without a vertical scroll, with one named exception: at 800x600 the wrapped bar puts the production queue on a line of its own, so a Town Centre with one villager queued (261px) scrolls inside the band under rule (b), as it did under the old ceiling. A multi-kind selection's compact chip grid has a definite width (three chips per row), because an auto-fill grid in an auto-width column collapses to one column and stacked 269px tall. **Entering placement mode does not reshape the bar (v0.3.179, owner: "the UI needs to be elegant").** The status — `Placing: <Building>` plus the `Choose a clear map tile` hint — is a pill inside the Build group's heading, never a sibling section of the bar: the palette keeps its width (a check holds its visible width at 90% or more of its pre-placement width, at the wrapped and unwrapped viewport regimes), the stance and formation groups stay where they were, and the heading's height does not change. Where the group is 380px or wider the pill sits between the heading's title and its count; narrower than that (a 1280px window gives the group 261px) the title and count step aside and the pill is the heading, so the hint is shown whole or not at all — never cut mid-word — and the heading never runs past its group. **Command-bar tooltips float above the bar**, not above the card they describe: a tooltip for anything in the command bar never covers a card, a heading, or the status, which is what a tooltip centred above a first-row card did. **No tooltip may cover the minimap frame**, in placement mode or out of it: the shared tooltip positioner slides a tooltip left of the minimap panel, or above it when there is no room beside it, and the browser suite hovers every build card before and during placement at three viewports to hold that. **A tooltip belongs to the cursor, not to the focus a click left (v0.3.189, play-test finding F6).** A control the pointer PRESSED does not own a tooltip: a clicked build card keeps keyboard focus, so the player can act on it again from the keyboard, but its tooltip leaves with the cursor, as DE's does. A control focused from the KEYBOARD still owns one, and keeps it across the panel's re-render. The claim is held against the control's identity rather than its element (the panel re-renders after a click and re-focuses a NEW button with the same `data-command`) and is released when focus reaches a DIFFERENT control — never by a keystroke on its own, because any key would then do it and the panel's own re-render would hand the tooltip straight back. Touch is exempt: a finger has no hover, so a tap is the only way a touch user can ask what a tile is. This resolves the open question the previous revision recorded: before it, "Place a House foundation…" stayed over the bar while the player was already out on the map choosing the tile, still showing after a twelve-step pointer sweep across the canvas. This is presentation and interaction hierarchy only: it does not create a second command authority, reorder or hide bridge-provided choices, pre-charge resources, or change simulation validation.

**An unavailable command explains itself (2026-09-06).** Every command the command card DRAWS but the player cannot use right now carries one short sentence naming what is missing, and it reaches the player BEFORE the click — in the control's tooltip and its accessible name — not only afterwards as a rejection toast. A DISABLED control cannot be clicked at all, so a refusal it cannot explain has no channel: that is how the Town Center's greyed "Research Feudal Age" came to say only "Cost: 500 food; takes 130s." to a player holding 1,295 food, who then sat in the Dark Age for 38 minutes. The sentence is the SIMULATION's — computed in `bridge/commandAvailability.ts` beside the option lists and carried to the HUD in `SelectionState.unavailableCommands` — so a tooltip can never disagree with the validator that would reject the same click, and the HUD routes each reason to the control whose `data-command` matches and adds nothing of its own. It names the missing thing: the buildings an age-up wants and how many are done ("Needs 2 completed Dark Age buildings (Mill, Lumber Camp, Mining Camp, Barracks, or Dock). You have 1."), the research a technology waits on, the resources a price is short of ("Not enough wood and stone.") — never a bare "unavailable", and never the resource the player already has. A structural rule outranks a price, as the validators' own order does, so an age the player cannot yet reach is never reported as merely expensive. Affordability names the RESOURCE and not the shortfall figure: the card re-renders off a signature over the whole selection state, so a live number would redraw the bar on every gather tick, and the amount is already in the same tooltip in the cost the button quotes. This adds to what a control says and changes nothing about what it does — §7.2 still decides what may be researched, §14.2 still keeps a Short build card dimmed-but-actionable, and no button's enabled state moved.

**No command button moves because the simulation did something (v0.3.223, 2026-09-11, found by the standing loop).** A player repeating a click at one screen position must keep hitting the same command. Nothing that appears, fills or grows on a simulation event may sit AHEAD of the command deck in the bar, and the deck's own used width may not change when it does: the production queue is drawn after the deck and is the single member of the bar that absorbs a width deficit, so a bar too narrow for everything clips the queue rather than moving a button. A destructive order — Ring Town Bell, Ungarrison, Delete — must therefore never come to rest in a cell a production button just vacated, because no cell is ever vacated. The queue is bounded to the band and scrolls inside it rather than drawing past its bottom edge. Before this the first "Training: Villager" card slid the deck 140px right, Ring Town Bell landed where Train Villager had been, and the second and third clicks of an ordinary three-villager queue garrisoned the player's own villagers.

**A placement in progress can always be put away (v0.3.223).** Escape cancels it, and a second click on the build card that armed it cancels it too; both leave the builders selected with their palette, so the player can arm something else without re-selecting. The foundation ghost follows the POINTER and exists only where the pointer is: there is no ghost before the pointer has been over the world, and none when it is off the map — a click off the map spends nothing, so the preview and the confirm always read one verdict. A left drag during placement remains a placement, as it is in DE.

### 14.3 Minimap, Alerts, and Camera

Required behavior:

- minimap with terrain, visibility, viewport, and high-contrast entity marker representation. The minimap is projected as an ISOMETRIC DIAMOND matching the world camera angle (the same 2:1 `cellX-cellY` / `cellX+cellY` transform the game view uses, scaled to fit and centred in the minimap canvas) rather than a top-down rectangle: terrain and fog tile the diamond, entity markers are fixed-size dark-backed dots at their projected cell centres, and the camera viewport draws as a projected quad. Click/drag-to-pan inverts the diamond projection to a cell and ignores clicks that fall outside the map diamond (the canvas corners are off-map). Shipped v0.1.130. Left click and drag pan the camera. RIGHT click issues the selection's context order at that cell — the same meaning a right click on the world has, decided by the simulation — and does not pan (v0.3.223). Both buttons go through the same diamond inversion, so neither can act on a cell the other would reject, and the minimap suppresses the browser's own context menu.
- attack alerts: a horn, the words "You are under attack!", and a minimap mark that holds on simulation time and is sized in screen pixels, with Space jumping the camera to the newest hit — the full rule is the attack warning at the end of §14.5's audio-cues bullet (v0.3.229)
...
- isometric voxel art direction: the sole renderer uses an original procedural Age-of-Empires-style vocabulary rather than generic whole-object blocks. Buildings combine neutral stone/plaster/timber walls, stepped voxel roofs, doors, windows, beams, towers, merlons, banners, awnings, blades, crop rows, and construction scaffolds. Player color appears as readable accents (flags, trim bands) AND as an owner-colour blend carried by the stepped roofs themselves (v0.3.83: each roof layer mixes 55% toward the owner tint before its per-layer shading, so a skyline reads as its owner's colour at a glance — the AoE2 rule that three bases are three colours of rooftop — while walls stay neutral stone/plaster and the material still reads as thatch/tile). Units share seven coherent body families but all 34 concrete types choose an exhaustive armor, headgear, shield, mount, weapon, tier, and signature-detail profile; the matching attack rig transforms the equipment that is actually visible. Resources use clustered trees, faceted mineral piles, forage berries, distinct animals, and relic ornament; sparse deterministic terrain props add surface detail without changing simulation height or picking. Parts are bounded rigid instances grouped into matte, metal, UI-feedback, cast-shadow, and reduced-opacity memory materials. The shared `voxel` package may own configurable target-tracked daylight, but AoE owns every palette, recipe, role, fog cue, and animation meaning. The current path stays elevation-zero; true shadow maps, AO, skeletal animation, deeper per-civilization facade GEOMETRY, and raised-geometry interaction require separate measured gates (the per-civilization roof-material pass shipped v0.3.105 — see the architecture-sets rule below; the geometric sun-cast shadows shipped v0.3.193 — next bullet).
- sun-cast ground shadows (owner steering 2026-09-02, "the rendering needs to feel realistic"; shipped v0.3.193. Reshaped v0.3.194 on owner steering 2026-09-03: "the shadow from units, and buildings, and environment objects such as trees and bushes and ores, they all need to look more realistic ... they need to roughly match the shape of the thing that created the shadow instead of always being a rectangular", with the standing requirement that "whenever the shape of the object changes, the shadow also needs to be re-adjusted in the future"): every live building, resource and unit whose recipe has a solid part casts a shadow on the ground (the unit layer's malformed-projection fallback, which has none, keeps a flat footprint patch), projected from the SAME sun the daylight rig lights the scene with -- `aoeVoxelDaylight.ts` is the one source for both, so a point at height h lands h*22/36 toward +x and h*18/36 toward +z, away from the sun at {-22, 36, -18}. THE SHADOW IS THE CASTER'S SILHOUETTE, DERIVED FROM THE CASTER'S OWN PARTS AND NEVER AUTHORED BESIDE THEM. `aoeVoxelShadowShape.ts` reads the solid parts as a vertical PROFILE -- the footprint the mass occupies at each height, as at most four bands from the ground up, merged where consecutive slices share a footprint so a plain box costs one band -- and the shadow is the union of those bands' swept footprints. So a tree narrows to its trunk near the ground and tapers to its crown-top far along the sun ray, a house throws walls under a wider roof, a Town Center's tower streaks past the roof it stands on, and a felled animal's shadow is the shape of the animal lying down. Nothing in the shadow code names a recipe, a part or an entity type, which is what satisfies the standing requirement: a recipe that changes shape moves its own shadow on the next frame with no shadow-side edit, and `tests/rendering/aoeVoxelCastShadows.test.ts` gates exactly that by changing a caster's height and width and measuring the reach. The union is tiled with TRIANGLES on a dedicated flat geometry (`aoeVoxelShadowWedge.ts`, the shadow lane's own `geometryKey`), because a caster's silhouette is a polygon and a box instance can only ever be a parallelogram -- which is why every shadow used to be the same rectangle-derived hexagon. The tiling is a strip decomposition in the sun's own frame: the region between the lower envelope of the bands' near boundaries and the upper envelope of their far ones, sliced at every envelope breakpoint, two triangles per trapezoid, merged wherever a straight run stays within 0.03 world units of the exact outline. The pieces of ONE caster never overlap, so a caster never darkens its own shadow twice, and the tolerance is RELAXED until the tiling fits ten strips, so a silhouette cannot become the frame's instance budget however many parts a recipe grows (measured mean 3.0 -> 15.7 instances per caster across thirteen archetypes, and 862 -> 1188 instances for the whole boot map, with the batch count unchanged at nine). The geometry is FLAT, with no thickness: the first version was a prism like the slab it replaced, and two neighbouring pieces then stood their 0.036-tall side walls back to back along every shared edge, blending both and drawing the tiling as a mesh of dark seam lines across every shadow in the scene. A CASTER'S OWN BASE IS A SHADOW RECEIVER, not only a caster. Almost every building sits on a plinth wider than its walls and the sun is 52 degrees up, so the whole of a building's shadow lands inside its own plinth -- a ground-only shadow left the largest objects in the game looking unplanted (measured on the Town Center: 0.82 tiles of shadow reached past a 3.44 tile plinth and the rest fell where nothing drew it). A caster whose lowest band is a PAD -- no taller than 0.45, at least 1.15x the plan area of the mass on it, and FLAT, meaning the widest part contributing to it ends at the pad's top rather than continuing up through it -- draws a second layer: the shadow of the bands above, projected onto the pad's top plane and clipped to the pad. Flatness is what separates a plinth from a heap. A gold mine's lowest band is also its widest, but the rock that makes it wide keeps going up, so there is no exposed surface there and a patch drawn at that height would hang inside the rock. The ground layer under the pad is hidden by the opaque pad itself, so the two layers never blend one pixel twice. Shadows of different entities never draw at the same height, because two coincident translucent pieces z-fight and tear into stripes: each caster's pieces sit at one height (a pad layer keeps its fixed offset and shifts with them), 0.002 world units separate adjacent heights (the depth-buffer floor for the 4000-unit orthographic range on a 24-bit buffer), and the shadow lane is drawn top-down, so where two overlap the lower piece fails the depth test -- one blend per pixel, no tearing and no darker patches along a forest edge. A caster's height starts from its own cell (x mod 3, z mod 3, nine levels), which is a good colouring -- a dense woodline is a nine-clique of overlapping tree shadows and the cell rule colours it exactly, where greedy colouring from level zero does not -- and a whole-lane pass run per presented frame then steps up any caster whose height a caster it overlaps already holds. That repair is what the cell rule cannot supply: two casters in the SAME cell seed identically, and units share cells routinely (a 12,000-tick AI-vs-AI run of the boot map reached four villagers on one tile and thirteen same-height overlapping slab pairs at once); a large building's shadow also outreaches three cells, so a Town Center and a House three cells away share a seed and overlap. The overlap test is on each piece's own TRIANGLE, not on the parallelogram enclosing it: the enclosing box is half again the area and reports pairs that never share a pixel, which lifted casters that did not need it -- a 6x6 woodline measured 0.084 world units of lift that way against 0.038 from the triangles. The repair search is UNBOUNDED, because a capped one must fall back on a height that is taken by construction -- a nine-step cap measured 38 coincident overlapping slab pairs on a 6x6 woodline with twelve villagers gathering in it. A clique of K mutually overlapping casters therefore needs K heights and rises ABOVE the nine-level band: three to five pixels of screen offset at the closest camera zoom for a dense crowd. Nothing bounds that but how tightly the simulation can pack casters, and it is the accepted price of never tearing. Two consequences are accepted and unfixed: a caster's height changes as the crowd around it moves and as it crosses a cell boundary (18.9% of caster-frames in a walking group of 24, up to three pixels), and the 0.002 separation assumes a 24-bit depth buffer that nothing in the renderer pins. Mass BELOW the ground plane casts nothing, so a felled carcass does not sweep the mass it has rotated underground. `water`-surfaced parts never enter a profile, so a ship's wake cannot flatten its shadow across the surface it floats on. The shadow does NOT turn with a unit's heading and is cast from the rest pose, so a swinging arm cannot make it flicker; memory (fogged) entities cast nothing. The Moebius resolve quantises the 22% darkening into a full tone band, so a shadow reads darker on screen than its opacity alone implies -- the Moebius style's property, shared with every other shading gradient; the DE style (v0.3.227) draws the 22% as it is. Render-only: picking, silhouettes, occupancy, saves and replays are unchanged, proved by a digest over every unit, resource and building recipe's NON-shadow parts being byte-identical across the reshape (2326 parts, `e2f40e86a2352cb74c7c071ab2c1e5b4f3f31a5b3d64463b4dd0f316a0471c3a`). Real shadow maps and AO stay deferred (previous bullet); the daylight rig is backlit relative to the fixed camera (both visible wall faces receive fill light only), and moving the sun to light one visible wall is the next measured candidate -- see `docs/threads/done/render-realism-2026-09-02/DESIGN.md`.
- animated voxel units: live non-memory units animate their existing rigid procedural parts without changing authoritative simulation state, damage, reloads, pathing, roots, selection, fog, health bars, or commands. Identity-phased ambient profiles provide subtle breathing/bobbing plus role-readable secondary motion: opposing humanoid limbs, horse legs/tail and rider bob, monk sleeves/staff, and siege mechanisms. Perspective-eligible successful unit hits additionally project one bounded latest impact event per attacker entity reference. Before each successful impact, AoE proves visibility current: the first impact each tick and every later impact after a player-command LOS mutation synchronize sources, while an unchanged same-tick burst reuses that snapshot; fingerprinting avoids unnecessary `VisibilityMap` recomputation. A player, including the attacker's owner, becomes a witness only when at least one cell of the attacker footprint and at least one cell of the target footprint are visible to that perspective at that impact. If no perspective qualifies, no event or coordinates are recorded, so a retained ranged order cannot publish a fog-hidden target coordinate. At the authoritative damage tick attack sampling starts at the authored target-facing impact keyframe and then recovers smoothly; several same-tick attackers animate independently. Coincident presented roots remain a valid strike even though they have no geometric target direction: a warm renderer preserves its last finite displayed facing and a fresh renderer uses the role's authored forward instead of dropping the pose or ambient suppression. The first actual root change records a cancellation tick: movement direction becomes authoritative immediately while attack and ambient suppression weights smoothstep to zero across that one observable tick, so the actor neither snaps nor resurrects the strike after returning to its source. Presented roots and the attack channel (phase, weight, and ambient suppression) remain deterministic and continuous across that boundary; warm and fresh gait history is deliberately not an equivalence contract. Every concrete unit weapon uses an AoE-owned pose style whose controlled rigid pieces share a pivot and remain attached; recipes are deterministic, contain at most 32 parts, keep feet, mounts, wheels, roots, and shadows planted during attack, and reuse the exact prepared parts for rendered matrices and presented silhouette picking. AoE advances one monotonic Voxel animation clock only by positive simulation-display deltas. Equal display time freezes gait, ambient breathing, secondary motion, attack geometry, and matching hit geometry; rewind and bridge replacement rebase without sending time backward to Voxel, while disappearance, memory projection, and entity-generation replacement reset disposable unit history. Raw projection retains live entities so stationary fog reveals remain possible, but the bridge applies the final current-perspective filter; the prior-tick position frame it once captured through its visible-cell frame is gone (v0.3.192), and the displayed-root smoother keeps its own per-unit history under the same rule — a unit absent from the previously presented tick starts fresh at its sim position rather than gliding from where it was last seen. When a witnessed attacker leaves one perspective's current view, the bounded feed persists that perspective in `suppressedFor` before recorder checkpoint publication; tower targeting uses one immutable pass-start visibility snapshot, then a successful tower pass with one or more kills resynchronizes final LOS exactly once before this suppression/output step. That event stays hidden after re-reveal, while `RenderStore` keeps a renderer-local tombstone as defense in depth. Ordinary saves drop in-flight attack presentation, while new recording snapshots retain a bounded, canonical feed so replay scrubs restore the event, per-perspective suppression, exact source/target coordinates, attack phase, weight, and cancellation state without making it authoritative gameplay state; coincident coordinates use the same deterministic displayed-facing fallback; playback's first frame only establishes the wall-clock baseline and does not synthesize a tick. A fresh renderer may reinitialize disposable gait history, so full assembled rig matrices are not a replay contract. Legacy recordings that lack the slot preserve that absence, and historical checkpoints whose feed predates `suppressedFor` cannot retroactively reconstruct a hide/reveal that occurred before that checkpoint. Monks remain non-attacking: validation and the direct issue helper reject new attacks, and the execution boundary clears hydrated or persisted Monk attacks before damage or event emission. Wildlife retaliation animation shipped in v0.2.8; gathering clips, projectiles, skeletal animation, clip assets, and root motion remain deferred. The sibling renderer may own only game-neutral bounded injected-time rigid transforms; AoE owns every role, event, part mapping, pivot, amplitude, period, phase relationship, clock policy, and gameplay-to-presentation transition.
- speed-matched voxel locomotion: AoE samples the already-interpolated displayed `x/y` path per `id:generation`, advances a wrapped gait phase by displayed distance divided by an AoE-owned role stride, and bakes foot lift/travel, opposing limb pose, cavalry stride, monk sway, and siege wheel rotation into ordinary rigid-part base transforms. The injected simulation display clock determines displayed speed and a bounded start/stop/turn blend only, never advances gait phase, and freezes identically during manual or replay pause whether selection forces redraws or not. Direction-aligned pitch and transformed-corner clearance keep feet flexing in the travel plane without penetrating ground. Disappearance, memory projection, generation replacement, bridge replacement, and clock rewind start fresh unit history; the renderer-level monotonic accumulator retains its value and rebases the next simulation sample. Static and animated surface lanes remain separate so large static scenery does not inherit the active-batch slot ceiling. The shared renderer contract does not gain unit roles, velocity inference, stride policy, foot planting, clock policy, or mutable gait history; its harmonic lane consumes AoE's injected monotonic time and therefore freezes with the game. The start/stop blend reaches full gait at three quarters of the unit's own base movement rate (§12.4.2), so a unit walking normally plays the authored stride and a unit slowed by traffic fades toward rest; movement technologies only clamp at full (2026-09-05, v0.3.206: the blend had measured against a flat 2.5 tiles/s no unit reaches since v0.3.160).
- unit motion continuity and facing: every fine-grid movement step must publish a new `unitTransform` through the ECS component API so render projection observes each fixed-tick endpoint; mutating a retrieved component object in place is forbidden. A stationary spawn begins at its engine-allocated occupancy slot, and a commanded unit must aim at and test arrival against that same assigned slot, so neither its first cardinal step nor its final stationary sync can recenter or teleport it to a different sub-cell point. The assigned slot or explicit no-slot overflow state must be persisted separately from a still-moving fine root; save/load and replay rebuild numeric assignments before overflow units, while legacy saves without slot data use their serialized fine root as a one-time preference. An explicit overflow is reconstructed without opportunistically claiming a slot that uninterrupted play had not claimed; when gameplay later finds a same-cell slot, the command remains active until the fine root reaches it by ordinary bounded steps. Presentation draws each live unit's root from the simulation's OWN sampled trajectory replayed a step cadence behind (v0.3.192, after the owner saw units "move and briefly stop mid-movement between cells" on 2026-09-01; corrected v0.3.192, below): a sample is taken only when the sim position changes, the root is interpolated between the two samples bracketing `now − delay`, and the delay is the unit's longest unobstructed fine-step interval at its BASE rate (`ceil(100 / round(0.32 × basePercent))` ticks — a villager's four, a scout's three, a Battering Ram's five; a sheep uses its six-tick herd throttle; a fleeing deer its fourteen-tick hop clock, so a whole-cell hop is drawn as a 1.4 s glide at its 0.71 tiles/s rather than a cell crossed in one tick — measured on `deer-flight-fixture`, the share of frames inside a hop that drew the deer standing fell from 91.1% at a one-tick delay to 0%; charging wildlife, which the sim moves a whole cell every tick along its range plan, one tick), so the render time always sits inside a segment whose far end is known and a unit walking unobstructed on open ground is drawn advancing on EVERY frame at that interval's average speed, never standing for the ticks between fine steps. A unit the sim stops — blocked, yielding in a contested cell, arrived — is drawn stopping where the sim stopped it, one delay later; no motion is ever invented. A unit leaving rest is drawn moving the tick its first step lands, at cadence speed; the delay is paid at arrival, where the drawn root settles `delay` ticks after the sim does. The drawn root never runs past the newest sample it has — it never extrapolates, so it can never be ahead of a sim position the sim has not reached — and every ordinary 60 Hz displayed root step stays bounded, including through a path corner. It follows the sim's own trajectory including the sim's own reversals, so when the sim steps backwards (a unit repathing to a moving target) the drawn root does too, one delay later; that is replaying the simulation, not inventing motion, and it is the same at every frame pace. A DISCONTINUITY IS A JUMP IN SPACE, NOT A JUMP IN TIME (corrected v0.3.192; until then a forward tick gap was itself treated as a discontinuity, which is the rule this replaces — the two must never both be written here). A move is a teleport, and snaps, when it is longer than the gap it crossed could have carried THAT ENTITY: its own per-tick ceiling times the ticks between the two observations, capped at the five ticks a frame can coalesce (the 250 ms visible-simulation bound times the fastest speed setting). Both halves of that are load-bearing. PER TICK, because the fastest thing in the game is not a unit — `wildlifeCombatSystem` moves a CHARGING boar or wolf a whole cell every tick along its range plan, so a two-tick frame moves one 2.83 tiles and a flat guard called that a teleport (measured on `wolf-aggro-fixture`: the drawn root covering 2.0616 tiles in one frame at 150 ms and 3.1623 at 200 ms, half its moving frames drawn standing). PER ENTITY, because an allowance loose enough for that wolf is far looser than a unit can ever use: whole-cell movers get 1.5 tiles a tick, one diagonal cell, and everything else gets 0.75. The whole-cell list is a fleeing DEER, a charging BOAR or WOLF, and a RELIC being carried by a monk — which is not obvious and is the kind of thing the list has to be enumerated from the code for: the monk carries the relic by putting it on the monk's integer CELL, so it moves a whole cell every time the monk crosses a boundary while the monk beside it glides. A SHEEP is deliberately NOT on it: it takes one quarter-tile subgrid step every sixth tick, so its ceiling is the ordinary one and only its CADENCE is its own — which is PROVED rather than observed, since `moveCarryCapHundredths` bounds the bank at `max(100 + perTick, round(perTick x 1.5))` and the fastest unit in the game (the Demolition Ship, 200% base and 260% with every movement technology, 83 hundredths a tick) can be granted at most TWO fine steps in one tick, half a tile, with nothing in the roster reaching three. A FORWARD TICK GAP GLIDES: a frame that coalesced several ticks — the live loop coalesces up to 2.5 ticks a frame, 5 at double speed, and EVERY frame of a machine drawing slower than 10 fps does — or a multi-tick test advance brackets the gap with two OBSERVED sim positions, which is the same evidence the smoother interpolates between on any other pair of samples and less extrapolation than a deer's fourteen-tick hop already asks for, so the drawn root keeps its history and keeps gliding; a gap large enough to matter resolves at the sim position on its own, because the render time then lands a hair short of the newest sample. Snapping there was this rule's first cut and it was worst exactly where the smoother is needed most: measured on `deer-flight-fixture` through the real bridge, the share of frames inside a hop that drew the deer STANDING was 0.0% at a 16.7 ms frame and at 100 ms, 14.8% at 105 ms, 92.4% at 143 ms, 93.4% at 167 ms and 100% at 200 ms, and a villager's open-corridor walk stood in 25.0 / 31.7 / 37.8 / 20.0% of mid-walk frames at 125 / 150 / 200 / 250 ms with a per-frame step ratio reaching 25.0 — all 0.0% once the gap glides. What a coarse frame grid does cost, and is accepted, is ALIASING rather than a stop: a presentation that only looks every 1.5 ticks sees a villager's 3-tick sim cadence as alternating 1- and 2-tick intervals, so the drawn speed ripples — 1.67 at 150 ms, and up to 2.50 across 110-240 ms and 3.00 at 320 ms — where a 60 Hz presentation ripples 4:3. What it also costs, and is accepted because the smoother cannot see it, is that "absent from the previously PRESENTED tick" is weaker than "absent the tick before": an entity that garrisoned and was ejected, or blinked out of fog and back, entirely inside ONE coalesced frame was never presented as absent, so it glides across that rather than snapping. The per-entity ceiling above is the only thing that bounds it, and is most of why it is worth having: a unit's exposure is 1.5 tiles across an ordinary two-tick frame and 3.75 across the worst five-tick one, where a single wildlife-sized allowance would make those 3.0 and 7.5. Closing it entirely would need the bridge to report the absence; at frame granularity the history does not exist. A REWIND (a replay scrub, a rollback) still snaps every drawn root, because the samples ahead of the new render time belong to a timeline being left; so do a bridge swap, a recycled entity id, an entity absent from the previously presented tick (a fresh sighting drawn where the sim says — the store's former visible-last-tick rule, now kept by the smoother itself), and a unit whose type changed under the same id. Gated by `tests/browser/unit-motion-smoothness.spec.ts`, and the gate is bound by the FRAME PACE it drives as much as by what it asserts, so it drives two: on the real boot map a villager's eight-tile open-corridor walk, driven at 60 Hz, may draw a root that did not move on at most 2% of mid-walk frames, with the largest per-frame step under twice the smallest (measured 67.4% → 0.0%, ratio 1.33); the SAME walk driven at 150 ms a frame — 6.7 fps, the pace of a machine several times slower than the one this was written on, and past the 100 ms line where a frame starts coalescing ticks — holds the same 2% still bar with a step ratio under 2.5, the aliasing allowance (measured 31.7% and ratio 8.00 before, 0.0% and 1.67 after); the same walk unpaused under real animation frames may spend at most 2% of the window's DURATION standing still, measured over a window of at least 2,500 ms, because a still-frame COUNT over a short live window gives a 2% bar an allowance of exactly one frame and lets a single duplicate animation-frame timestamp decide the verdict — and that live half SKIPS, naming the measured pace, when the machine's median animation frame is slower than one simulation tick, because a still frame cannot be shorter than a frame interval and a 100 ms stall on a 150 ms frame leaves no trace at all, so a pass there would mean nothing; a sync that coalesces five ticks must leave the drawn root gliding and still trailing the sim root rather than snapped onto it, with no still frame in the eight ticks after it; and a scout-lured deer's hops must each be drawn as a crossing, at most 2% still frames within the hop's fourteen ticks less a margin (13.3 ticks at 60 Hz, 10.5 at 150 ms), at BOTH paces (measured by the gate at 150 ms: 98.1% → 0.0%, and no frame drawing more than half a tile where the defect drew a whole diagonal cell, 1.3637 tiles, in one). Every one of those tests also bounds how far the DRAWN root may trail the SIM root — half a tile for a villager whose four-tick cadence is 0.32, one and a half for a deer whose fourteen-tick hop is 1.414 — because a delay that is not the entity's own passes every still-frame and step-ratio bar there is: a 30-tick deer delay draws it 2.879 tiles behind and was invisible until that bound existed. A blocked autonomous unit may recenter toward its assigned in-cell slot only by one normal fine-grid step per tick before selecting a new heading; obstacle escape must not trade a visible snap for a deterministic patrol livelock. The complete actor and presented hit silhouette turn together through the shortest arc toward displayed travel, while the sun-cast ground shadow does NOT turn with them — it falls away from the sun and is cast from the rest pose (v0.3.193; before it, the contact slab was a child of the actor and rotated with the heading); each authored role's semantic front must have a positive dot product with current displayed velocity once moving. Explicit discontinuities such as replay scrubs, bridge replacements, identity-generation changes, and authoritative teleports snap once rather than interpolating unrelated state. A long animation-frame gap during replay consumes at most the shared 250 ms visible-simulation bound in one callback instead of fast-forwarding the whole hidden interval in one rendered frame.
...
The north star is the look and polish of Age of Empires II: Definitive Edition, reached entirely through ORIGINAL/procedural art that evokes that look — no copyrighted Age of Empires assets (sprites, textures, UI art, audio) are reproduced. **Owner decision, 2026-09-23:** *"try to make it look as close to DE as possible, which preserve the other option of moebius"*. The coordinator's reading of it, recorded as the coordinator's and not as the owner's words: the visual target is Definitive Edition's look, and Moebius stays selectable as an art-style option. A DE art style ships beside Moebius since v0.3.227 and becomes the default once it has its own textured terrain (the art-style bullet below). **Owner decision, 2026-09-25:** *"Yes include HUD"*, the answer to whether this target covers the HUD. The coordinator's reading, again the coordinator's and not the owner's words: the HUD becomes Definitive-Edition-like, and there is ONE HUD; Moebius stays selectable as an art style for the game world only. §14.1 carries the HUD rule, and steps H1–H7 of the de-look plan close the gaps. Presentation targets:
...
- units behind buildings (shipped v0.2.5, superseding the v0.1.133 Phaser cue that v0.2.0 removed; the LOOK reshaped in v0.3.216 and described here — earlier descriptions of a flat opaque two-colour mirror on the unlit `ui` lane are historical): every displayed live unit whose MID-BODY screen anchor is covered by a non-memory building's projected part silhouette shows a mirror of its posed non-shadow parts, drawn TRANSLUCENT and LIT in its OWNER'S PLAYER COLOUR. The lane is `memory`, the renderer's existing see-through lane for fog ghosts — which is exactly what an occluded unit is, something you cannot presently see — so the building shows through the shape and the shape takes the same daylight the real unit takes; head, torso and arms separate instead of merging, and the cue reads as something BEHIND rather than as a decal stuck to the wall. It costs no extra instance batch (the snapshot declares nine and both the pinned ceiling and a browser spec forbid a tenth). The film is laid down TWICE: the lane's opacity is fixed at 0.5 and shared with fog ghosts, so coverage is the only dial this cue owns, and at one pass a pixel is half cue and half background — where the background is the tint's complement the two cancel, and this game pairs exactly that, a blue player against orange roofs. Measured at one pass on the showcase fixture, a friendly silhouette's head over a sunlit roof rendered (171,126,135), a grey-mauve, while its torso over the dark chimney stayed blue: one shape carrying two colours. Two coincident passes leave 75% of the tint on the pixel, put that head at (131,141,195), and still pass 25% of the roof's own light and shade. COLOURS ARE THE PLAYER'S, one per slot, in AoE2's own order (blue, red, green, yellow, cyan, purple, grey, orange; slots past eight wrap the way `ownerTint` does, and a unit with no owner is gaia and takes the not-yours colour). They are authored rather than taken from `ownerTint`, because that helper rotates hue while PRESERVING LUMINANCE — right for a material shade, wrong for a saturated cue: the yellow lands within 70 of `roofTile` and the grey within 33 of `stoneDark`, invisible on a stone wall. Each authored colour keeps its slot's AoE2 hue within 10 degrees and carries its own brightness, lightened until it clears every building material at a floor of 90; two slots cannot clear that floor and the gate NAMES them rather than hiding them, because this world's stone and plaster are greys and a sunlit roof is orange, and AoE2 has both collisions too. Deriving the colour from the unit's own tint still does not work: those tints are pale material shades (a villager is cream for the human owner and pink for an enemy), so both lighten to the same near-white. The colours are not chosen by maximising distance from the building palette, and that objective is recorded here as REJECTED with its reason: maximising distance from the walls maximises distance from the player colours too, and the player colour is the entire message — the magenta it produced scored 137 from its nearest building material, better than anything else considered, and still made a red player's unit read as pink (defect register, 2026-09-06). The anchor sits at the unit's own `visualTop * 0.55` above its recipe root (x+0.5/y+0.5), NOT at the ground pixel: an ankle-high plate that merely paints at a unit's feet — farm soil and team markers (<=0.51 wu), construction foundation slabs and knee-high wall courses — must never ghost a body standing in plain sight. Occluder regions project from the same rigid-part recipes picking trusts, EXCEPT that parts without real horizontal mass (min(width, depth) < 0.15 wu — flag poles, banners, scaffold rails, palisade stakes) are excluded: predicate and paint deliberately diverge for slivers, because a 2-px pole otherwise cuts flickering full-body ghost corridors across open ground. The building's MAX footprint-corner iso depth key must be strictly greater than the unit's — max-corner (not origin) depth is required in real 3-D because deeper wall cells paint over units on the building's high-(x+y) flank, while polygon containment geometrically excludes buildings wholly behind the unit. Mirrors carry fresh `ui:occlusion:` keys (the prefix says overlay CUE rather than world geometry, and sorts after every entity-identity key so the silhouette blends last), displaced along the camera ray by the drag marquee's screen-locked depth offset so they depth-test in front of buildings up to 8 world units tall; the engine-side ambient micro-wave (~1px) is deliberately dropped from mirrors. Inputs are the already fog-filtered presented entities, so a fog-hidden unit can never cue and memory ghosts neither occlude nor cue; only units are eligible (never buildings or resources). Three deliberate no-cue classes follow from the rules above and are intended, not gaps: units behind trees/resources (buildings only); units whose legs alone are hidden behind a low stone wall (0.76 wu base — the body stays readable, matching AoE2); and units behind palisades (every stake is hairline, so palisades contribute no occluder regions at all). The computation is change-gated with the snapshot rebuild (pause-frozen byte-identical, replay-identical) and exposed to tests via `getOccludedUnitStates` plus the `occlusion-showcase-fixture` capture scene, which seats an ENEMY-owned occluded unit beside the friendly one behind the same Town Center. Six of the eight slots are proved by arithmetic only — a skirmish opponent is in fog at boot, so no capture has ever reached one.
- entity selection by visible voxel silhouette (v0.2.0): a click or context order tests the iso-pixel against convex hit regions projected from the same AoE rigid-part recipes and displayed transforms used to build units, resources, and buildings, with the established ground target as a fallback. Hit state is promoted only when its epoch/revision matches the renderer's presented state; input is fenced during context loss or an accepted/presented mismatch. A click, a right-click or a read of the click stack draws the waiting snapshot before it hit-tests, and draws nothing when the snapshot on screen is already the latest: every such frame is a full render the player never sees, and on a slow renderer a burst of them queues ahead of the frames the player is waiting for (2026-09-24). This keeps moving bodies, raised walls, tools, and roofs interactive even when their ground-plane inverse falls outside the simulation footprint. Unit layer priority and human-owner exact-selection priority remain AoE rules. Repeated clicks at the same exact point cycle distinct unit/building/resource groups from the current hit set; prior group order is only a hint, so moved or vanished targets cannot be selected, and a different projected subpoint resets the cycle. Same-type equivalent units still use the double-click gesture. For a villager context order, a human-owned unit silhouette with no villager action does not hide the first resource, building, or non-owned unit hit beneath it; this makes a boar remain commandable while hunters surround it without changing raw selection order or Monk healing targets. Memory ghosts are never interactive. The proxy is data-only and Three-free; a reusable presented-state ray query remains a future engine contract.
- per-civilization ARCHITECTURE SETS, first pass (v0.3.105): AoE2 dresses each civilization's buildings in a regional set, and a scouted town names its owner's culture before a flag resolves. Every roster civilization maps to one of six sets — western-european (Britons, Celts, Franks), central-european (Teutons, Goths, Vikings, Huns, Magyars, Slavs), middle-eastern (Saracens, Persians, Turks, Berbers, Malians, Ethiopians, Indians), east-asian (Chinese, Japanese, Koreans, Mongols, Vietnamese, Khmer, Burmese), mediterranean (Byzantines, Italians, Portuguese, Spanish), mesoamerican (Aztecs, Mayans, Incas) — in `architectureStyles.ts` (sim-side, because the projector stamps `architecture` onto building views and fog memory snapshots it; unknown or absent civilizations read western-european, which is also the pre-v0.3.105 look, so legacy saves and replays without civ data render unchanged). The renderer re-keys exactly five canonical colours per set — the civilian thatch, the military tile, the dark ridge/cap tile (all three BEFORE the v0.3.83 owner-colour blend, so an architecture is a material family and the owner tint still sits on top), and from v0.3.108 the two wall-plaster slots (whitewash for a middle-eastern town, honeyed infill for a central-european one, limestone for a mesoamerican one; consulted centrally by the building part adder, so every wall face and plaster-toned prop follows its set); any other colour (stone, timber, pottery, hay bales, rope lashings) passes through untouched, because hay is hay in every civilization and a castle stays a castle. Fog-memory ghosts keep the set they were seen wearing; saves carry it per memory row (absent on older saves). v0.3.111 added per-set roof SILHOUETTES: the set shapes every stepped roof's steepness, taper, and tier count (western-european the identity; central-european steep alpine tiers; middle-eastern low two-tier flat roofs; east-asian an extra tier with a fast taper — the pagoda read; mediterranean low-pitched; mesoamerican tall temple steps). Geometry only ever shrinks within the recipe's own footprint fractions, so selection rings, health bars, and hit regions are untouched, and roof accents anchor on the returned roof top so they ride the new heights. v0.3.120 added the TOWN-CENTER CROWN: the structure atop the TC watchtower is each set's signature silhouette — a pale-drum dome for the middle east, a wide-eave pagoda finial with a spike for east asia, a steep spire for central europe, a low terracotta cap for the mediterranean, a stepped stone crest for mesoamerica; western european keeps the original square cap and a set crown REPLACES the cap (never stacks). The flag above stays owner identity. v0.3.127 added the MONASTERY FINIAL per set — the default keeps the cross; middle-eastern wears a dome with a gold orb, east-asian a tiered eave with a gold spike, mesoamerican a stone crest, central-european a steep spire, mediterranean a low dome with a lantern — each REPLACING the cross, all abstract silhouettes rather than real-world religious symbols. v0.3.143 added per-set DOOR FORMS through one pure data table (`architectureDoorBoxes`): the west keeps the plain timber leaf; the middle east adds a stepped arch head with pale stone jambs; central europe a timber frame and lintel; east asia a broad thin lintel and sill; the mediterranean a pale stone surround; mesoamerica the trapezoid doorway (wide base, stepped head). Every box hugs the door inside the recipe's own footprint (selection, picking, and health bars untouched), the western set is byte-identical to before, and the treatment rides the Town Center, houses, mills, the four military halls, and both gates. v0.3.154 closed the family with per-set WALL MERLONS through the same pure table (`architectureMerlonBoxes`): the west keeps its five square merlons byte-identical; the middle east crests with narrow pointed caps, central europe with tall teeth, east asia with a continuous ridge over thin posts, the mediterranean with three wide low merlons, and mesoamerica with a stepped band — every box inside the wall's own footprint.
...
- voxel selection and combat feedback (v0.2.0, attack follow-up v0.2.3): selection and drag preview use static yellow/cyan rigid-part outlines, the drag rectangle uses four cyan voxel segments, HP loss emits a bounded 260 ms voxel spark cluster, and the transient fog-correct witnessed-death feed drives a DEATH COLLAPSE (v0.3.119, superseding the static debris cubes): the dead unit's OWN recipe is rebuilt as a corpse that tips over and sinks across the feed window — ease-in topple (most of the fall in the first half; a linear ramp read as a standing unit and hid the whole event), a hashed per-death fall heading so the same death always falls the same way in saves and replays, weapons/tools/shields dropped with the killing blow (a spear riding the fall read as a flying plank), and the residual debris joining late as the body settles. The capture tooling was fixed alongside: `captureMapScreenshot.mjs` now pauses the sim AT BOOT and screenshots still paused, so TICKS means exactly that tick and short-lived transients (this collapse lives 10 ticks) are photographable at all — before, the page free-ran through staging plus 1.5s after advancing, and every transient was gone by screenshot time. Perspective-eligible successful unit hits now also drive the attacker's target-facing role pose at impact and through recovery, independently for every attacker; wildlife retaliation animation shipped in v0.2.8. These cues never affect authoritative damage, reloads, paths, roots, selection, or replay determinism. The former Phaser selection pulse and collapse/dust animation are historical behavior, not a live path. Richer selection motion, gathering feedback, animated death collapse, building rubble, and projectiles remain future voxel work.
- art style setting — two styles ship: the DE style, labelled "Natural" in the menu, and Moebius, the default (Moebius: user direction 2026-08-17, "migrate aoe2 to moebius art style", then "needs brighter more vibrant colors"; the DE style: owner decision 2026-09-23, *"try to make it look as close to DE as possible, which preserve the other option of moebius"*, shipped v0.3.227 as step 1 of `docs/threads/current/de-look/PLAN.md`). **The art style is a player setting, never world state.** The game menu's Settings row cycles it live, with no reload, and the choice is remembered per browser in `localStorage` under `aoe2:art-style` (`src/rendering/artStylePreference.ts`). It never enters the save format or a replay, so a match played in one style can be watched in the other. Reads fail soft: an unknown or unreadable stored value falls back to the default, and that includes the `painted` stored by builds before v0.3.196. The roster lives in `src/rendering/artStyles.ts`, where every constant records the frame measurement it was chosen against. **Moebius** resolves the world canvas through an ink-and-flat-colour pass instead of drawing it straight to the screen. The frame is quantised into flat tone bands, chroma and gain are lifted so flat colour reads as *drawn* rather than as a lit frame with its gradients removed, and contours are inked where depth breaks. The pass belongs to the sibling `voxel` package (`StylizedResolvePass`, opt-in through `ThreeRenderRuntimeOptions.stylizedResolve`), because it reads only depth, normals and luminance and so is not AoE-specific; AoE owns the tuning. Contours are SILHOUETTE-ONLY, and this is a property of voxel geometry, not a preference: a normal-crease edge source cannot discriminate when every face junction is exactly 90 degrees, so it inks every cube edge equally and costs a quarter of the frame's brightness without adding structure. **The DE style** draws with NO resolve pass (no contours, no tone bands, one render) and maps the frame through the ACES filmic tone curve at exposure 1.5. That lifts the unstyled frame to a sunlit, natural palette: L=127.3 and saturation 0.513 on the default view's lit mask, against Moebius's L=114.5 and 0.611, as measured before the style had its own ground. With its textured ground (v0.3.232) the same mask measures L=111.6 and saturation 0.560, darker than Moebius on it, because the mask reaches the rim of vision, which the soft fog edge darkens; inside the explored area it measures L=126.4 and 0.523. AoE holds the WebGL renderer, through the runtime's `rendererFactory`, so a style can set its tone mapping. Moebius keeps its frame exactly because it sets no tone curve and its pass's final draw does not apply one. A live switch rebuilds no voxel content; the first switch to the DE style recompiles the scene's shaders once. The daylight rig is unchanged: exposure alone gives the brighter frame. Explored-but-unseen ground is dimmed to each style's own level, 0.32 in Moebius and 0.6 in the DE style. On screen both draw it at about half the brightness of visible ground (0.491 and 0.506, measured on the default map after the human's units walked away), because ACES crushes dark values and Moebius's gain lifts them. Unexplored ground is black in both styles, and that is a fog-of-war rule, not a style choice (§12.1). **The DE style draws its own ground (v0.3.232, the plan's step 2).** It replaces the voxel terrain chunks in that style rather than drawing over them, because a mesh laid over them z-fights (the plan's experiment E3): the renderer lends the voxel runtime a scene of AoE's own, which holds one ground mesh (the map's top face at the voxel terrain's height, and the slab's four edge faces) lit by the same daylight rig, tone curve and colour encoding as the models, and the adapter leaves the chunks out of the snapshot while it shows (`src/rendering/voxel/aoeDeGround*.ts`). Moebius keeps the voxel terrain. What the ground shows: six procedural surfaces, baked once per page on the CPU from value noise into mipmapped tiling textures (grass with cool and warm patches, tufts and dry specks; darker forest floor with leaf litter; drier hill grass; water; sand; dirt), with a large macro field over them so the four-tile repeat does not read; soft, irregular blends between kinds, moved off the tile edge by each surface's texture and a tile-sized wander field, least where water meets land; sand within about half a tile of water and lighter water near the shore; and dirt under and around every building the player knows of, a presentation of the footprint that writes nothing back to the simulation. No image file is used. The shore surf strokes are left out in this style, because they follow the tile edge the shoreline no longer does; the water's ripples stay. Nothing on the ground animates, so a paused frame is the same frame and nothing about it can become simulation state. **Fog on the DE ground:** an unexplored cell is black to the pixel, and nothing about it is packed for the shader, so no neighbour can blend toward it; known ground darkens to black over about 0.7 of a tile beside it (0.54 to 1.0 as a wander field moves it), so the edge of the explored area is soft on the known side; explored-but-unseen ground is dimmed through a linearly filtered fog texture, so the edge of vision is a gradient that reaches at most half a tile past the visible cells. The style's explored level scales the sRGB colour, as the voxel ground's does, so 0.6 still draws explored grass at about half the brightness of visible grass (0.465 on the GPU and 0.467 on SwiftShader, measured by the browser gate below). A building dirties the ground only if the player has explored a cell under it. Packing the ground's data costs 0.12 ms a frame on the default 60x36 map and 0.61 ms on the largest, fully explored, on the development machine. On SwiftShader, the CPU rasteriser CI draws with, the ground costs more than the voxel ground: with the DE style as the default, CI's live frame median was 233.3 ms against 116.6 ms in Moebius on the run before (CI runs 36077810827 and 36073613836, the unit-motion-smoothness report). Gated by `tests/rendering/aoeDeGroundData.test.ts` (the packed bytes are identical whatever an unexplored cell holds, kind or building; kind codes stay 1 to 4, which the shader indexes by), `tests/rendering/aoeDeGround.test.ts` (the mesh, what shows when, uploads only on change, and the shader splice into Three's Lambert program), `tests/rendering/AoeVoxelWorldRendererArtStyle.test.ts` (one ground at a time, chunks returning as new incarnations), and `tests/browser/de-ground.spec.ts` (no terrain chunks in the runtime; sampled points of the unexplored rim black, with the known side darkening toward them; explored grass between 0.4 and 0.62 of the brightness of visible grass; the ground drawn again after a lost WebGL context). **Moebius is the default.** Decision D1 makes the DE style the default once its textured terrain lands (the plan's step 2). It was the default in v0.3.233 and went back one menu row in v0.3.234, because that ground doubled CI's SwiftShader frame (above); it becomes the default again once the ground costs no more there than the voxel ground. The default waited for the style's own ground because the owner withdrew the unstyled "Painted" look on 2026-09-03 ("I only want the moebius art style. Remove the other style."), and step 1's DE frame was that same unstyled render with a better colour grade, so showing it by default before it looked like its own style would have risked showing the owner the look they rejected. The flip is the one constant `DEFAULT_ART_STYLE_ID`, and a stored choice wins over it: a player who picked either style keeps it, and one who never chose sees the default. Moebius is a treatment over the same models, not a frozen snapshot (decision D2): later steps that change recipes or colours change both styles. Gated by `tests/rendering/artStyles.test.ts` (the roster and its default; the four measured DEPARTURES from `MOEBIUS_RESOLVE_PRESET` survive and the values deliberately inherited from it are still inherited, so a silent fall back to the preset fails and so does freezing this repo against an upstream improvement; the DE grade), `tests/rendering/AoeVoxelWorldRenderer.test.ts` (each style's pass and tone curve reach the runtime, and a live switch changes the pass, the tone curve and the explored level and rebuilds nothing), `tests/rendering/artStylePreference.test.ts`, `tests/app/artStyleSetting.test.ts`, `tests/ui/gameMenuArtStyle.test.ts`, and `tests/browser/art-style-setting.spec.ts`, which clicks the real menu row, sees the paused canvas change, reloads and sees the choice kept. `scripts/captureMapScreenshot.mjs` captures either style with `STYLE=moebius|de` and refuses a frame drawn in another. Deferred: per-style palettes, tapered line weight, a second Moebius tuning for zoom levels away from the default, worn paths on the DE ground, and the plan's steps 3 to 9 (water motion, depth and shore foam on the DE ground are step 5).
- distinct alert, economy, combat, and age-up audio cues — first slice shipped v0.3.109, all ORIGINAL/procedural (synthesized from oscillators at play time; no samples, no audio assets, the sound analogue of the art rule). Shipped cues: a two-tone brass-like horn when a blow from anything another player owns lands on a villager or a building owned by the human (a unit's since v0.3.109, a tower's, Town Centre's, Castle's or a blast's since v0.3.235) (throttled to one per 200 ticks — 20 s of game time at this build's TPS of 10 (v0.3.218; it was 400 ticks under a comment claiming "~20s at 20 TPS", so a raided player was warned every FORTY seconds from the day the horn shipped — defect register, 2026-09-06). The twenty seconds is what the comment always intended and is NOT a measured DE figure: DE's own spacing could not be established from any authoritative source, and the one statement found describes a rule of a different SHAPE — suppression only when two attacks fall within ten TILES *and* ten seconds — so two raids far apart alarm twice in DE and once here, and there is no single DE number to copy. The spatial half is unimplemented and is an open follow-up rather than a defect. The exact rule, and the ATTACK WARNING it drives, are at the end of this bullet), a rising fanfare on each age transition (never for the starting age), and a victory/defeat sting exactly once at match end. The decision logic (`gameAudioController`) observes projections only — the hit feed of blows on every player's units and buildings, of which the controller keeps the human's, the HUD age, the match outcome — and never touches the simulation; the synth voices (`proceduralVoices`) are injected, so the controller tests without an AudioContext. The AudioContext is created lazily on the first user gesture (the browser autoplay rule). A speaker toggle sits above the idle-villager bell; mute silences the speaker, not the state machine (unmuting does not replay missed cues), and persists per player in localStorage under the rule that a throwing storage must never break the game (the art-style preference set that precedent, was withdrawn with the second style in v0.3.196, and returned with the DE style in v0.3.227). v0.3.110 added a research-complete ding (one per completion, batches collapse to one — information, not noise; pre-seeded techs on an advanced-age start never ding) and a two-strike countdown bell when a wonder/relic countdown activates. v0.3.116 added the town-bell peal — three urgent strikes when the player rings the bell, observed through a live-facade ring tally (`getTownBellRings`; audio-only, never sim state, so replays stay silent and saves carry nothing). v0.3.118 added the order ACK — one very soft 60ms tick per successful order GESTURE (context click, move, attack-move, patrol, attack-ground; counted once per gesture on the live facade, not per unit, and batches between frames collapse to one click). v0.3.123 added SELECTION CHIRPS — a short two-note figure per role family when the primary selection changes to an own unit (bright rising villager, low firm military, soft-fifth monk, wooden-knock siege, hollow-whistle ship; melody, not speech — the procedural equivalent of the voice lines). Reselecting the same unit is silent; watching the selection id is the whole observable. v0.3.124 closed the list with AMBIENCE: a very quiet looping wind bed (filtered noise breathing under a slow LFO) and an occasional reproducible three-note birdsong (LCG-scheduled 6–14s apart) — scenery, not information; it observes nothing, starts with the AudioContext on the first user gesture, and honours the same mute switch as every cue. The distinct-audio-cues presentation target is COMPLETE: alert (horn, town bell, countdown bell), economy (research ding, order ack, selection chirps), combat (victory/defeat stings), age-up fanfare, and ambience, all synthesized. **The attack warning (v0.3.217) is one decision with four cues, and its rule is exact.** A warning is raised when a blow from anything ANOTHER PLAYER owns lands on a villager or a building owned by the human, whatever dealt it: a unit's swing or shot, a Town Centre's, tower's or Castle's arrow (garrisoned arrows included), the blast of a mangonel stone on the units beside its target, a bombardment of the ground, or a demolition charge, as DE warns whatever is attacking (v0.3.235, defect register 2026-09-24: an enemy Town Centre shot a villager from 25 to 10 hit points on the real map and nothing was shown). Each narrowing is deliberate: wildlife is excluded (a lured boar biting the villager that shot it happens in the opening of every match, on purpose), and so is the human's own attacker (its mangonel splashing its own villagers), while an ALLY's blow counts: an ally's mangonel stone or demolition charge among the human's villagers is damage to the human's economy by another player, and warns (decided v0.3.235, a reversible default: no DE source was found either way, and before the hit feed an ally could never warn only because targeting picks enemies), soldiers are excluded as targets (a villager losing health is news by definition; a soldier taking a hit is the fight you went looking for), and the rule keys on WHO WAS HIT rather than on who swung or on what is standing nearby, so the Town Centre shooting back raises nothing. It expressly does NOT suppress the warning when the camera is already on the spot — the defect it was written for happened inside the player's own base with the camera there. `gameAudioController` is the single decision point for all four cues (horn, words, Space's jump target, minimap mark) so sound and picture cannot disagree; the MARK and the WORDS are raised outside the mute switch, so a silenced player still sees and reads where the raid is, and the mark is drawn ON the minimap — a pulsing white-cored ring with a ripple running out of it — rather than floating over it, because nothing may cover the minimap. **A player under attack cannot miss it (v0.3.229, defect register 2026-09-24), as in DE, which says it in text, sounds a horn and flashes the minimap.** (1) The WORDS: an alert toast reading exactly "You are under attack!", raised on the same throttled decision that sounds the horn — one horn and one toast together, at most one of each per 200-tick window — styled apart from a rejection toast and up for 5 s. The toast carries the tick of the hit it announces (`data-hud-toast-hit-tick`), the throttle's own clock. The page draws it up to a frame of ticks after that hit, so the tick the page has reached by then is not the one the window is measured on. (2) The MARK HOLDS ON SIMULATION TIME: every hit on the human's economy, throttled or not, moves it to the newest hit and restarts its hold, and it goes dark only once a whole throttle window (200 ticks) has passed with no hit. The throttle is measured between the ticks of the HITS, the same clock as the hold, never the tick of the frame that noticed them. So a raid that keeps hitting keeps it lit at every game speed, a paused game keeps it however long the pause, and when it does go dark the next hit is sure to sound the horn and show the words again, however many ticks one frame stepped. A tick before the last hit (a load or a replay seek backwards) clears it. It was 8 s of wall clock against a throttle counted in ticks, which left the minimap dark for about 40% of a sustained raid at normal speed and dropped the mark 8 s into a pause. Its pulse is animation only and runs on the wall clock, so a paused frame still moves. (3) The mark is SIZED IN SCREEN PIXELS: a core at least 7 screen pixels in radius and a ring whose base radius is at least 20 (it pulses between about 17.5 and 20), at every minimap size, divided back out of the canvas's display scale; it was a multiple of one cell with a 3-canvas-pixel floor, which at the real minimap drew 16 alert pixels on a 1600x900 screen. Only the core and a thin halo around it are FILLED; out at the ring everything is a stroke, so the ground and the units between the core and the ring stay visible, and the core covers only the spot that was hit. The browser gate requires at least 100 screen pixels of the white core on every frame of a sustained raid and of a pause. (4) A LOAD STARTS THE WARNING AFRESH: when a save is loaded, the load path says so and the controller forgets everything it observed in the old world (its last horn tick, the last attack tick it saw, the Space target, the mark, and every other cue's memory), so the first raid in the loaded world sounds, says and marks at once. It kept them, so the same raid met again after loading an earlier save came with no horn and no words. Only a LOAD resets it: a replay step, a scrub, a marker jump and a fog-owner switch also build a new bridge, over the same recording, and keep the controller's memory, so stepping through a replayed raid announces it at most once per throttle window rather than on every step. Space's jump target follows the newest hit, as the mark does. This REPLACES a proximity rule that compared each swing's target cell against every own building's and villager's RENDER ORIGIN within one cell: a swing targets the VISUAL CENTRE, origin plus half the footprint less one, so every attack on a 4x4 building — Town Center, Market, Castle, Wonder — was silently dropped, and the check matched villagers only because a 1x1 footprint puts that offset at zero (defect register, 2026-09-06). The warning reads the HIT FEED (v0.3.235), which records every blow that lands on a player's unit or building where the damage is dealt, with its `participants` (attacker owner, target owner, whether the target is economy), so a blow is warned of on the tick it lands, and an arrow that misses warns of nothing. Every place hit points drop records into it (melee on a unit or a building, a shot landing on either, each blast victim, a wildlife bite with no participants), and a census compares it with every hit point each player-owned unit and building loses, and every death but a demolition charge spent by its own blow, over an all-AI match and eleven scenarios (`tests/simulation/playerHitCensus.test.ts`), so a damage path those reach cannot leave it silently. It is transient: a load starts it empty, so a load is silent until the next live blow. A replay step re-simulates from the latest snapshot at or before its tick, so it holds the blows re-dealt since that snapshot and none from before it, the same reach the swing feed had. Until v0.3.235 the warning read the swing feed, which records only a unit's swing, so a tower, Town Centre or Castle shooting a villager, a stone's blast and a bombardment warned of nothing.
...
- **No scheduled LLM playtest: the nightly is retired (owner decision, 2026-09-25).** The owner: *"no API key, no nightly playtest"*. The harness runs where a person or a session starts it (`npm run playtest:llm`, as in the summer campaigns), and nothing runs it on a schedule or in CI. The nightly workflow `playtest-llm.yml` came from an agent's own plan on 2026-05-08 (`675771e5`, "phase 5 — CI workflow + corpus runner"), not from an owner request, and it could play only with an `ANTHROPIC_API_KEY` repository secret, which the repository never had. Measured on 2026-09-25: of its 138 scheduled nights (2026-05-10 to 2026-09-24), 89 read green on the no-provider skip, 48 were never started by GitHub, each refused over the account's billing on a night between 2026-06-17 and 2026-09-05, and 1 was cancelled while installing a browser; the repository holds 0 Actions secrets, and none of its 323 artifact records, which reach back to 2026-05-09, is a `corpus-llm` upload. It played no game. The workflow is deleted, with everything that existed only to serve it: the corpus runner `scripts/playtest-corpus-llm.mjs` and its row file `playtest-corpus-llm.json`, the HTML dashboard over the runner's output (`scripts/playtest-corpus-llm-dashboard.mjs`), the modules `corpusLlmSchema.ts` and `corpusRegression.ts`, the `playtest:corpus-llm` and `playtest:corpus-dashboard` scripts, and their tests. The deterministic corpus (`npm run playtest:corpus`, `playtest.yml`) is unchanged and is still the remote playtest gate.
...
- **Checkpoint screenshots (option C, 2026-06-10 — replaces the Phase-6.C.1 visual-regression oracle).** The runner captures a screenshot whenever an advance lands exactly on a checkpoint tick (`--screenshot-every <ticks>`, default 1000, 0 disables; align to multiples of `--decision-interval`) and persists them to `<out>-screenshots/<tick>.png`, where a person reading the run looks at them and `playtest:findings` hands the last one to its conformance probe. There is NO baseline comparison and NO visual CI gate for LLM runs: a non-deterministic player has no "correct" reference image, so pixel diffs against a fixed PNG measured expected divergence rather than regressions (campaign-1 evidence: 9.86% diffs from a competently different base layout). Simulation determinism remains covered by the replay/snapshot-equivalence suites and render correctness by the deterministic Playwright browser tests; `engineHalt` plus the gameplay oracles are an LLM run's regression signals, and the conformance findings are the advisory build-backlog signal. The pure `visualOracle.ts` module is retained, unwired, for a future deterministic corpus row where baselines are meaningful.
...
### 15.8 Self-Play Content Coverage Lab (Internal Tooling)

What the game's content self-play actually exercises is measured in a match the AI cannot end. The simulation accepts a closure-local `disableAiAttacks` option under which no AI seat ever launches an attack, by land (the attack phase) or by sea (the ferry phase) — defence, building, production, research and trade are untouched — and the lab also disables the Wonder and Relic victories (`victory: 'conquest-only'`), so nothing can end an all-AI match before its horizon and the census reads the technology tree instead of the match's length. The lab exists because on a match that resolves, a winner's building and unit variety measures how long the match lasted: the boot map ends by conquest at tick 39,812 under v0.3.205, and the side that wins it shows fewer building types than the side that lost the longer match before it. The census records, per owner, every building type completed, every unit type trained and every technology researched. The coverage gate (`tests/simulation/selfPlayContentCoverage.test.ts`) holds a NAMED floor — a type that stops being exercised fails by name, never as a count — and prints the gap against the game's own content lists, which is the standing to-do for feature completeness (§16.3). The option never reaches the real game and is not persisted: a save made under it resumes without it, and a recording made under it cannot be replayed faithfully (a replay world takes no options), so nothing records under it.
...
**What "the whole game" covers (owner decision, 2026-09-23):** *"No multiplayer, no campaigns."* The whole game is the whole single-player game played against the computer (§2.2, §2.3). Multiplayer and campaigns are excluded by the owner, so the parity pursued above does not reach them.

Still out of scope:

- multiplayer — excluded by the owner, 2026-09-23
- campaigns (the campaign story mode) — excluded by the owner, 2026-09-23
- non-standard game modes (Regicide, Death Match, Empire Wars, King of the Hill, Wonder Race, Capture the Relic) — not requested
