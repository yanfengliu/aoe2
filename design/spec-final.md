...
- built by a villager (Dark Age, 60 wood, 1×1, 480 HP, 15-second build); becomes a gatherable 175-food resource on completion; when depleted it auto-reseeds for 60 wood while its owner can afford it, otherwise it is removed (see §6.6). Farm-upgrade techs and the Mill batch-reseed queue are deferred.
