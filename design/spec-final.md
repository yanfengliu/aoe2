...
- Minimap readability follows the same top-down tactical convention as AoE2 rather than mirroring the isometric camera: terrain cells draw first, high-contrast dark-backed entity markers draw above the terrain, and fog plus the viewport rectangle draw last. This keeps player/enemy/resource/building positions legible inside the explored blob while preserving the existing camera click/drag mapping. Shipped v0.1.120.
