...
**AI Lumber Camp siting (v0.3.177, REVERTED v0.3.178).** The v0.3.177 rule — Lumber Camp on the medoid of currently-targeted trees, Mining Camp requested first in the Dark Age — is withdrawn: at match resolution on the boot map it lost the game the previous behaviour wins, because its larger economy thinned the home woodline sooner and villagers were then sent to gather inside the enemy Town Centre's range (thirty killed between ticks 35,750 and 45,000). Camps are sited beside the harvestable tree nearest the Town Centre within 12 cells, and the Dark Age requests the Lumber Camp before the Mining Camp, as before. The exposed defect — automatic gather and hunt targeting ignores enemy static defences — is registered OPEN and must land before any camp-siting change is retried. Nomad is unchanged.
