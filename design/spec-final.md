...
**Curated bonus-bearing civilization roster (scope decision 2026-07-05).** The game deliberately does NOT implement bespoke identities for all 30 civilizations. Civilization bonuses (and, when added, unique units / unique techs) are implemented for a curated roster of FIVE civilizations only: **Britons, Franks, Goths, Aztecs, Mongols.** Every other civilization is selectable (`?civ=`) but plays with DEFAULT stats — no bonuses, no uniques. This caps civ variety at a manageable, well-polished few so effort concentrates on an otherwise complete and polished game rather than spreading thin across 30 civs. Do NOT add bonuses for civilizations outside this roster. (An earlier Slavs "Farmers work 15% faster" bonus, shipped in v0.1.100, was removed in v0.1.101 to honor this cap; Slavs is not in the roster.)
...
**Voxel feedback supersession.** Any later bullet in this historical section that says a Phaser painter cue is “shipped” describes the former 2-D renderer, not an additional live path. The sole voxel renderer currently provides static selection outlines, a visible four-segment drag marquee, authored-height health bars, bounded hit sparks, static witnessed-death debris, successful-hit unit attack poses, and the white behind-building unit silhouette (returned in voxel form in v0.2.5 — see the units-behind-buildings bullet). The former animated selection pulse and collapsing death body/dust rings are not present; gathering feedback and richer death feedback remain explicit future voxel work, and wildlife retaliation motion is an active 2026-07-14 directive above.
...
- attack-pose violence directive (user, 2026-07-14): the successful-hit pose must read as a forceful strike, not a gentle tap — a pronounced windup that loads the weapon arm, a fast impact snap toward the captured target, and a brief follow-through/recovery, with amplitude large enough to read at default zoom. This is a quality bar on the existing v0.2.3 channel and inherits all of its constraints unchanged: no authoritative root lunge, deterministic phase/weight, one-tick movement-cancellation smoothing, pause-freeze, and fog/witness rules.
- construction work animation (user, 2026-07-14): a villager in the actively-building state plays a repeating hammer work loop — raise the hammer overhead, smash it down toward the build site/ground, brief reset — target-facing toward the construction, frozen by pause, deterministic, presentation-only (build progress, repair rates, simulation, saves, and replays unchanged).
- wildlife retaliation attack animation (user, 2026-07-14; activates part of the wildlife-motion deferral above): a boar whose retaliation strike lands plays a target-facing charge/gore pose through the same successful-hit presentation channel as unit attacks (same witness/fog gating, determinism, pause-freeze, and no-root-lunge rules).
...
- resource visual variation (user, 2026-07-14): gold mines, stone mines, and trees draw deterministic per-instance variants — rotation, facet/canopy layout, prop mix, and small height/size jitter seeded purely by entity identity — so adjacent same-type resources no longer read as copies. Variation is a pure function of the entity (no per-frame or per-load randomness), render-only, stays within the resource's cell/footprint bounds, and preserves amounts, gathering, hit silhouettes, selection, fog memory, saves, and replays.
...
- voxel selection and combat feedback (v0.2.0, attack follow-up v0.2.3): selection and drag preview use static yellow/cyan rigid-part outlines, the drag rectangle uses four cyan voxel segments, HP loss emits a bounded 260 ms voxel spark cluster, and the transient fog-correct witnessed-death feed emits static voxel debris. Perspective-eligible successful unit hits now also drive the attacker's target-facing role pose at impact and through recovery, independently for every attacker. These cues never affect authoritative damage, reloads, paths, roots, selection, or replay determinism. The former Phaser selection pulse and collapse/dust animation are historical behavior, not a live path. Richer selection motion, gathering feedback, wildlife retaliation animation, animated death collapse, building rubble, and projectiles remain future voxel work.
