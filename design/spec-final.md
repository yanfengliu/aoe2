...
- sun-cast ground shadows (owner steering 2026-09-02, "the rendering needs to feel realistic"; shipped v0.3.193): every live building, resource and unit whose recipe has a solid part casts a shadow on the ground (the unit layer's malformed-projection fallback, which has none, keeps a flat footprint slab), projected from the SAME sun the daylight rig lights the scene with — `aoeVoxelDaylight.ts` is the one source for both, so a point at height h lands h·22/36 toward +x and h·18/36 toward +z, away from the sun at {-22, 36, -18}. The caster is ONE box per entity: the union footprint of its solid parts whose footprint area is at least a quarter of its largest part's, from the lowest solid part's bottom — or the ground, whichever is higher, so a felled carcass does not sweep the mass it has rotated underground — to the tallest qualifying part's top (for a ship that is the hull, so masts and sails cast nothing) — so a Town Center casts from its plinth-to-roof mass and not from its tower or flag pole, a tree from its crown, a unit from its body. The shadow is that box's exact swept footprint (a hexagon), drawn as three non-overlapping slabs on the existing translucent `shadow` surface: the footprint plus one sheared parallelogram per leading edge (a part with `groundAxes`). Shadows of different entities never draw at the same height, because two coincident translucent slabs z-fight and tear into stripes: each caster's slabs sit at one height, 0.002 world units separate adjacent heights (the depth-buffer floor for the 4000-unit orthographic range on a 24-bit buffer), and the shadow lane is drawn top-down, so where two overlap the lower slab fails the depth test — one blend per pixel, no tearing and no darker patches along a forest edge. A caster's height starts from its own cell (x mod 3, z mod 3, nine levels), which is a good colouring — a dense woodline is a nine-clique of overlapping tree shadows and the cell rule colours it exactly, where greedy colouring from level zero does not — and a whole-lane pass run per presented frame then steps up any caster whose height a caster it overlaps already holds. That repair is what the cell rule cannot supply: two casters in the SAME cell seed identically, and units share cells routinely (a 12,000-tick AI-vs-AI run of the boot map reached four villagers on one tile and thirteen same-height overlapping slab pairs at once); a large building's shadow also outreaches three cells, so a Town Center and a House three cells away share a seed and overlap. The repair search is UNBOUNDED, because a capped one must fall back on a height that is taken by construction — a nine-step cap measured 38 coincident overlapping slab pairs on a 6x6 woodline with twelve villagers gathering in it. A clique of K mutually overlapping casters therefore needs K heights and rises ABOVE the nine-level band: measured 0.058 for those twelve villagers and 0.072 for a 200-population blob packed at a third of a tile, three to five pixels of screen offset at the closest camera zoom. Nothing bounds that but how tightly the simulation can pack casters, and it is the accepted price of never tearing. Two consequences are accepted and unfixed: a caster's height changes as the crowd around it moves and as it crosses a cell boundary (18.9% of caster-frames in a walking group of 24, up to three pixels), and the 0.002 separation assumes a 24-bit depth buffer that nothing in the renderer pins. The shadow does NOT turn with a unit's heading and is cast from the rest pose, so a swinging arm cannot make it flicker; a felled carcass casts from its felled body; a body-less unit-layer fallback keeps a flat footprint slab; memory (fogged) entities cast nothing, as their contact slab never did. Under the Moebius art style the tone bands quantise the 22% darkening to a full band step, so a shadow reads darker there than under Painted — the style's property, shared with every other shading gradient. Render-only: picking, silhouettes, occupancy, saves and replays are unchanged. Real shadow maps and AO stay deferred (previous bullet); the daylight rig is backlit relative to the fixed camera (both visible wall faces receive fill light only), and moving the sun to light one visible wall is the next measured candidate — see `docs/threads/current/render-realism-2026-09-02/DESIGN.md`.
