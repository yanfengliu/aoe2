...
- **Software engineer**: Handles code writing.
  - After coding, ask the code reviewer to review (see Code review section) and iterate. Multi-CLI reviews run in the background; timing and poller mechanics are in the fleet runbook (see Code review section).
...
The default adversarial pass for non-trivial work is the in-process Workflow (see Core rules). Run the multi-CLI review (Codex + Claude, each reviewing independently) on high-risk changes and full-codebase audits. All multi-CLI mechanics — current review model pins, exact commands, sandbox flags, the background-run/poller pattern, the Codex output-extraction recipe, and CLI failure modes — live in the fleet-canonical runbook `../loop-ops/docs/skills/multi-cli-review.md` (review pins bump there, once for the whole fleet); `.claude/skills/multi-cli-review/SKILL.md` is this repo's thin stub naming aoe2's local pin sites (`scripts/propose-fix.mjs` — sync it on every fleet bump). Read both before every multi-CLI session.
...
- **Enrich the baseline prompt** (quoted in the fleet runbook) **with task-specific context** — the change's intent, prior-iteration findings to verify, files to focus on, and an anti-regression checklist. The bare baseline returns generic feedback; useful reviews need the specifics.
- **Keep model IDs current.** Use the latest-family alias when a command is meant to track the newest model (for example, `opus[1m]`); bump pinned strings whenever a more capable fixed variant ships (e.g. `claude-opus-5-0[1m]`, `gpt-5.6`). Verify with a one-line smoke test (`echo "ok" | <cli> ...`) before committing the bump — silent fallback to an older model is the failure mode to guard against. Review-command pins live in the fleet runbook (`../loop-ops/docs/skills/multi-cli-review.md`); the playtest pin is governed by `design/spec-final.md` §15.7.
...
- **Stay current with `civ-engine`.** Whenever `civ-engine` ships an update — especially a breaking one — this downstream must pull it and reconcile before continuing other work: check `../civ-engine/docs/changelog.md` for the new entries, update the pinned engine (the CI engine-dist chain fetches `civ-engine`'s prebuilt tarball, so keep the engine green + pushed before dependent aoe2 changes), adapt call sites to any changed API, and re-run the four gates (`npm test`, `npm run typecheck`, `npm run lint`, `npm run build`). This is the durable rule; the convenience trigger is the repo-scoped `/.claude/commands/check-engine` command (relocated here from the global namespace — engine-update discipline is repo-owned, not a universal command). The canonical breaking-change contract lives in each downstream's AGENTS.md, not in `civ-engine` (which we don't modify); propagate this rule to sibling downstreams via the `sync-instructions` flow when they exist.
