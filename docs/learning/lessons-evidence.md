...
Lesson: for any recorded-run bug, `npm run replay:inspect -- <bundle>` (or a `SessionReplayer.openAt` + `WorldDebugger` pass) BEFORE writing a repro or a fix. Pointer: `scripts/replay-inspect.mjs` (`fromBundle` → `openAt(tick)` → `getEconomyState()` per owner, un-fog-filtered), `docs/engine-feedback/current.md` endTick entry. This rule lived a second life as an AGENTS.md "Known traps" bullet until 2026-08-07; that duplicate was deleted and this entry is the only copy.
...
Pointer: [fleet/docs/skills/multi-cli-review.md](../../../fleet/docs/skills/multi-cli-review.md) — "Reading Codex output" (moved out of AGENTS.md 2026-07-09, promoted to the fleet-canonical runbook 2026-07-10 with `-o` extraction as primary); affected reviews under `docs/threads/done/replay-scrubber/2026-05-01/impl-{16..24}/REVIEW.md` (post-mortem note added).
...
## A normal-crease edge source cannot discriminate in voxel geometry — every face junction is exactly 90°, so it inks every cube edge equally — 2026-08-17

| Field | Value |
|---|---|
| Surfaced by | First render of the Moebius art style on the default view (`aoe2-prototype`, 800x600), using `MOEBIUS_RESOLVE_PRESET` from `voxel/three` unchanged. |
| Symptom | The frame came out visibly dark and muddy; trees and rocks lost their shapes to grey. Mean world-canvas luminance fell from the Painted baseline of 90.1 to **69.8** — 23% of the frame's brightness gone. |
| Wrong hypothesis, ruled out by measurement | The obvious suspect was the linear/sRGB round trip through the offscreen render target, since the pass does its own `toDisplay` encode and a double encode or a missed one would darken everything. Configuring the pass as a pass-through (`flatten: 0, inkStrength: 0`) reproduced the baseline at **L=90.2 against 90.1**, which ruled the colour pipeline out in one capture and reframed the whole problem as tuning. |
| Root cause | A normal break is a *crease inside one form* — the thing that draws a roof ridge against the wall beneath it in smooth geometry. Voxel geometry has no such thing: every face junction is exactly 90 degrees, so `1 - dot(n, neighbour)` is exactly **1.0** at every cube edge. It is not a weak signal, it is a *constant* one, and any `normalEdgeScale` at or above 1 inks every cube edge at full strength. The preset's 2.4 was tuned on the smooth-geometry sibling game where creases are rare and meaningful. |
| Fix commit | `src/rendering/artStyles.ts` — `normalEdgeScale: 0`, `depthEdgeScale: 1.2`, with the measurement table in the module doc comment. |
| Test added | `tests/rendering/artStyles.test.ts > makes Painted the absence of the pass, not a neutral configuration of it`; `tests/rendering/AoeVoxelWorldRenderer.test.ts > starts in the requested style and hands its options to the runtime`. The tuning itself is pinned by the before/after/diff artifacts, not by a unit test — a constant chosen by looking at pixels cannot be asserted by one. |
| Behavior delta | Before: contours drawn from both depth and normals, which on voxel content meant every cube edge, at 23% of the frame's brightness. After: silhouettes only. Intermediate values were measured rather than guessed — `normalEdgeScale: 0.25` read as haze over the blocks rather than as line (L=88.4), and `depthEdgeScale: 2.4` thickened villagers and grass tufts into blobs (L=82.1). |
...
**Recurrence (2026-08-29), on the tool built to stop exactly this:** checked the new `npm run ci:status` with `... | tail -6; echo "EXIT=$?"` and read `EXIT=0` as green while the script was reporting a RED corpus gate in the text above it. Unpiped it returns 1 correctly. Reading a gate through a pipe is how the gate stops being a gate — and this time the gate in question was the one that reads other gates.
...
### Play-testing beside running agents must use the preview build (2026-09-02)

Anchor: `package.json` scripts — `predev`, `prebuild`, `pretypecheck`, `pretest` and `prelint` each run `voxel:build` (`npm --prefix ../voxel run build`), and `node_modules/voxel` is a symlink to the sibling repo, so the dev server imports `../voxel/dist` live.

What happened: a live match on `npm run dev` reset to boot state, then went to a black page. The Vite server log showed a "page reload" for every file under `../voxel/dist/**` at 7:48:53 PM followed by "Failed to load url …/voxel/dist/three/index.js — does the file exist?" — the sibling's `dist/` was being wiped and regenerated by a worktree agent's gate run, and for the rebuild window the modules the page was importing did not exist (500s). The dist was rewritten again two minutes later by the next gate run. Two fixes were tried first and were both wrong for this: a Vite `server.watch.ignored` for `.claude/**` and `tmp/**` (correct for worktree writes inside the repo root, and kept — but the sibling lives outside the root), and restarting the dev server (it just re-imported the same live link).

The rule: while any agent may run gates on this machine, play-test on `vite preview` (launch config `preview`, port 4173). The preview serves aoe2's own bundle with voxel baked in at build time, watches nothing, and cannot be clobbered by a sibling rebuild. Build once first so it serves the state the gates verified. HMR is the only loss, and a play test does not want it.

Why it is a lesson and not a gate: nothing in the test suite can observe a live play session losing its server; the check is the launch config choice, and this note is what makes the next session choose it.

**Addendum, same day — the pane must be VISIBLE too, or the play test must be headless.** With the preview server up and no agent in the loop, the match still froze at 00:00 with orders accepted and never executed. `tabs_context` reported "The Browser pane is currently hidden": the sim is `requestAnimationFrame`-driven and a hidden document receives no frames, so input lands (the idle badge count fell 3 -> 1) while nothing ticks. The first manual session worked only because the pane was displayed the moment it was opened. This is DE-faithful (single-player pauses when unfocused) and not a defect. Consequence: a manual play test needs the owner to have the pane showing; a repeatable one runs under Playwright, whose headless page is visible to itself — that harness is `tests/browser/play-opening.spec.ts`.

### A horizon that shows a candidate winning is suspect until the resolution horizon agrees (2026-09-02)

Anchor: `tests/simulation/aiReachesCastleAge.test.ts` (horizon 24,000 -> 30,000 in c47436cf -> 45,000 in the revert), and the critic's re-runnable probes.

The 24k gate scored the boot map before its Castle (25,250), which was true and led me to raise it. I raised it to 30,000 — and the candidate passed. The candidate's per-horizon peak army on the boot map, baseline/candidate: 24k 8/7, 30k 9/10, 34k 9/10, 40-45k 20/10. It leads in a ten-thousand-tick window and nowhere else, and the gate landed inside that window. I had a 45k audit showing "6 of 8 slots reach Castle, boot map conquest at 39,890" and cited it as the state of the game — it was the baseline. The candidate was never run at 45k until the critic did it, twenty-seven minutes into a read-only review.

Three things compound here and each is a rule. (1) Re-justify a horizon against MATCH RESOLUTION, not against the age the test is named for: the test's own 24k comment — "the army keeps growing after that, so a shorter run would gate the army bar on a half-built one" — applied verbatim to 30k (9 of 20). (2) When a candidate's lead appears only after moving a bar, run the candidate at the longest horizon the repo already uses before believing it. (3) "Bars derived before the candidate" must be verifiable from the repo — a timestamp in a scratchpad is a story; commit the baseline measurement, then the bars, then the candidate, as separate commits.

### `git stash` is repo-global, not worktree-local (2026-09-02)

Anchor: the enemy-range agent's report at ~03:20 on 2026-09-02; stash commit 9e01f415 ("On worktree-agent-a691a8adaa597cf3b: castshadow-wip-2").

Two agents in separate worktrees each did the obvious thing to bring main into a dirty tree: `git stash`, `git merge main`, `git stash pop`. The stash list is shared by every worktree of the repository. Between one agent's `pop` (which conflicted) and its `git stash drop`, the other agent pushed its own stash above; the `drop` then removed the wrong entry. The first agent noticed, restored the other's stash verbatim with `git stash store` (same commit object, byte-identical) and sent the warning. The second agent may meanwhile have popped the first agent's files into its own worktree — enemy-defence modules and gather-assignment changes appearing in a rendering branch.

Nothing was lost this time because one agent read the stash list before dropping. The rule is in local-rules: do not stash in a worktree while others are live; commit a throwaway WIP, merge, squash. Named stashes addressed by `stash^{/<name>}` are the fallback, never `stash@{N}`.
