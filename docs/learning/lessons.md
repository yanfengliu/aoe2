...
- A guard that is safe on one resource can be ruinous on another: the same defensive floor that fixed the WOOD reserve destroyed every economy when applied to FOOD ([evidence](lessons-evidence.md#a-guard-safe-on-one-resource-can-be-ruinous-on-another-2026-09-01))
