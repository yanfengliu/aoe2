...
**Narrowed to one line of enquiry (2026-08-29, after the v0.3.162 carry fix).** Owner 2's phase budget is now 48% walking to a resource, 36% idle, 8% hauling, **8% gathering**. The state at tick 6,000 is the tell: **all six villagers sit `task=idle` with `targetResourceId=null`**, three of them still holding a carry of 10 — while within 8 tiles of their Town Center lie 6 berry bushes, 2 sheep, 4 gold mines, 4 stone mines and 30+ trees, every one of them `baseOwner=2` or unowned. They are not walking far and they are not blocked; they have no target at all.

And it is NOT the assignment failing. Instrumented across 200 ticks, `assignNearestResource` is called for owner 2 repeatedly, its candidates pass the harvestability and kind filters, and it never once takes the no-target branch — no `ZERO-MATCH`, no `NO-TARGET`. **Assignment always succeeds when it runs; something clears `targetResourceId` back to null after it, within the same tick.**

**Next instrument, and it is a narrow one.** Find the writer, not the symptom: log every transition of an owner-2 villager's `targetResourceId` from non-null to null, with the system that did it. The candidate sites are already enumerated — `aiDecisionOps` (the rebalancer donor clears target and task when it reassigns), `toResourceStep` (depleted and unreachable branches), `villagerEconomySystem`, `productionQueueSystem`, `bridgeHelpers` — so this is a matter of tagging each write, running 200 ticks, and reading which one fires. Suspect system ORDER first: if a system that clears targets runs after the one that assigns them, every villager is re-orphaned on every tick, which is exactly the 8%-gathering signature.
