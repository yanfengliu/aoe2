...
**The open question, stated carefully.** Why does a villager with a target and free near nodes end up assigned to, or walking toward, something far away? Look at the selection inside `assignNearestResource` — `preferUnsaturated`, `spreadCap`, and the `probeFrom` ordering — and at whether the chosen target is genuinely the nearest reachable one. Measure the DISTANCE of each assignment at the moment it is made, comparing owner 1 and owner 2 on the same map; that single number separates "assignment picks badly" from "the walk itself is being lost".
