...
**How it is checked from now on.** `lastSeenStatic` is written only when an entry's APPEARANCE changes (`sameAppearance` excludes `lastSeenTick`), and the ops split reading from dirtying — `getOrCreateMemoryMap` no longer marks the slot, and a caller that writes calls `noteMemoryChanged()`. Measured after: that slot 358 MB → 2.0 MB, the bundle 537 MB → 181 MB, three times under the ceiling. Nearest-first is now passed explicitly for a walk to an in-progress construction site and nowhere else (`findBuildingApproachPlan`'s `nearestFirst`, set from `playerCommandsSystem` on a `build` command; the cache key carries the flag). `aiEconomyKeepsMoving.test.ts` is the corpus gate's cheap twin — owner 2's stockpile must move over 6,000 ticks, and every owner must leave the Dark Age — so this class fails in a minute locally instead of in CI. The corpus horizon moved from 13,000 to 20,000 ticks, derived from the measured transitions (boot map 14,100/13,800, `default-seed` 10,100/10,400) rather than guessed.
...
## 2026-09-02 — An AI economy freezes byte-identical on `ai-feudal-stone-fixture` at about tick 15,000 (OPEN)

**Symptom.** Found while re-calibrating `aiFeudalStone.test.ts` on 2026-09-02, not by play. On `ai-feudal-stone-fixture` owner 2's economy stops: all four resources go byte-identical (food 40, wood 135, gold 612, stone 220 in one run; food 30, wood 125, gold 425, stone 330 in another) and the villager count pins, from about tick 15,000 through 24,000 — the end of the probe. Villagers keep their desired-resource assignments (9 food, 2 stone, 6 gold, 10 wood) while gathering nothing.

**Why it is not that day's change.** Reproduced with the day's approach-ordering change both ON and OFF, identically. The test never saw it because its window ends at tick 14,000 — 11,000 ticks of warm-up plus a 3,000-tick measurement — about a thousand ticks before the freeze. Widening the window to look for a bigger number walked straight into it, which is how it was found.

**How it is checked from now on.** It is NOT, yet — that is the fix's job, and this entry is the standing record that a gate passing at 14,000 says nothing about 15,000. `aiEconomyKeepsMoving.test.ts` holds the same property on the BOOT map (the stockpile must move across a 6,000-tick window), and the same shape on this fixture is the natural extension once the cause is known. The likely relative is the boot-map deadlock above — villagers stacked on one cell, latched in `to-resource` — which had a different trigger but the same signature, so the position trace is where to start.
