...
## 2026-08-29 — On the map the game boots, one AI never leaves the Dark Age (OPEN)

**Symptom.** `aoe2-prototype` is `DEFAULT_SEED` — the map a player lands on. Run it all-AI and owner 1 reaches Feudal around tick 16,000 while **owner 2 is still in the Dark Age at tick 24,000** (40 minutes of game time). Owner 2's villager count does not sit flat the way a starving economy would; it OSCILLATES — 9, 6, 3, 7, 9 — with food pinned between 20 and 40 the whole time, which is the signature of income going straight back into replacing dead villagers at 50 food each. Ten villagers are lost in clusters between ticks 6,000 and 9,000. Reproduced on `corpus-seed-b` as well (owner 2 censored at 20,000 ticks), so it is not one unlucky map.
...
**Ruled out, each by measurement.** It is NOT enemy raids: the Town Centers are 43 tiles apart, both players hold only 1-2 military units through the window, and BOTH players' building counts grow (3 to 7) — nobody is being razed. It is NOT ordinary gather assignment walking villagers onto a live boar: read at component level off `gatherer.targetResourceId`, zero villagers ever target one. It is NOT villager auto-aggression: wildlife are resources, not units, so they are not in the `unit` query the stance system scans. The boar profile cannot start it either — `autoAggro: false`, 75 HP, 7 damage, aggro range 3; it only ever retaliates.

**Still unknown: what kills them.** Two boars sit at (46,27) and (45,27), three to four tiles from owner 2's Town Center at (48,24), and villagers are repeatedly within 3.5 tiles of them. The AI hunt phase dispatches exactly `HUNT_PARTY_SIZE = 4`, which matches the cluster size, and its own comment says the party is picked nearest-first "so the party arrives together rather than trickling in one at a time and being killed one at a time" — a guarantee that was written under the pre-v0.3.160 movement clock and may not survive honest walk speeds. That is a hypothesis, NOT a finding: the probe that would have confirmed it read `getRenderState()`, which is fog-filtered to the human player, so owner 2's boars and villagers were absent and the result was empty rather than negative.

**Next step for whoever takes this.** Instrument the KILL, not its surroundings: log the attacker on every villager death for owner 2 (combat/damage path), from `getEconomyState` or the world components — never from `getRenderState`, which is a per-player view. That single log answers in one run what six indirect probes could not.

**What this predicts.** A gate written as "did anyone progress" cannot see the player who did not, and every such `some()` in a check deserves re-reading as a possible blind spot. And a fixture family that only ever asserts the WINNER's trajectory will keep hiding the loser's.
