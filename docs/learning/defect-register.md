...
Two things constrained that choice, both worth recording. The default-seed row CANNOT carry the strict gate: its owner 1 needs 15,800 ticks (a regression this fix caused, see below), and raising that row's budget to 18,000 makes the harness crash outright — `JSON.stringify(result.bundle)` throws `RangeError: Invalid string length`, so replay bundles have a hard length ceiling somewhere under 18,000 ticks. And the gate was RED-CHECKED end to end rather than trusted for printing "ok": temporarily requiring `castle-age` made the corpus exit 1 with "a living owner never reached castle-age (behind: 2:feudal-age, 1:feudal-age)". A gate that has only ever passed is not known to work.

**Measured across three seeds.** `aoe2-prototype` owner 2: **never (>24,000) → 9,300**, and it is now the faster player on that map. `corpus-seed-b` owner 2: **never → 15,500**. The cost, recorded rather than buried: `default-seed` owner 1 slows 12,200 → 15,800, because the old short timeout had been supplying accidentally-useful fan-out there. Two unbounded freezes against one bounded slowdown. Two slack values were measured (1.6 and 1.25) and the better one kept.
