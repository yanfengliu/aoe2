...
- **Self-improvement ledger:** added `playtest:self-improve` as the canonical command for turning saved LLM playtest artifacts into a JSON/Markdown ledger. It recovers shared `ImprovementFinding` payloads, records replay self-check evidence, classifies proposal/fix/observe routing, and compares baseline/current metrics through the shared civ-engine comparator. Dogfood on campaign-10 -> campaign-11 recovered 3 standardized findings and recorded a replay self-check failure as blocking evidence rather than overclaiming success. No gameplay/user-visible version bump.
