...
- **Upgrading a unit no longer takes away your civilization's unique technology.** The Celts' Siege Onager upgrade used to strip Furor Celtica's extra hit points from every Onager it upgraded (a 90-hit-point Onager became a 70-hit-point Siege Onager instead of 105), and the Capped Ram upgrade did the same to Battering Rams. The Mongols' Drill stopped speeding up the same two, and the Britons' Elite Skirmisher upgrade removed Yeomen's extra range. They now keep it, as in Definitive Edition.
- **The Siege Onager throws a stone.** Its shot used to be drawn as an arrow and fly at an arrow's speed; it now looks and flies like the Onager's.
- **Cannon Galleons and Turtle Ships fire cannonballs**, drawn like the Bombard Cannon's, as in Definitive Edition. They used to be drawn as arrows.
- **Skirmishers, Scorpions, Cannon Galleons and the Trebuchet have their minimum range**, as in Definitive Edition, their elite versions too: a Cannon Galleon holds its fire inside 3 tiles and a Trebuchet inside 4, buildings included, a Scorpion will not fire at a unit on its own tile or on the four tiles beside it, and a Skirmisher will not shoot a unit standing on its own tile. Tiles are counted along rows and columns, so a unit diagonally beside a Scorpion is 2 tiles away and can be shot; and a Skirmisher can still shoot a unit fighting it from the next tile, which Definitive Edition's cannot.
- **A unit no longer freezes on a target it cannot hit.** A unit with a minimum range, left to pick its own target, now passes over an enemy inside that range and takes the best one it can fire at, as a tower does. A computer player no longer orders its units at such an enemy, and switches them to another target when one comes that close. Until now a Mangonel, Onager, Siege Onager or Bombard Cannon that took an enemy right beside it held its fire at it for as long as the enemy stood there. An enemy that walks inside the minimum after your own unit picked it still holds the unit's fire until it dies or steps back out; give the unit a new order to free it.
- **A siege unit bitten by a wolf beside it no longer turns on the wolf**, since it cannot fire that close. The Mangonel line and the Bombard Cannon used to lock onto the wolf, which also kept them from picking any other target; the Scorpions and the Trebuchet used to shoot it point-blank.
- **The Elite Skirmisher aims like the Skirmisher.** It used to hit with every shot and loose it at once; it now has the Skirmisher's 90% accuracy and half-second wind-up, as in Definitive Edition.
- **With Anarchy, the Goths' Barracks trains Elite Huskarls once the Elite Huskarl upgrade is in.** It used to go on training base Huskarls.
- **The Capped Ram hits like the other rams.** Its blows on units now count against melee armour, as the Battering Ram's and the Siege Ram's do, so a Skirmisher takes 3 damage from it, not 1. In a formation it now leads with the other melee units.
- **Saves and replays.** A Siege Onager, Capped Ram or Elite Skirmisher already upgraded in a saved game keeps the hit points and range it was saved with; ones upgraded or trained after loading get the bonus. Replays recorded before this version are re-simulated under the new rules, so one in which any unit changed here fights, or a siege unit is bitten by a wolf, may not play out as it was recorded.
...
- **In a team game, an ally's stone or charge landing among your villagers warns you too.** It is damage to your town by another player, so it sounds the horn like an enemy's.
...
- **Unexplored ground is black, and the map no longer gives it away.** Ground you had not explored was drawn at a very dark 12% of its real colour. That was too dark to notice, but brightening a screenshot showed every lake and forest on the map, and the minimap had the same leak. The cells at the edge of what you had explored were also shaded by what lay beyond them, so their colour could outline a hidden forest or lake one tile deep. Unexplored ground is now plain black in both art styles and on the minimap, and nothing you have not scouted changes how the ground or the minimap is drawn. (The building outline you drag while placing a building still turns red over water and forest you have not explored. That is known and will be fixed separately.) Ground you have explored but cannot currently see is still drawn dimmed, as before.
...
- **Escape cancels a building placement.** Pick a house by mistake and there was no way to put it down again: Escape opened the game menu with the house still on your cursor, clicking the card a second time did nothing, and every click on the map was eaten as another attempt to place it. Now Escape puts it away and leaves your villagers selected, and clicking the same build card again does the same. The foundation outline follows your cursor properly too — it no longer sits stranded in the corner of the screen before you have moved the mouse, and it no longer offers you the edge of the map when you are pointing past it.

- **Escape closes whatever is on top.** With the technology tree open over the game menu, Escape used to close the MENU — leaving the tech tree on screen with no way out but its ✕, and the match running again behind it. Now the thing you are looking at is the thing that closes, and the menu underneath it stays where it is.

- **Double-clicking a villager selects all your villagers again.** It worked for a villager standing on their own, and picked the Town Centre or the sheep behind them for any villager who was not — which, in your own base, is most of them. Two quick clicks now always mean "select every one of these on screen"; clicking the same spot more slowly still cycles through whatever is stacked there.

- **Right-clicking the minimap sends your army there.** It did nothing at all before. Left click still moves the view.

## 0.3.222

- **The computer player spends what it is sitting on, and spends it on a plan.** Late in a long match a computer player would end up with thousands of one resource it had no use for while it had run out of the one that gates almost every unit it can build — one seat finishing a hundred-minute game on 4,681 gold with 58 food and 18 wood, unable to train a single soldier, and the other on 1,093 stone with 23 gold, its infantry still at the lowest tier because every upgrade costs gold it did not have. The computer has had a Market all along and used it for exactly one thing: getting over the hump into the next age. It never used it again after reaching the Imperial Age, because there is no next age to age up to. Now, once it is in the Imperial Age, it goes to its Market and trades — and it trades toward the building it is stuck on or the upgrade it is saving for, not just toward the next soldier, so it stops buying more of fewer things. It also buys an upgrade before it buys another soldier. Its late army is bigger, it is armoured and upgraded rather than whatever happened to be cheap, and the resource piles it could not spend are gone. One thing the measurement turned up that is not this change's doing: a computer player with a bigger economy runs its own villagers out to whatever is left to gather, and on a crowded map what is left is inside the other player's base, where they get shot. That was already true; a stronger economy just gets there sooner. It is recorded separately and is not fixed here.

## 0.3.221

- **Your units walk across farms now, the way they do in Age of Empires II.** A farm was a solid wall: a soldier ordered to the far side of one went round it, and where farms filled the only gap he never arrived at all. A Town Centre with farms all round it could not put down the villager it had just trained. The computer players build up to eight farms each once they are past the Dark Age, so on a crowded base their own farms were the most numerous piece of wall on the map — and their villagers were walking round every one of them. A farm is ground with crops on it now: every land unit of every player crosses it, stands on it, and is drawn standing on it rather than shoved onto the tile next door. Nothing else about farms changes — you still cannot build on one, they still cost the same wood, hold the same food and reseed the same way, and your own buildings still block exactly as they did.

- **The computer players no longer wall the map shut.** Their buildings could cut the map into pieces that nothing could walk between. On Black Forest, twenty minutes in, not one enemy unit was standing anywhere your army could reach — you could select everything you owned, order it across the map, and no battle would ever happen. On the tighter maps your own villagers ended up sealed into pockets a few cells across, with their sheep and their woodline on the other side of a wall of houses. The computer already had a rule against this — do not put a building where it would close the last gap — and a second rule underneath it that threw the first one away whenever nothing else fitted, on the grounds that building in a bad spot beats not building at all. On a map whose only free ground is a corridor, that filled the corridor every time. Now the computer looks twice as far from its Town Centre for somewhere that does not close a gap, and if everywhere in reach would close one it waits and builds later, which is what a person does. It also remembers what it decided a moment ago, so two buildings put down in the same breath can no longer close a gap between them that neither of them closes alone. Nothing changes for your own buildings: you may still wall yourself in on purpose, and walls and gates are exempt, because closing ground is exactly what they are for. One more thing came out of the measurement rather than the intention: the computer now gets FURTHER into a match than it did with the old rule. Played out to the end on the standard map with the fighting turned off, both computer players now reach the Imperial Age, put up thirty-four and thirty-eight buildings and research twenty-nine technologies; under the old rule one of them never left the Castle Age, and between them they built eight fewer buildings each and six fewer technologies. Walling yourself in turns out to be worse for your own economy than waiting for a better spot.

## 0.3.220

- **A building's health bar is its construction progress again, so a full bar means finished.** Put more than one villager on a building and its health bar filled up long before the building did — with three builders it read FULL at 56% built, with eighteen at 37% — and then sat there saying nothing for the rest of the job. That made three different things look identical: a site still going up, a site everybody had wandered off from, and a building that was actually done. It also made the age-up button look broken, because advancing an age counts completed buildings, so you could be looking at a row of full health bars and be told you owned none of them. The cause was that health and build progress were counted separately and at different speeds; health is now read from the progress itself, so it rises steadily with the work and reaches full exactly when the building does. Nothing about build speed, cost or the multi-builder rule changes — only what the bar tells you. Foundations still stand up at 10% health as before, and a foundation an enemy has damaged now mends at the rate your villagers are building it rather than jumping back up the instant one arrives.

## 0.3.219

- **A move order the game cannot carry out now moves your units and tells you why.** Select an army, click a spot it cannot reach, and until now nothing happened at all — no step, no message, no error. The order was accepted and then thrown away in silence, and a unit walled in far from where you clicked would sit there for the rest of the match. Two things change. Your units now walk as far toward the spot as they can get, stopping at the reachable cell closest to it, and a message tells you what is in the way and what would fix it — "Cannot reach (55, 30): forest and a town-center (building) block every route out from where all 4 selected units stand. Clear a path, or pick a cell on their side of it." The message appears once for the order you gave, not once for each unit, and if only part of your selection is stuck it says how many are stuck and that the rest are on their way. This applies to plain move orders and to attack-move. Nothing changes about orders that already worked: an order the game could carry out before ends in exactly the same cell it used to. Orders across a wall also got much faster to work out — from about 163 milliseconds down to about 1 — which matters because an army of fifty in that state used to be enough to stall the game. One honest limit: this reports and works around a blocked map, it does not unblock one. If ordinary building has cut your side of the map off from your enemy's, your units will now walk to the wall and say so, which is the correct behaviour and still leaves you walled in.

## 0.3.218

- **A raided player is warned twice as often.** The "your town is under attack" warning — the horn, the flashing minimap mark, and the camera jump on Space — is spaced apart on purpose, so that a long fight does not turn into a stream of horns. That spacing was meant to be twenty seconds and had been forty since the warning first shipped, because the number was written for a game clock running twice as fast as this one does. It is now the twenty seconds it was always meant to be. In practice: an enemy that keeps hitting your villagers will now get your attention twice as often, and a raid you miss the first warning of is half as likely to go unanswered. Nothing else about the warning changed — it still fires only when another player's soldier hits one of your villagers or buildings, and hunting a boar or fighting a pitched battle still raises nothing. One honest limit, and it is a gap rather than a change: the spacing is purely a matter of time, so a second raid on the far side of your base is still silenced if it lands inside the twenty seconds, no matter how far away it is.

## 0.3.217

- **The game tells you when your villagers are being killed, and now it tells you when your Town Centre is too.** An enemy soldier could walk into your base and hit one of your villagers, and the screen would say nothing at all — no message, no marker, nothing to look at. There was a horn, and a horn is the one warning a muted tab, a pair of headphones on the desk or a noisy room takes away from you. Now the minimap flashes a bright pulsing mark at the spot the blow landed, it stays up for eight seconds so you can look up a moment later and still find the raid, and Space still jumps the camera there. The mark shows even if you have the sound muted. Separately, the warning's own rule had a hole in it: it compared where the blow landed against where your buildings start rather than where they are hit, and for the four largest buildings in the game — the Town Center, the Market, the Castle and the Wonder — those are far enough apart that an attack on any of them raised no warning at all. Attacks on those now warn like everything else. The rule is otherwise deliberately narrow, so the warning stays worth reacting to: it fires when another player's soldier hits one of YOUR villagers or buildings. Hunting a boar that fights back does not warn you, a pitched battle between armies does not warn you, and your own Town Centre shooting back does not warn you. Warnings are spaced apart the way they always were — 400 ticks, which is about forty seconds of game time at normal speed — and this release did not change that.

## 0.3.216

- **A unit standing behind a building now looks like it is behind the building, and in its owner's colour.** When one of your units walks behind a house or a Town Centre the game draws its outline through the wall so you do not lose it. That outline used to be a flat, solid, unlit block of colour pasted on the building: a unit behind a house read as a unit standing on its roof, and the very first thing you saw on the opening map was a bright blue figure stuck to the Town Centre's wall. The colours were wrong too — your own units were a sky blue that is nobody's player colour in particular, and everyone else's were magenta, which is nobody's at all, so an enemy, an ally and a neutral all looked the same and none of them looked like themselves. The outline is now translucent and lit like the rest of the world, so the building shows through it and the shape reads as something behind rather than something in front, and each of the eight player colours gets its own: a red player's unit behind your house is red, a green player's is green, and yours is your own blue. Nothing about what the cue means changed — the same units are shown through the same buildings, under the same fog rules — and it costs no extra drawing work.

## 0.3.215

- **The game can be started from a short window.** Opening the game with no settings in the address bar lands you on the match setup screen, and that screen is the only way from a bare link into a match. In any window shorter than about 600 pixels — a 1366x768 laptop once the browser's own chrome is counted, or a window snapped to half a tall screen — the panel was taller than the window and neither end of it could be reached: the "Skirmish" heading was cut off above the top of the screen, the Start button sat below the bottom of it, and nothing scrolled. There was no other route in, so the game simply could not be started. The setup screen now scrolls when it needs to and stays centred when it does not, so every control on it — map, players, civilization, teams, difficulty, resources, victory, population cap, speed, and Start — is reachable at any window height. It also no longer runs off the side in very narrow windows. Nothing changed at heights where it already fitted: at 800x600 the screen is pixel-for-pixel what it was.

## 0.3.214

- **A match you lost no longer tells you that you won.** Losing a game used to end it with the words "Conquest Victory" printed in small grey text in the bottom-left corner — the same words a win printed, because the game was only ever saying HOW the match was decided and never which way it went. It also said nothing about who had won, and there was no dialog of any kind, so it was possible to finish a match and not notice it had ended. Now the end of a match announces itself: the screen dims, and a panel says plainly whether you won, lost or drew, how the match was decided, and who won it — you, a named opponent, or, in a free-for-all decided by conquest, that your opponents did. A draw says nobody won rather than picking whoever was ahead on score. The scores stay on the panel with the winner marked, there is a Close button, and the small end-of-match card in the corner stays behind as the record once you have dismissed it, now carrying the same corrected wording. One honest limit: when you lose by conquest to more than one opponent the game will tell you that your opponents won, not which of them, because it does not yet record that.

## 0.3.213

- **The opponent finishes the buildings it pays for, and no longer stalls on something it cannot afford.** Two things used to stop the AI dead without stopping it visibly. If a builder was ever knocked off a site — one bad tick was enough — nobody was ever sent back, so the half-built shell stood there for the rest of the match; and because the AI counted that shell as a finished building, it never ordered another one. In one measured match an opponent's Siege Workshop sat at a quarter built for over half an hour of game time, which is why it never fielded a ram, never fielded a mangonel and never researched a single siege upgrade. Separately, the build order was being read as a strict queue: an opponent that could not afford the next thing on its list built **nothing at all** until it could, so one seat spent its whole Castle Age with almost no wood while sitting on enough stone for three Castles, and never advanced an age. Both are fixed. The AI now re-crews any site of its own that has nobody working on it, and it builds the first thing on its list it can actually pay for — still saving toward its real first choice when it can afford none of them. Expect opponents that finish what they start, keep advancing, and reach their siege units.

## 0.3.212

- **A building you cannot reach is refused instead of eating your wood.** Placing a building on ground no villager could walk to used to show a green preview, take your resources, and leave a foundation that could never be finished — no message, no refund, and the villager you sent simply went idle. Worse, an unfinished foundation is a solid obstacle, so doing it a few times walls you into your own base: in one real match it happened eight times for 440 wood, and cut the ground that player could still reach from 1,353 tiles to 106. The game now checks that a villager can actually walk to the site before it accepts the click. A site nobody can reach is refused up front, with the building, the tile and the reason named, and the nearest ground a builder CAN reach suggested — and nothing is charged. Refusals are more honest in the other direction too: the old check was rejecting some placements for the wrong reason, and four of twelve refusals in that same match named ground that was fine. The same rule now binds the computer opponent, so it can no longer strand its own foundations either.

## 0.3.211

- **The game now says when it has stopped.** A match that broke used to look exactly like one that was running: the clock and the units froze, but the HUD kept drawing resources, age, population, the minimap and a ticking match timer over a world that was already dead, with no banner, no message and no way back except reloading the page — one observed match sat frozen at tick 29,297 with nothing on screen admitting it. A stopped match now puts a notice in front of you the moment it happens. It says the match cannot continue, names in one line what failed (and, where the simulation reports it, the tick it failed on), says plainly that nothing you did caused it, and offers a Reload button. The frozen match stays on screen behind it, so you can still look at where things stood, and you can still load a saved game after reloading. Paused, finished and broken are now three things you can tell apart. This also covers the second, quieter way a match could stop — a failure the simulation caught for itself, which until now was recorded internally and shown to nobody. Nothing about a healthy game changed: no rule, no pacing, and no pixel while the match is running.
...
- **Villagers chop the trees they can actually walk to.** Automatic gather assignment ranked trees by straight-line distance to the drop-off, while units walk around obstacles — and a forest is nothing but obstacles. Villagers were sent to trees that looked five cells away and were a thirty-four-cell walk, past trees they could have reached in two, and they spent two thirds of their time walking. Trees are now ranked by the walk a villager really makes to the nearest drop-off, and a tree nothing can reach is never chosen. On the standard map the computer's first lumberjacks delivered 110 wood in the first ten minutes; they now deliver 340, and across the shipped maps the share of wood targets the ranking got wrong falls from as much as seven in ten to a few in a hundred. This applies to your own villagers when they pick their next tree after finishing one; an explicit order to a tree is still honoured at any distance. Simulation speed does not suffer: the walk field costs a quarter of a percent of tick time at most. Known and recorded: on Nomad, a map with almost no reachable wood, the better ranking sends every lumberjack to the same two trees and the return walk then jams them against each other, so the computer gathers less wood there than before (about two fifths less for one seat, a tenth less for the other); the fix for that return walk is built and waiting on a decision about two self-play gates.
...
- Two more places where the computer could pick a unit that is inside a building and then be unable to do anything with it: its army count treated sheltering soldiers as ready to march, and it could send a villager taking cover under the town bell to lay a foundation it could never walk to.
...
- The computer gathers far more wood on Arena, Fortress and Gold Rush, because its Lumber Camp finally goes up at the trees. It would only consider a woodline within 12 tiles of its Town Centre, and those three maps deliberately keep every woodline at least 14 tiles clear of a start so a patch of forest cannot punch a hole in Arena's wall — so it never found one, planted its camp beside the Town Centre, and then walked every load fifteen tiles home. On Arena it had 19.6 villagers assigned to wood and only 3 of them ever actually chopping; that is now 5.4, and the wood it gathers over a 100-minute match goes from 2,964 to 4,076. Maps whose trees are already close by are unaffected: the change can only bite where the computer could not see a woodline at all.
- An opponent who hides its last villagers inside a building can now be finished off. The computer prefers to hunt the enemy's villagers, and a garrisoned villager is still alive — but it is inside a building, so there is nothing to walk to and nothing to hit. The order was quietly dropped and re-issued every few seconds forever: on Gold Rush an army of 151 stood idle for the last 50 minutes of the match, 45 tiles from an opponent that had no units left and 14 buildings standing. The same thing happened one step earlier, too — the computer waits for a full attack group before committing to a march, and that wait applied even to an opponent with no army left to be caught by, so a force cut below the threshold sat at home rather than finishing a base that could not fight back. Across the four audited maps, matches that reach a result go from one in four to three in four.
...
- A building's shadow now falls on its own base. Nearly every building here stands on a stone plinth wider than its walls, and with the sun as high as it is, most of a building's shadow lands on that plinth rather than on the grass — which nothing was drawing, so big buildings looked like they were sitting on the map rather than standing on it. The part of the shadow that lands on the plinth is now drawn on the plinth.
- Where two shadows overlap they still do not get darker or break into stripes, including under a crowd packed onto one tile and in a woodline being felled. Shadows still do not turn when a unit turns, fogged buildings you only remember still cast nothing, and nothing about picking, selection, fog or saved games changed.
