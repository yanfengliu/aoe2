...
- `src/` — game code (TypeScript)
  ...
  - `game/` — gameplay rules, scenarios, content
    ...
    - `simulation/` — simulation bridge, scenario setup, command handlers.
      ...
      pass without changing what is claimed; `syncBuilding` takes a CLAIM KIND
      rather than assuming every structure is a wall),
      `passableStructures.ts` (v0.3.221 — the one decision of which
      structures a unit walks across: `structureClaimKind` returns `'farm'`
      for a farm and `'building'` for everything else, because the engine's
      sub-cell grid refuses a unit slot on any cell an entity occupies, so a
      farm has to stop claiming rather than merely stop blocking),
      `worldOccupancyCells.ts` (`blocksWholeCell` — the ONE list movement,
      spawn, wildlife and the group allocator read, and the one place movement
      and placement diverge: a `'farm'` claim is absent from it while
      `isPlacementBlocked` still counts it, so nothing may be built on a farm
      that every land unit walks over), `spawnPassabilityMemo.ts` (the
      ...
      - `bridge/` — helper modules factored out of `createSimulationBridge.ts`. After Phase 4 + Phase 5 of the createSimulationBridge shrink, the orchestrator is a ~500-LOC facade that delegates world construction, render projection, and command dispatch to the modules below. Side-map ownership lives in `bridgeState.ts:createBridgeState()`; `createWorld.ts` instantiates it once and threads the same references through every dep-bag factory so save/load and destroy-entity hooks see consistent state.
        ...
        State + types tier:
        - `bridgeState.ts` — single `createBridgeState()` factory that owns bridge-only runtime caches and side maps that are intentionally not Tier-1 serialized state (`movePathCache`, `monksByOwner`, `monkConvertProcessedThisTick`, `pendingCommands`, `unitAttackFeed`, `playerHits`). Phase 2D moved the Tier-1 codec slots, including `monkTasks` and `unitCommands`, into `world.state.aoe2.<slot>` through `BridgeStateAccessor.mutate(codec, ...)`; Phase 2F persists `pendingCommands` through `aoe2.pendingCommands` at save time so AI intentions queued between ticks survive schema-2 save/load. The bounded attack feed is mirrored into `aoe2.replayUnitAttacks` only for recorder checkpoint fidelity and is stripped from ordinary user-save snapshots; hydration inspects only the final 1,024 candidates, validates and canonicalizes fresh in-bounds records, and retains only the latest record per attacker identity. `playerHits` (`playerHitFeed.ts`, v0.3.235) is the attack warning's source: every blow that lands on a player's unit or building, whatever dealt it, recorded where the damage is dealt (melee in `blastDamage.ts` and `attackDelivery.ts`, every landing shot in `projectileOps.ts`, every blast victim in `applyUnitBlast`, wildlife bites with no owner). The damage functions require the recorder, so a caller cannot omit it. It is transient with no codec: a load starts it empty, and a replay step holds the blows re-dealt since the snapshot it re-simulates from. The swing feed keeps the animation and still carries `participants`, which nothing reads since the warning moved: they stay because the swing feed is copied into replay snapshots, and dropping them would make bundles recorded before v0.3.235 diverge on re-simulation.
        ...
        - `humanSight.ts` — a player's SIGHT: itself plus the owners whose vision it shares (allies from the first frame, everyone with Spies), and the cell, footprint and witness-list questions asked of that set. Every human-facing visibility question — the render filter, fog memory, click finders (the Monk's ground route included), animation filters, the replay bridge — goes through it; the per-owner records (the visibility map, witness lists) and the rules of play stay per-owner. `tests/architecture/humanSightQueries.test.ts` refuses the human's id passed straight to a visibility query; the behavioural gates it names hold the rest.

        Systems tier (`bridge/systems/*` — 20 ECS factories):
        - `aiSystem.ts`, `autoAggressionSystem.ts`, `playerCommandsSystem.ts`, `monkBehaviorSystem.ts`, `productionQueueSystem.ts`, `scoutMovementSystem.ts`, `villagerEconomySystem.ts` (with `dropOffStep.ts` and `idleGatherAssignment.ts` — the latter owning what a gatherer with no workable target does next: the resource-kind fallback, safe-first since v0.3.191 and taken by EVERY automatic assignment site including the post-delivery one, and the reachability retry cadence — and, one tier down, `bridge/enemyDefenceRange.ts`, which answers where a COMPLETE enemy static defence's arrows reach and owns the range arithmetic `towerCombatSystem` fires with. `villagerGatherAssignment.ts` and `targetFindingOps.findNearestDropOffBuilding` read it through a per-tick, per-owner memo (`createEnemyDefenceLookup`, keyed on world and tick — there are THREE instances, one per consumer including the drop-off walk field's safe-drop-off narrowing in `wirePreSeedOps`, so a building completing mid-tick can be visible to one and not another for the rest of that tick); `aiSystemHuntPhase.ts` collects it directly and uncached, because it runs only on the AI decision interval and behind two early returns, plus `bridge/enemyDefenceRetarget.ts`, which moves a committed gatherer off a node that has come under enemy arrows on the reachability retry cadence), `wildlifeCombatSystem.ts`, `herdableMovementSystem.ts`, `herdableOwnershipSystem.ts`, `visibilitySystem.ts`, `fogMemorySystem.ts`, `towerCombatSystem.ts`, `relicGoldSystem.ts`, `garrisonHealSystem.ts`, `wonderCountdownSystem.ts`, `relicCountdownSystem.ts`, `winConditionResolverSystem.ts`, `conquestOutcomeSystem.ts`, `scoreTimerSystem.ts`. Each factory takes a typed deps interface and registers exactly one `world.registerSystem({...})`. Execution order is driven by explicit `before`/`after` deps, not registration sequence. `aiSystem.ts` stays the single registered AI factory but delegates its per-tick decision work to sibling phase modules (`aiSystemGating.ts`, `aiSystemBuildingPhase.ts`, `aiSystemProductionPhase.ts`, `aiSystemAttackPhase.ts`, with shared context types in `aiSystemTypes.ts`).

        Helper-ops tier (factories used by either systems or the input surface):
        - `playerQueries.ts`, `aiDecisionOps.ts`, `targetFindingOps.ts`, `selectionFinders.ts` — read-side queries. `dropOffBuildings.ts` is the one enumeration of an owner's complete drop-offs for a kind, read by `targetFindingOps.findNearestDropOffBuilding` and by `dropOffWalkField.ts` — the multi-source breadth-first field of true 4-connected walk distance from every cell to the nearest drop-off, cached per owner, kind and movement domain on `structuralRevision` PLUS the drop-off set (a camp completing bumps no revision). `villagerGatherAssignment.ts` ranks candidates and bounds the home range by that haul (the deposit leg in `systems/dropOffStep.ts` still resolves by Manhattan distance and A*; the field-driven version waits on branch `deposit-walk-field`). The field is built in `wirePreSeedOps` and rides on the movement-plan ops object because that object is what every system dep bag spreads.
        - `entityCreateOps.ts`, `entityDestroyOps.ts`, `transformOps.ts`, `movementPlanOps.ts`, `movementTrafficOps.ts`, `placementOps.ts`, `trainingMarketOps.ts`, `aiSitePlacement.ts` — write-side entity/state mutators. `aiSitePlacement.ts` (v0.3.221) is WHERE THE AI PUTS A BUILDING, split out of `trainingMarketOps.ts` for the 500-line budget and because the site search is its own subject: it applies the three rules that stop the AI stranding itself — do not build where no builder can GET (the placement validator's own `isPlacementBlocked` with `builderIds`, shared so the AI cannot propose a site the validator will refuse), do not SEAL ground (`placementSearch.ts`, rings 2..12 then 13..24, both guarded, and no unguarded pass), and do not forget what you asked for a moment ago (every pending `building.placeConfirm` footprint counts as blocked ground, for the footprint test and for the guard, whichever owner asked for it, because two AI seats decide on the same ticks). The seal guard walks the SAME `isCellPassableForUnit` the pathfinder and the walk-order flood use, asked about a probe that is no unit at all and memoised per structural revision, so it cannot hold a second definition of "walkable"; one consequence is that a farm reads as open ground to it. Its floods, guard refusals, empty searches and wall time are published on `getDebugSnapshot().aiSitePlacement`, so a claim about the search's cost or its refusals is a number. `moveDestinationSearch.ts` (v0.3.219) sits under `movementPlanOps.ts` and owns WHERE a walk order ends and whether the player can be told it cannot end where they asked: a 4-connected flood out of the unit's own cell over the same passability predicate `findGridPath` is handed, picking the reachable cell with the smallest `(manhattan-to-target, y, x)` — the same key the ring scan it replaces used, so an order the old scan could already carry out ends in the same cell. It also owns `describeWalkOrderReach`, which floods out of the ORDERED cell instead (reachability over a grid is symmetric, so one flood answers for a whole selection, and a selection costs at most one flood per unit DOMAIN) and names the barrier through `describeBlockedCell` — the placement validator's own `describePlacementBlockers`, threaded in from `wirePreSeedOps` so the walk-order toast and the placement toast cannot disagree about what a cell holds. `movementPlanOps.ts` keeps route planning, slot allocation and approach targets and delegates the destination question here; `humanInputOps.ts` calls the description once per order gesture. `constructionHealth.ts` (v0.3.220) is the other small owner in this tier: a building under construction has ONE quantity, build progress, and its hit points are a reading of it — `constructionStartHp` is the 10% foundation floor (read by `entityCreateOps.ts` when it stamps the foundation), `constructionEarnedHp` the value that floor rises to at a given progress, and `constructionHealthAfterWork` the per-builder-tick update `systems/builderWorkStep.ts` applies, with the earned value as a ceiling and the credited progress as the step. Health is never a second accumulator on its own clock, and the floor is defined once rather than repeated at each writer. Fine-grid movement in `transformOps.ts` publishes replacement `unitTransform` components through the ECS API on every fixed step and targets the occupancy grid's allocated slot; the component persists that numeric assignment or an explicit overflow marker so live-load and replay occupancy rebuilds preserve continuation. `movementPersistenceTypes.ts` isolates the additive movement-authority fields from the cap-pressured core component catalog. `movementTrafficOps.ts` — with the election itself, the starvation threshold and the one clock rule (`trafficProgressClock`) in `movementTrafficElection.ts` — sits between route planning and the step executor: permanent topology remains the global A* authority, while commanded/task movers in a straight or turning one-cell passage use a tick-local owner-scoped origin reservation, centered lane, and directed occupied-cell dependency closure. Four additive `UnitTransformComponent` provenance fields retain the last actual traffic leg, matching intent identity, and attempt tick so save/load can reproduce cycle decisions without serializing or consulting `movePathCache`; stale, multi-occupied, and noncalling branches fail closed. The dependency closure ELECTS one caller — the stable lowest id, unless a member's starvation clock (continuous contested waiting, restarted by a cell change or by a gap in attempting, and read from the same tick-start snapshot as every other input) has passed the threshold — and that decision is final — the per-cell cross-flow and same-flow rules are not consulted for an elected caller, because a narrower rule that vetoes the winner deadlocks the jam it was meant to arbitrate (see `docs/architecture/decisions.md`). Transient per-tick state (attempted directions, origin reservations) lives in the ops closure and never in a component: writing `unitTransform` during arbitration disturbs the per-tick snapshot the arbiter reads from it. Direct in-place mutation would bypass render dirty tracking.
        ...
        - `cellPassability.ts`, `placementBlockReport.ts`, `visibilityQueries.ts`, `visibility.ts`, `fogMemoryOps.ts` — terrain/visibility queries, raw live-entity projection, and fog-memory production. `placementBlockReport.ts` (v0.3.221) holds WHAT BLOCKS A FOOTPRINT IN WORDS — split out of `cellPassability.ts` when that file reached the 500-line budget, and re-exported from it so consumers keep one import. Its cause strings are agent- and player-facing ('water', 'a town-center (building)', 'the map edge'), and a farm deliberately reads as 'a farm (building)' even though its claim kind is `'farm'`: the player asked why the ghost is red, and the answer is the farm rather than the bookkeeping. Raw projection retains stationary entities while hidden so a later LOS change can reveal them without an ECS mutation; `renderStateOps.ts` owns the final perspective boundary.
        ...
        - `attackDelivery.ts`, `blastDamage.ts`, `animalDamage.ts`, `projectileOps.ts` — where an attack lands (spec §10.4, §10.7). `attackDelivery.ts` is the one decision, per kind of target (a unit, a building, an animal), between a blow that lands at once and a shot (`projectileOps.ts`) that lands on its impact tick; a shot at a building aims at the centre of its footprint (`footprintDistance.footprintCentre`). `blastDamage.ts` owns the blast: centred where a shot lands, or for a detonation where the unit that goes off stands (`inPersonBlastCentre`); it reaches units and animals, and a blast delivered in person spares its own side, read with the teams (`projectileRules.blastSparesOwnSide`). `animalDamage.ts` (v0.3.238) is the one place a blow on an animal is dealt, because a wolf or a boar is a resource with a wildlife state and neither the unit path nor the blast's unit query reaches one.
