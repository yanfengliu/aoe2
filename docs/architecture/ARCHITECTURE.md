...
- `src/` — game code (TypeScript)
  ...
  - `rendering/` — pure isometric projection/interpolation/view contracts and
    ...
    continuous motion rather than a move-and-stop pulse), and drives AoE-owned
...
- Render interpolation is a display concern; it does not feed back into simulation
  ...
  teleports, gaps, rewinds, bridge swaps and generation changes snap instead of
  gliding across unrelated state. Click hit-testing prefers the displayed entity so
  the player's perceived target matches the authoritative target.
