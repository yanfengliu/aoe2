...
- `src/` — game code (TypeScript)
  ...
  - `rendering/voxel/` — AoE-owned adapter, original procedural art recipes, and composed Three.js world host. `aoeVoxelAdapter.ts` orchestrates identity/epoch/revision, movement-history reset, and game-neutral snapshots only; `aoeVoxelTerrain.ts` owns flat chunk projection, quantized terrain color variation, and sparse surface props; `aoeVoxelBuildingRecipes.ts`, `aoeVoxelUnitRecipes.ts`, and `aoeVoxelResourceRecipes.ts` own role-specific rigid-part compositions; `aoeVoxelUnitAnimation.ts` owns identity-phased idle/movement profiles for named AoE unit parts; `aoeVoxelRecipeTypes.ts` owns deterministic color/hash/transform helpers; `aoeVoxelResources.ts` packs the centred group-less cube plus matte/metal/contact-shadow/memory material and animation arrays. `AoeVoxelWorldRenderer.ts` owns the sibling `voxel/three` runtime, antialiased canvas, AoE daylight values, explicit composite capture, metrics, bridge epochs, and teardown; `aoeCameraSync.ts` maps the established Phaser camera to the shared 2:1 orthographic view. The pure exhaustive unit-role mapping lives in `phaser/scenes/gameScene/unitRole.ts` and is shared by both AoE presenters. AoE visual and animation semantics remain here rather than entering the reusable package.
