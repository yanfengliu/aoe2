...
Until v0.3.235 the attack warning read the swing feed (`unitAttackAnimationFeed.ts`), which exists for the swing animation. That feed keeps one entry per attacker, stamps it when a unit swings or fires, records it only if some player saw both ends, and refuses any attacker that is not a unit. So a Town Centre, tower or Castle shooting a villager, a stone's blast on the villagers beside its target, and a bombardment of the ground never reached the warning (defect register, 2026-09-24). Three choices, each against an alternative.
