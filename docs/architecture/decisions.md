...
## 2026-09-06 — A walk order's fallback destination is found by flooding out of the UNIT, not by scanning rings around the target

The old fallback for "the ordered cell cannot be reached" asked the question from the target's side: enumerate every cell within `max(mapWidth, mapHeight)` manhattan cells of the ordered cell, in rings, and run a full A* to each until one succeeds. It has two independent faults and only one of them is a performance fault.

The correctness fault is that the radius is not a bound on anything real. On a 60x36 map the radius is 60 and the greatest distance between two cells is 94, so a unit sealed in a pocket at the far corner from the ordered cell has no candidate inside the scan it can reach — the resolver returns null and `playerCommandsSystem` clears the order without a step or a word. A radius large enough to cover the map (`mapWidth + mapHeight`) would fix that instance and not the class: the scan's cost is quadratic in the radius and it is paid entirely in FAILING A* searches, so the correct radius is the one that makes the failure mode unaffordable.

Flooding out of the unit inverts the question and has no radius at all. One breadth-first pass over the cells the unit can actually reach answers "which reachable cell is closest to the order" exactly, for any map size and any pocket shape, and it cannot run out of range because its frontier is the pocket's own boundary. Measured with both arms in one process against one bridge: a reachable order 0.89 ms → 0.02 ms, an order sealed across a wall 162.91 ms → 1.09 ms.

**The compatibility constraint, which drove the design more than the cost did.** The flood picks its destination by the smallest `(manhattan-to-target, y, x)` — deliberately the ring scan's own ordering, since rings by manhattan distance visited row-major within a ring and took the first reachable cell. That is what lets this be a fix rather than a behaviour change: every order the old scan could already carry out ends in the identical cell, which was verified by running both arms and comparing destination and path length per call rather than by arguing it from the code. `resolveWalkPath` also still tries A* to the chosen destination first and falls back to the flood's own predecessor path only when A* refuses, so a route the old code would have produced is still produced by the same router.
...
## 2026-09-10 — A walkable farm is a CLAIM KIND, not the fourth question that splits `structuralRevision`

The 2026-09-01 entry above named the trigger that would end the bare `structuralRevision` counter: "the moment a fourth consumer wants a DIFFERENT question (say 'have buildings changed but not resources?', which a farm-aware cache would)". Making farms walkable is that farm-aware change, so this is the entry that says whether the trigger fired. It did not.

Nothing here asks a narrower question. `structureClaimKind` decides once, at the point of claiming, whether a structure's footprint is a wall or walkable ground, and `worldOccupancy.syncBuilding` records the answer. Every cache downstream still asks "has durable topology changed?", and a farm going up or coming down still changes durable topology — it is a placement blocker either way, and its cells stop being buildable. The revision is bumped by claims and releases exactly as before, and no consumer learned to care WHICH kind of claim moved. So the counter stays bare and stays opaque, and all four consumers keep comparing it for equality only.

What DID have to move is where the divergence is stated. Before this, four separate places decided that a cell was blocked whole — spawn passability, wildlife passability, the spiral search and the group allocator — and a fifth, `isPlacementBlocked`, decided the same thing for placement. A claim kind that movement honours and placement ignores cannot survive five copies, so `blocksWholeCell` is now stated once in `worldOccupancyCells.ts` and the allocators' duplicate was deleted. The rule to keep: this is the ONE place movement and placement are allowed to disagree, and a sixth reader of "is this cell blocked" is a reason to read the list rather than to write another one.

The alternative — leaving the claim as a building and flipping only the passability predicate — was rejected because it does not work rather than because it is untidy. The engine's sub-cell grid refuses a unit slot on any cell an entity occupies, so a predicate-only fix routes units onto farms and then has nowhere to stand them: the showcase villager reads `slot undefined,undefined overflow true` on its own farm cell under that arm, and `slot 0.5,0.75` under this one.

## 2026-09-10 — The Imperial Market trade aims at the PLAN, not at the unit queue, and it is scoped to one age

Two arms of one A/B decided this, and the rejected arm is the reason the record exists.

**The rejected arm worked.** A Market trade aimed at the unit the AI wanted and could not pay for emptied both hoards outright — owner 1's stone 1,093 to 490, owner 2's gold 4,681 to 16 — and both armies grew. It was refused because of what it did to everything else the AI buys. The unit queue runs every decision tick and buys in 25-wood, 45-gold bites, so a stockpile the queue is drinking from never reaches the 200 wood a Siege Workshop costs, or the 220 food and 120 gold of an `iron-casting`, in one place at one time. Measured: owner 1's peak wood fell 1,005 to 725 and its peak gold 1,428 to 841 on unchanged income, and it could afford the Siege Workshop in 2 of its 122 Imperial samples against the baseline's 14. The content census moved in one direction only, every move a loss. An opponent that converts its whole economy into the cheapest unit on its list cannot break a building, so it cannot win, and it is not the opponent Definitive Edition gives you.

**So the plan outranks the queue.** `blockedBuildTarget` names the highest-priority building the owner wants and cannot pay for — deliberately a different question from `pickNextBuildTarget`, which returns whatever cheap entry is affordable; measured, the build order reads as blocked on a 60-wood farm in 63 of 122 Imperial samples while the Siege Workshop's 200 wood never forms, so before this nothing in the AI could NAME the thing it was waiting for. `marketActionForUnaffordableWant` aims at that building and at the technologies standing unbought at idle producers before it aims at the queue, and `researchOutranksQueue` buys an affordable upgrade before the next unit, because both loops are charged from one stockpile in the order they ask and a loop that asks every tick outbids a purchase made once.

**The age scope is load-bearing, not caution.** All three parts are Imperial-only. Run earlier, the same arithmetic spends the gold the AI is saving for its age-up: one seat lost both the Imperial Age and 37 of its 38 villagers over a 45,000-tick match, and the upgrade-first ordering outranks the age-up push itself and left another seat in the Castle Age at the horizon with 12 technologies against 29 and 2,768 gold banked. The rule that is right at tick 35,000 is wrong at tick 5,000, and the age is the only thing that separates them.

**A military reserve for the blocked building was measured and is NOT used.** Every Imperial unit costs wood, so reserving a 200-wood building's cost freezes the army: on the gate fixture it took one seat's army to zero and the other's to four. The same experiment exposed a dead band in the trade arithmetic — a flat 400 `keep` on every resource made the rule sell while `gold < cost + 400` and buy only at `gold >= cost + 600`, so a seat holding 436 gold could do neither and stopped trading for the rest of the match — and a permanent 400-stone floor under every seat, which is this entry's own defect wearing a smaller number. The keep is per-resource now and applies to selling only.

**What this costs, recorded rather than absorbed.** Both seats stop training rams although both still own the Siege Workshop, and the suspected cause — a nearest-first aim favouring the cheapest want — is REFUTED: the opposite aim was built, run as a third arm, and came back byte-identical across 45,000 ticks and both seats, while a counter showed the unit-queue branch reached 0 times in 1,159 calls. The queue never reaches the Market at all, so recovering the rams means changing WHICH LIST is aimed at. That is open, and so is the villager-defence gate's configuration bound that this change's larger economy exposed.

## 2026-09-23 — Unexplored ground is a constant of the terrain pipeline, not an art-style field, and fog is the absence of a cell rather than a darkening of it

The de-look plan's step-1 contract gave each art style `fog: { explored, unexplored }`. The style now carries `exploredGround` only, and unexplored ground is `UNEXPLORED_GROUND` (black) in `aoeVoxelTerrain.ts`, for two reasons.

**It is fog of war, not a look.** Unexplored ground drawn at 12% of its colour showed every lake on the map under a contrast stretch (defect register, 2026-09-23). A per-style field is a way for the next style to get that wrong, and `tests/rendering/artStyles.test.ts` now fails if any style grows an `unexplored` key.

**Darkening was the wrong operation.** The old fog multiplied an unexplored cell's real colour and then handed it to the same pipeline as known ground, so each path had to hide the real data after the fact, and three did not: the ground at 12%, the neighbour blending at the edge of the explored area (which read the unexplored cell's true kind), and the minimap's 94% wash. The adapter now NAMES the unexplored cells, and `terrainCells` draws them black and leaves them out of the neighbour lookup, as if the map ended there; the minimap never draws an unexplored cell's terrain. The rule that holds it is non-interference (the snapshot and the minimap's draw calls must not change when an unexplored cell does), not a brightness ceiling, because a ceiling would have passed the defect: its signal was two levels per channel.

**Cost, stated.** In Moebius the known cells at the edge of the explored area no longer blend toward what lies past it (38 of 164 cells at boot), so the fog edge there is a harder tile staircase than before; before, the blend dropped some of those cells a whole tone band and so outlined the hidden terrain. Soft fog edges are the DE terrain's job (the plan's step 2), and they must be computed from the fog state, never from the unexplored cell's content.

## 2026-09-24 — The attack warning's mark holds for one throttle window of simulation ticks after the LAST hit, and the words ride the horn

The minimap mark used to be a flash with its own wall-clock life (8 s), raised only when the throttled horn sounded (defect register, 2026-09-24). Three choices replace it, each against an alternative that was considered.

**The hold is simulation ticks, not wall clock.** Every other clock the warning depends on — the throttle, the attack feed, the game speed — counts ticks, and a pause stops ticks. A wall-clock life cannot agree with a tick-counted throttle at more than one game speed, and it runs while the game is paused. The pulse animation stays on the wall clock on purpose: it carries no information, and a paused frame that still moves still draws the eye.

**The hold equals the throttle, and every hit restarts it.** Any hold at least as long as the gap between two blows keeps a sustained raid lit, so a shorter hold (the old 8 s, now 120 ticks at normal speed) would also have fixed the dark 40%. The throttle's length was chosen because it gives an invariant rather than a number: the mark is dark only once a whole throttle window has passed with no hit, and then the next hit is sure to sound the horn and show the words, since the last horn is at least that old. A raid is never in progress with the minimap dark and the horn silent. The cost is that the mark stays up for 200 ticks (13.3 s at normal speed) after the last blow.

**The words are throttled with the horn, not with the mark.** A toast per hit would be a storm (the fixture's two Militia land 55 blows in 280 ticks); a toast per horn is one per window, which is what DE's horn-and-message pair does. The mark alone is unthrottled, because it is state, not an event.

**A load resets the controller, and the load path says so.** `handleLoadGame` dispatches `WORLD_LOADED_EVENT` on the HUD root and the audio mount calls `resetForNewWorld()` on it. Two signals were rejected. A tick going backwards misses a load of a LATER save, whose old horn tick can still throttle the new world's first raid. Bridge IDENTITY was built first and was wrong: a replay builds a new bridge on every step, scrub, marker jump and fog-owner switch, over the same recording, and the replayed world carries its recent swings with their participants, so every step forgot the throttle and announced the same raid again (the independent review found it before it shipped; `tests/browser/attack-warning-replay.spec.ts` holds it). Only the code that performs the load knows it is a load.

## 2026-09-24 — The DE ground is AoE's own mesh in a lent scene, its surfaces are baked textures rather than per-pixel noise, and it drops the shore surf

The de-look plan's step 2 put the DE style's ground on screen. Four choices, each against an alternative.

**A mesh in a scene AoE lends the runtime, not a vertex-coloured voxel geometry lane.** The plan's fallback, a subdivided mesh through voxel's geometry lane, needs no borrowed scene, but it cannot reach pixel-level texture, and every fog change re-uploads its colour arrays. The lent scene costs one constructor option (`ThreeRenderRuntimeOptions.scene`, measured to work by the plan's experiment E3); the runtime adds its root group and its daylight rig to it, so the ground is lit by the same rig as the models. The ground REPLACES the chunks: E3 showed a mesh laid over them z-fights, so the adapter leaves them out while the ground shows and they return as new incarnations when it hides.

**Baked textures, not noise evaluated per pixel.** The first shader drafted computed its value noise per fragment, a few hundred operations a pixel. CI's browser suite draws on SwiftShader, a CPU rasteriser, and once the DE style is the default that cost lands on every frame of every spec. Six 256x256 layers baked once per page on the CPU (about 85 ms in Node) cost the shader one mipmapped lookup per surface present, the mipmaps keep fine grain from shimmering when zoomed out, and the bytes are the same on every rasteriser. Measured on the paused default view: a median 0.8 to 0.9 ms a frame on an RTX 4090 at 1920x1080 against 1.1 ms on the voxel ground, and more frames in five seconds on SwiftShader at 800x600, not fewer.

**Fog is decided in the shader from what was packed, and unexplored ground is never packed.** An unexplored cell is kind 0 with nothing else, so the shader draws it black to the pixel and cannot blend a neighbour toward it; the soft edge lives on the known side (known ground fades to black over about 0.7 of a tile), which keeps the 2026-09-23 rule "fog is the absence of a cell" true for a soft edge. The explored level stays the style's `exploredGround`, applied to the sRGB colour as the voxel ground's `shade()` applies it: a linear factor of 0.6 dimmed explored ground so little that it read as visible.

**The shore surf goes in the DE style.** The surf strokes follow each water tile's land edges, and the DE shoreline wanders off them, so they drew a white tile outline round every pond. Moving them into the shader is the plan's step 5 (water); until then the DE style has no surf, and its water keeps the ripples.

## 2026-09-24 — Moebius is the default again until the DE ground costs no more on SwiftShader than the voxel ground

Supersedes the measurement in the section above ("Baked textures, not noise evaluated per pixel"), not its choice. That section argued that CI's browser suite draws on SwiftShader, so the ground's per-pixel cost lands on every frame of every spec once the DE style is the default, and it measured the baked ground as cheaper. The measurement was wrong on both counts: the GPU figure was a median animation-frame interval, which a pipelined GPU process holds near a millisecond, and the SwiftShader comparison ran on 32 threads and against the DE style's previous ground, not against Moebius on CI's four. On CI the DE default doubled the live frame (233.3 ms against 116.6) and failed two frame-timed specs.

**The default goes back rather than the suite drawing Moebius.** A suite that drew Moebius whatever the product's default would have hidden the same cost from a player without a graphics card, and would leave the default look's gameplay untested on the rasteriser CI has. Going back costs the owner the Natural look by default until the ground is cheaper; it is one menu row away meanwhile.

**The condition for the flip is a measurement, not a date.** The DE style becomes the default again when its ground costs no more on SwiftShader than the voxel ground, measured on CI's CPU budget against Moebius, and a gate holds that (the lesson in `docs/learning/lessons.md` names it).

## 2026-09-24 — The attack warning reads blows recorded where the damage is dealt, not the swing feed

Until v0.3.235 the attack warning read the swing feed (`unitAttackAnimationFeed.ts`), which exists for the swing animation. That feed keeps one entry per attacker, stamps it when a unit swings or fires, records it only if some player saw both ends, and refuses any attacker that is neither a unit nor a live animal. So a Town Centre, tower or Castle shooting a villager, a stone's blast on the villagers beside its target, and a bombardment of the ground never reached the warning (defect register, 2026-09-24). Three choices, each against an alternative.

**A feed of its own, not more entries in the swing feed.** Teaching the swing recorder about building attackers would have fixed the towers alone. Its one-entry-per-attacker key would still keep only one victim of a blast, and a later hit by the same attacker on a soldier would still overwrite its hit on a villager before the warning polled. Its witness rule belongs to animation and fog fairness, and the warning needs neither: a player always knows its own villager was hit. `playerHitFeed.ts` appends every blow with its target, its attacker and the tick it landed.

**Recorded where the damage is dealt, not where the attack is decided.** There are five places where hit points drop: melee on a unit, melee on a building, a shot landing on a unit, a shot landing on a building, and a blast victim. Wildlife bites are a sixth. Every one records, and each damage function requires the recorder as a parameter, so a new caller cannot leave it out. The alternative was one hook per attacker kind (towers, blasts, bombardments, detonations), and that list is exactly what went wrong: it can be one kind short and still look complete. `tests/simulation/playerHitCensus.test.ts` compares the feed with every hit point each player-owned unit and building loses, and every death but a demolition charge spent by its own blow, in an all-AI match and eleven scenarios. So a damage path added later that forgets the feed fails there if one of them reaches it; one none of them reaches is not covered. The cost is that the warning now follows the blow rather than the launch. An arrow that misses raises nothing, and a shot warns on the tick it lands rather than the tick it was loosed. For a warning, what landed is the event.

**Wildlife is recorded, with no owner.** The warning ignores it, as it always has: a lured boar is not a raid. Recording it anyway makes the census exact, with no exception list for bites.

**The swing feed keeps its `participants`, which nothing reads now.** They were removed in the first draft, and the independent review found the cost. The swing feed is copied into replay snapshots (`aoe2.replayUnitAttacks`), so a bundle recorded before v0.3.235 holds them, and a re-simulation without them diverges from that snapshot. Keeping them costs two component reads per swing.

## 2026-09-25 — The nightly LLM playtest is retired, and what went with it is what served only it

The owner: *"no API key, no nightly playtest"*. `playtest-llm.yml` came from an agent's own plan (`675771e5`, 2026-05-08, "phase 5 — CI workflow + corpus runner"), and it could play only with an `ANTHROPIC_API_KEY` secret the repository never had. In 138 scheduled nights it played no game (spec §15.7 has the count).

**The line between removed and kept is "exists only to serve the nightly".** Removed: the workflow; the corpus runner and its row file, whose one caller was the workflow (the summer campaigns ran `npm run playtest:llm` directly); the HTML dashboard, which reads only the runner's `output/corpus-llm/` summaries; `corpusLlmSchema.ts` and `corpusRegression.ts`, which only the runner imported; the `playtest:corpus-llm` and `playtest:corpus-dashboard` scripts; and the three test files that pinned them. Kept, because each serves more than the nightly: the local harness (`scripts/playtest-llm.mjs`, `src/game/playtest/llm*`, and both providers, since `anthropicProvider.ts` is how a local run uses a key if someone sets one); its checkpoint screenshots and envelope fields, which a reader of the run and `playtest:findings` use; `tests/architecture/workflowSecretExposure.test.ts`, which guards every workflow and now holds only the next key-bearing one; and the frozen `workflow--playtest-llm.yml` fixture and the recorded runs that name the nightly, because `ci:status` still judges commits that carried the workflow, and it tells runs apart by exact name, which those entries exercise.

**Reversible only by the owner.** A scheduled LLM playtest comes back only by the owner's say (local rules, the 2026-09-25 amendment to "Every feature is playtested the way a human plays"). Whoever brings one back also brings back a gate that reads its no-provider path red, because the one that did went with the workflow.

## 2026-09-25 — On a CPU rasteriser the DE ground draws one surface sample per pixel, and the renderer it has decides

This is how the condition in "Moebius is the default again until the DE ground costs no more on SwiftShader than the voxel ground" (2026-09-24) is met: on SwiftShader the DE style's default frame now costs less than Moebius's, and `tests/browser/de-ground-frame-cost.spec.ts` holds that. The flip back to the DE style is its own change. The debugging record (`docs/debugging/2026-09-24-de-ground-swiftshader-cost.md`) named four ways out. The coordinator chose the first; three choices follow from it, each against an alternative.

**A software tier, not a pinned suite, a cheaper blend or a slower CI.** Pinning the suite to Moebius would stop CI testing the default look; a cheaper blend changes what every player with a graphics card sees, to save a cost only a CPU rasteriser pays; accepting the cost is the constitution's rule on slowness (R22) broken on purpose. The tier costs a second path: CI draws a ground a GPU player does not see, which `tests/browser/de-ground-full-blend.spec.ts` answers by forcing the blend on SwiftShader and checking it draws.

**Detected from the renderer the game has, not configured.** A build flag or a URL parameter would be set by the suite and never by a player, so it could not reach the player whose browser fell back to SwiftShader, which is the one the tier is for. The WebGL context the renderer factory built is asked for its renderer name (`WEBGL_debug_renderer_info`, else `RENDERER`), and asked again when a lost context comes back, because Chromium can come back on SwiftShader after its GPU process fails. A context that names nothing keeps the blend. One override exists, a localStorage key read when the renderer is built, and only a spec and the capture script set it.

**The fields a single sample needs are filtered by the hardware, not read from a 3x3.** Nine cell reads and their decoding were about half the blend's cost on SwiftShader. The fog texture became four channels read linearly filtered (fog level, known mask, dirt, known water), so one read gives the fade beside unexplored ground, the dirt edge and the shore; the blend reads its first channel only, and draws the same pixels as before on both rasterisers. The single sample itself is read at a mip level worked out from the fragment's own derivatives, because SwiftShader's explicit-gradient read cost 0.18 of Moebius's whole frame more, and inside a loop that a quad of unexplored fragments never enters, because SwiftShader skips such a loop and an if saves it nothing.
