...
The implementation removes the fallback instead of hiding its limitations. Terrain remains elevation-zero; entity picking follows the current presented recipe-derived silhouettes, while geometry-aware raised-terrain picking is a named future voxel feature. Voxel 0.1.4's `raycastDensePaletteChunks` now proves a bounded Three-free query over caller-owned uniform dense terrain occupancy, and AoE's current 16x1x16 terrain drafts satisfy that narrow data contract. The helper is not bound to the runtime's presented epoch/revision and does not compose geometry or instance hits, so it is not a replacement for raised or moving entity silhouettes and is not wired into live flat-ground picking. Selection and death cues are currently static rigid parts, and the old white behind-building x-ray is intentionally absent until a depth-aware voxel pass exists. There is no hidden painter or alternate renderer masking those boundaries. Exact final gate, visual, and review evidence lives in `PLAN.md`, the linked detailed devlog, and `2026-07-13/7/REVIEW.md`.
...
Add elevation-aware terrain presentation and a generic presented-state query together, retaining exact terrain occupancy under the same epoch/revision fence as the pixels before using `raycastDensePaletteChunks` in live input. Keep AoE's recipe silhouettes for geometry and moving instances unless a future query explicitly composes those lanes. Then consider depth-aware unit visibility and richer event-synchronized animation from measured gameplay needs. Split static world data from interpolated unit/interaction deltas before scaling scene size. Prove reuse with one City instance-batch consumer and one Townscaper geometry-resource consumer before generalizing the AoE host. Advanced meshing begins only after the documented Voxelize versus `block-mesh-rs` bake-off.
