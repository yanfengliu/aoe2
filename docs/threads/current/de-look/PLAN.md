...
Status (2026-09-25, software tier): the coordinator chose the software tier, and it shipped in v0.3.237. When the game's own WebGL context names a CPU rasteriser (SwiftShader, WARP, llvmpipe and the like), the Natural ground draws one surface sample per pixel; a graphics card keeps the blend, unchanged to the pixel on both rasterisers (`src/rendering/voxel/aoeDeGroundTier.ts`). On CI's SwiftShader the default view now costs 0.85-0.87 of Moebius's frame, where the blend costs 2.12 (run 36100866192). `tests/browser/de-ground-frame-cost.spec.ts` holds that and retires the SwiftShader lesson, and `tests/browser/de-ground-full-blend.spec.ts` keeps CI drawing the blend. A correction to the GPU figures in the lines below: on the RTX 4090 every arm took 2.2-2.7 ms a frame at 1920x1080 (Moebius, and Natural with and without its ground), so no difference is measurable there. The 0.8-0.9 ms in the step 2 line and the 1.1 ms beside it are withdrawn. Next: D1 again, Natural the default.
...
Status (2026-09-24, D1): Natural is the default look from v0.3.233 (decision D1), and Moebius is one menu row away; a stored choice is kept. The default view with no style asked for is v0.3.232's Natural frame to the pixel in six views on both rasterisers, and Moebius is unchanged to the pixel. The flip exposed one spec tuned to Moebius, the behind-building cue's brightness (the Natural cue is visible and owner-coloured but never passes 200 in blue, as before this lane). Step 2 is closed; step 3 (player colours) is next.

Status (2026-09-24, step 2): step 2 shipped in v0.3.232. In the DE style (Natural) the ground is AoE's own mesh in a scene the renderer lends the voxel runtime, in place of the voxel terrain chunks: six procedural surfaces baked once per page, soft irregular blends, sand at the water, dirt under every known building, and soft fog edges, with unexplored ground black to the pixel, known ground fading to black over about 0.7 of a tile beside it, and the edge of vision a gradient reaching at most half a tile past the visible cells. The shore surf is left out in that style until step 5 moves it into the shader; worn paths are not drawn. Moebius changed 0 pixels in every captured view on both rasterisers (default view at 1920x1080, zoom 0.7 and 2.4, a coast, a forest edge, the fog edge after 900 ticks, both showcases); the Natural views changed by the ground's share of the frame (8.29% of the default 1920x1080 frame, all of it inside the explored area and none in the HUD, 3.18% at zoom 0.7, 54% at zoom 2.4, 40% on the coast, 28% at the forest edge, 28% at the fog edge, 78% on the terrain showcase, 48% on the building showcase). A paused default frame costs 0.8 to 0.9 ms on an RTX 4090 against 1.1 ms before. The grade inside the explored area went from L=132.2, saturation 0.489 to L=126.4, 0.523; on step 1's mask, which reaches the rim of vision, from L=127.3, 0.513 to L=111.6, 0.560, most of it the soft edge. Gated by `tests/browser/de-ground.spec.ts` and three unit files, each red under mutation. Next: the default flip (D1).

Status (2026-09-24, later): step 2 is in progress in worktree `de-terrain`. Its first milestone shipped: `scripts/captureMapScreenshot.mjs` takes `RASTERISER=gpu|swiftshader`, so the terrain shader is looked at on the graphics card as well as on SwiftShader, which is what CI draws with and every capture in this plan used (section 10, "GPU rendering").
