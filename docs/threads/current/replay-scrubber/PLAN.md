...
Per DESIGN §5.4. ADRs 8-10. Stateful play mode using closure-local `replayContext`, `displayedTick`, and TPS-paced accumulation. Frame-coalesced drag scrubbing.

2026-05-05 status: closed for the controller layer. `src/game/replay/ReplayController.ts` now owns live/replay mode, stores the paused live bridge, restores the live pause state that existed before replay entry, swaps the mutable bridge cell to a replay bridge, clamps scrub targets to the same incomplete-aware upper bound as `SessionReplayer.openAt`, supports coalesced drag scrubbing via `scrubTo(tick, { coalesce: true })` plus `commitPendingScrub()`, and plays forward by submitting recorded commands for the current tick before stepping the cached replay world. `src/game/simulation/replay/makeReplayBridge.ts` wraps replay worlds in the full `SimulationBridge` read surface while making scene-frame `step(delta)` non-advancing so only `ReplayController.play()` moves replay time. `tests/replay/ReplayController.test.ts` covers mode enter/exit, prior live pause restoration, replay-entry/exit/scrub rollback, coalesced scrubbing, command-driven playback parity with `SessionReplayer.openAt`, cached playback without per-frame `openAt`, and replay bridge render/HUD reads.
