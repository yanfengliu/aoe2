...
**Status:** Draft v12 (2026-04-30). aoe2 v0.1.6 ships **Phase A (bridge-state migration) only**; Phases B-F (ReplayController, TimelinePanel UI, scrubber, playback) deferred to v0.1.7 pending architectural decision per the v12-deltas BLOCKER below.

**v12 scope decision:** iter-11 review found a fundamental mismatch — aoe2's `GameCommands = Record<string, never>` (`pureHelpers.ts:38`) means bridge methods drive state directly, never through `world.submitWithResult`. Recorded bundles have `commands.length === 0`, and `SessionReplayer.openAt(t > startTick)` throws `BundleIntegrityError('no_replay_payloads')` per `session-replayer.ts:226`. ADR 10's step-based `play()` model also wouldn't reproduce the live game (deterministic stepping ignores bridge-driven inputs). The 11 iterations of design work remain load-bearing: bridge-state migration is the foundation for any future replay model (diffs only capture `world.state` mutations, so the migration enables rich post-hoc analysis even without live playback). v0.1.6 ships the migration; the replay-UI architecture decision (commandify input vs diff-playback via BundleViewer vs other) goes to v0.1.7 with user input.

- **v0.1.6 scope (this spec, Phase A):** bridge-state migration to `world.state.aoe2.*`; per-slot codecs; `BridgeStateAccessor`; `tier3SyncSystem` + `bridgeSnapshotSystem` output tail; `bootstrapFlush`; schema-2 save format; schema-1 back-compat migration; createWorldSkeleton split; bridgeMeta slot. Enables: cleaner save files, schema-2 forward path, bridge-state visibility in BundleViewer/Hotspots for AI/agent analysis.
- **v0.1.7+ scope (deferred):** ReplayController, TimelinePanel UI, scrubber + play, hotspot pins, replay-mode bridge swap, replay-aware annotation UI. Architectural decision required first: how to navigate aoe2 bundles forward when no commands are recorded.

The replay-controller / UI sections (§5.3 createReplayWorldOnly, §5.4 ReplayController, §5.5 TimelinePanel, §5.7 Annotation UI in replay mode, ADRs 6/7/8/9/10) remain in this document as forward-looking design — they describe the v0.1.7+ surface AFTER the architectural decision lands. The Phase A migration in §5.1 / §5.2 / §5.6 is what v0.1.6 delivers.
...
## 7. Determinism / Equivalence Invariants
...
## 9. Performance considerations (v2 — corrected)
...
## 10. Optimizations deferred to v0.1.7+
