...
Increment 8-anim shipped as v0.1.108 (unit idle/walk ANIMATION): `unitRenderer.unitBobOffset` — a small idle sway / faster walk bounce of the FIGURE above its ground shadow, driven by the render clock (`this.time.now`, threaded into `drawUnit`), phased by unit id, kept within the bounding radius; pure presentation (replay-safe, no sim/Date.now/random). Increment 6b (facade doorway) shipped as v0.1.109: a dark door on the front-lit wall for tall buildings. NEXT candidates: more facade detail (windows/wall banding/per-role), fuller 3/4-view unit figures (limbs/weapon swing/attack lunge) + wildlife species differentiation, death effects, 9 (elevation/cliffs — blocked on the per-cell-elevation render contract, the one place civ-engine co-evolves). HP-bar anchor now follows the iso volume (v0.1.110). The diamond edge-feather re-add shipped as v0.1.111: `drawTerrainCell` stipples a dithered band of blend-tinted specks just inside each of the 4 diamond edges whose von-Neumann neighbour is a different kind (nudged 26% toward the centre so they stay in the tile, dithered per cell/edge/slot via `cellNoise3`, neighbour kinds looked up through a per-frame WeakMap grid), so kind-to-kind seams soften as they did on the pre-iso map. This closes the last "dropped at the coordinate switch" item; remaining iso work is the NEXT-candidate list above (facade detail / fuller figures + attack anim / death effects / elevation).
