...
Increment 7a shipped as v0.1.105 (foot units read as head+body figures): `unitRenderer.drawHead` adds a head circle above the body for the villager/infantry/archer roles (monk already upright, cavalry has a rider), kept within the bounding radius so HP-bar/selection geometry is unchanged. NEXT candidates: 6b (per-building FACADES — windows/doors/wall texture on the iso faces), 7b (fuller 3/4-view unit figures — limbs/weapon posture), 8 (idle/walk/attack animation), 9 (elevation/cliffs — blocked on the per-cell-elevation render contract), and a NEW candidate not in the original 1–9 list: RESOURCES in iso (trees/gold/stone/berries still render as flat top-down shapes — the forests are a big flat area). Also open polish: the diamond edge-feather re-add (terrain); the building HP-bar anchor could rise with the taller volume.
