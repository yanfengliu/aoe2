...
- **no-pinned-or-oscillating-units** — replays unit positions tick by tick using `bundle.initialSnapshot.entities` for the seed plus `bundle.ticks[i].diff.components.position.set` (each entry is `[entityId, Position]`) for incremental updates. For each unit with an active `unit.move` command over a sliding window of `pinnedWindowTicks` ticks, computes net Manhattan progress (`|x_end - x_start| + |y_end - y_start|`). If `progress < pinnedNetProgressCells`, fire a violation. Catches both pinned units (zero progress) and oscillating units (progress oscillates around zero).
