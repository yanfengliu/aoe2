...
This is the mechanics companion to AGENTS.md → Code review. Policy (when multi-CLI review is required, the review aspects, the reviewers-must-read directive, convergence criteria, and the thread/REVIEW.md artifact conventions) lives in AGENTS.md; this file is how to actually run the CLIs. Read this file before every multi-CLI session.
...
Bump review pins here first, per the AGENTS.md model-currency rule, with a one-line smoke test before committing; then sync the known secondary pin site `scripts/propose-fix.mjs` (loop-machinery reviewer pins: Claude per `design/spec-final.md` §15.7, plus its own hard-coded Codex model). The playtest LLM pin is separate and governed by §15.7.
...
Every prompt must additionally include the reviewers-must-read directive (quoted in AGENTS.md → Code review) and the doc-accuracy addendum (quoted in AGENTS.md → Documentation → Verification step).
